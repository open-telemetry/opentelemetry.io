...
async function imageMinifcation() {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (check) {
            task.pipe(through2.obj(checkFile))
                .pipe(gulp.dest(os.tmpdir()))
                .on('end', () => {
                    if (numFilesWithIssues > 0) {
                        console.error(`Found ${numFilesWithIssues} images that are not minified:`);
                        filesWithIssues.forEach(file => {
                            console.error(` - ${file}`);
                        });
                        reject(new Error('Some images are not minified. Run `npm run fix:images` to fix.'));
                    }
                    resolve();
                })
                .on('error', reject)
            ...
            task.pipe(imagemin())
                .pipe(through2.obj(createCacheEntry)).pipe(gulp.dest('.'))
                .on('end', () => {
                    fs.writeFileSync(
