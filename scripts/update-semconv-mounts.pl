#!/usr/bin/perl -w
#
# Script to generate module.yaml from module-base.yaml
# by calling print-spec-mounts.pl and adding the new mount entries.
#
# Reads from: $MODULE_BASE_YAML (version-controlled template)
# Writes to:  $MODULE_YAML (generated, gitignored)
...
# Configuration
my $MODULE_BASE_YAML = "config/_default/module-base.yaml";
my $MODULE_YAML = "config/_default/module.yaml";
...
sub main() {
  ...
  print "Successfully generated $MODULE_YAML with semconv mount entries\n";
}
...
sub generate_module_yaml {
  ...
  open(my $read_fh, '<', $MODULE_BASE_YAML) or die "Cannot open $MODULE_BASE_YAML: $!\n";
  my @lines = <$read_fh>;
  close($read_fh);
  ...
  die "Marker not found in $MODULE_BASE_YAML:\n$START_MARKER\n" if ($start_line == -1);
  die "Marker not found in $MODULE_BASE_YAML:\n$END_MARKER\n" if ($end_line == -1);
  ...
  open(my $write_fh, '>', $MODULE_YAML) or die "Cannot write to $MODULE_YAML: $!\n";
