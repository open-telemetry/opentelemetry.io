...
export function registerLiveChecks(registerTest = test) {
  registerTest('GET /schemas/1.40.0 → YAML response', async () => {
    const ref = baseRef();
  ...
  registerTest(
    'GET /schemas/1.40.0 → X-Asset-Fetch-Ga-Info header',
    async () => {
      const ref = baseRef();
      const url = absUrl(schemaVersionPath, ref);
      const res = await fetch(url);
  ...
  registerTest('HEAD /schemas/1.40.0 → success with empty body', async () => {
    const ref = baseRef();
    const url = absUrl(schemaVersionPath, ref);
  ...
  registerTest('GET /schemas/latest → redirect', async () => {
    const ref = baseRef();
  ...
  registerTest('GET /schemas/does-not-exist → not found', async () => {
    const ref = baseRef();
