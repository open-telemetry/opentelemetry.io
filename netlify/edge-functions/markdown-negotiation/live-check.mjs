 ...
 *
 * Not run by `npm run test:edge-functions` (that command only runs `*.test.ts` files).
 */
...
function usage() {
  const cmd = 'node netlify/edge-functions/markdown-negotiation/live-check.mjs';
  console.error(`Markdown negotiation - live checks against a deployed host.
...
Examples:
  ...
  npm run test:edge-functions:live -- 9603
