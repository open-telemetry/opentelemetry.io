...
function renderAccordion() {
  ...
  statusData.types.forEach((typeName, index) => {
    ...
    const statusSummary = ['cpp', 'go', 'java', 'js'].map((langId, index) => {
      ...
      if (typeData) {
        ...
        return `<span class="lang-status-pill status-${typeData.status}" title="${langName}: ${statusLabel}" data-position="${position}">${position}</span>`;
      }
      return `<span class="lang-status-pill status-missing" title="${langName}: No data" data-position="${position}">${position}</span>`;
    }).join('');
...
function init() {
  if (!document.getElementById('config-lang-status-accordion-container')) {
    return;
  }
