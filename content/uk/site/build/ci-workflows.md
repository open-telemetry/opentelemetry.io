...
## Встановлення залежностей {#dependency-installation}

Завдання CI встановлюють npm-залежності відповідно до [контрактів встановлення](../dependencies/#install-contracts), спільних для всього сайту: точне блокування версій та без скриптів, а завдання, які збирають сайт, повторно вмикають лише перевірену повторну збірку Hugo.
...
| [`label-manager.yml`][labels]      | `pull_request_target`, `workflow_run`, `schedule` | Токен GitHub App для редагування міток та читання на рівні org/team |
...
Скрипт [`pr-approval-labels.sh`][script] обробляє один PR (встановлюється через змінну середовища `PR`). Він викликається файлом `label-manager.yml` при подіях PR та файлом [`blog-publish-check.sh`][batch-script] у пакетному режимі.
...
2. **`label-manager`** запускається через `workflow_run` (коли попередній робочий процес відпрацював). Він запускається в контексті базового репозиторію з повним доступом до GitHub App token, завантажує артефакт та оновлює міти.

У разі змін вмісту (`opened`, `reopened`, `synchronize`), `label-manager` запускається безпосередньо через `pull_request_target`.
...
- **`label-manager`**: запускається з токеном GitHub App (`OTELBOT_DOCS_CLIENT_ID` / `OTELBOT_DOCS_PRIVATE_KEY`), що має права на читання на рівні org/team та редагує мітки PR. Використання `pull_request_target` та `workflow_run` дозволяє бути впевненим, що виконання відбувається у контексті базового репозиторію.
...
- **`/fix:refcache`** (застаріло) все ще працює, через сумісний псевдонім `fix:refcache`; коментар з результатом вказує на `/fix:link-cache`.
...
2. **`generate-patch`** (ненадійний): перевіряє гілку PR, запускає команду виправлення і завантажує артефакт латки (`site.patch`), до 1024 КБ.
...
> [!NOTE]
>
> Робочий процес [`refcache-refresh.yml`][] також запускається щодня та торкається `.lycheecache`, Отже, ці два PR від ботів можуть конфліктувати залежно від порядку злиття. Конфлікти вирішуються автоматично, оскільки обидві гілки синхронізуються з `main` під час кожного запуску. Перенесення `refcache-refresh` у модуль багаторазових дій з накладення латок — що усуває такі конфлікти за самою своєю суттю — відстежується в [плані проєкту][project plan].
...
| -------------------------- | ------------------------------------------------------------------------------------------------------------ |
| `check-links.yml`          | Збірка сайту та [перевірка посилань][link checking] за допомогою Lychee                                      |
...
| `auto-update-versions.yml` | Автоматичне оновлення версій компонентів OTel (крім [репозиторіїв специфікацій](#spec-integration-branches)) |
