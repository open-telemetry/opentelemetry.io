...
cSpell:ignore: вебхука коміту розсинхронізовані
---
...
Завдання CI встановлюють npm-залежності із зафіксованого `package-lock.json`:

- `npm run ci:min` встановлює зафіксований граф залежностей: жодні скрипти життєвого циклу не виконуються, а встановлення завершується помилкою, якщо `package.json` та файл блокування розсинхронізовані.
- Завдання, які збирають сайт, після встановлення запускають `npm run ci:prepare`, який перезбирає `hugo-extended` (єдиний повторно ввімкнений хук залежностей) і потім виконує власний етап `prepare` репозиторію.
- Devcontainer натомість використовує `npm run install:safe`: той самий контракт, але зі збереженням необовʼязкових залежностей, які містять локальні інструменти, як-от `netlify-cli`.

Деталі щодо скриптів дивіться в [npm-скриптах](../npm-scripts/).
