...
Щодо питань, які стосуються учасників локалізації — рекомендації з перекладу, відстеження змін в англомовній версії та поточна підтримка — дивіться в розділ [Локалізація сайту][Site localization]. Для версії кроків рівня репозиторію, яку виконує агент, див. навичку [`setup-new-localization` skill][].
...
LANG_ID:
  label: NativeName
  locale: LANG_ID-REGION # опціонально; див. нижче
  params:
    description: <опис сайту, перекладений на нову мову>
```

Поле `locale` є опціональним: використовуйте його, коли сайт повинен видавати регіональний тег мови, наприклад `en-US`, `pl-PL` або `zh-CN`, у RSS-каналах, наприклад; інакше використовується ідентифікатор мови. Google Translate потребує повного тегу `zh-CN` для китайської мови, тоді як більшість інших мов використовують основний субтег.

Як приклад, запис для польської мови виглядає так:
...
### Власники коду {#code-owners}

Розділ локалі у [`.github/CODEOWNERS`][] генерується з реєстру [`data/locale-teams.yaml`][]; не редагуйте його вручну.

1. Додайте локаль до реєстру, в алфавітному порядку, зазначивши ментора та початкових учасників як approvers:
   ...
   Локаль без супровідників є [unstaffed][]: її рядки CODEOWNERS також містять `@open-telemetry/docs-approvers` як запасного власника, доки локаль не отримає власну команду супровідників.

2. Перегенеруйте розділ локалі у CODEOWNERS:

   ```sh
   npm run fix:codeowners
   ```

Включіть обидві зміни до PR, який додає `content/LANG_ID/`: CI вимагає, щоб
реєстр і каталоги локалі збігалися.

**Не додавайте** записів до [`.github/component-owners.yml`][]; зміни до цього файлу більше не потрібні з червня 2026 року.
...
- [ ] **Крок 5** — `.github/component-label-map.yml` (введення міток) та `data/locale-teams.yaml` (запис реєстру) оновлено; `.github/CODEOWNERS` перегенеровано
...
- **`npm run check:codeowners`** — підтверджує, що розділ локалі у CODEOWNERS відповідає реєстру.
