...
```sh
npm run fix:link-cache  # Перевіряє посилання, оновлюючи кеш посилань
npm test                # Повний запуск тестів, включно з перевіркою посилань
```

### 3. Commit та push {#3-commit-and-push}

Якщо ваш патч був створений під час виправлення PR link-cache (наприклад, гілка `otelbot/refcache-refresh`), зафіксуйте зміни в `patches.yml` разом з оновленим `.lycheecache`, а потім виконайте force-push з lease:
