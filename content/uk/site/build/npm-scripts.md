...
weight: 20
default_lang_commit: 33d7c4949c8d636c667fea3e3f7f48996639d161
...
| `test:compound-tests`      | Виконує складені скрипти `test:*-*`.                                                               |
...
| `test:local-tools`         | Node test runner для `scripts/**/*.test.mjs`.[^local-tools-note]                                   |
| `test`                     | Виконує найбільш часто потрібні тести.                                                             |

[^local-tools-note]: Цей скрипт має складену назву, а не `test:tools`, щоб його підхоплював `test:compound-tests`.
