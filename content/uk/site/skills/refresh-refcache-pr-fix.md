...
У кроках нижче _`TARGET_BRANCH`_ — це головна гілка цільового PR.
...
1. Визначте PR, повʼязаний з upstream _`TARGET_BRANCH`_.
2. Якщо жодного не існує, зупиніться.
3. Якщо локальна гілка _`TARGET_BRANCH`_ вже існує і містить коміти, яких немає в upstream гілці, зробіть їх резервну копію або зупиніться перед скиданням будь-яких змін.
...
3. Якщо жодного не залишилося, тобто скрипт подвійної перевірки успішний:
   ...
   - Якщо `static/refcache.json` змінився, зафіксуйте зміни та надішліть їх до upstream _`TARGET_BRANCH`_ (на цьому етапі).
