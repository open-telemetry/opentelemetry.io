...
2. Файл `App.config` або `Web.config`
   ...
   - `OTEL_LOG_LEVEL`
   - `OTEL_DOTNET_AUTO_NETFX_REDIRECT_ENABLED` (Застаріле)
   ...
   > [!NOTE]
   >
   > В .NET Framework, значення `OTEL_*` з `Web.config` або `App.config` піднімаються до змінних середовища на рівні процесу під час запуску, і SDK OTel ініціалізується лише один раз на процес. В IIS, де кілька застосунків можуть ділити один робочий процес (Application Pool), це означає, що перший застосунок, який запускається, визначає конфігурацію для всіх застосунків у цьому пулі.
...
| ----------------- | ------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| `AZUREAPPSERVICE` | Детектор Azure App Service      | [Документація детектора ресурсів Azure](https://github.com/open-telemetry/opentelemetry-dotnet-contrib/blob/Resources.Azure-1.15.1-beta.1/src/OpenTelemetry.Resources.Azure/README.md)                                                      | [Експериментальний](/docs/specs/otel/versioning-and-stability) |
| `CONTAINER`       | Детектор контейнерів            | [Документація детектора ресурсів контейнерів](https://github.com/open-telemetry/opentelemetry-dotnet-contrib/blob/Resources.Container-1.15.1-beta.1/src/OpenTelemetry.Resources.Container/README.md) **Не підтримується на .NET Framework** | [Експериментальний](/docs/specs/otel/versioning-and-stability) |
| `HOST`            | Детектор хосту                  | [Документація детектора ресурсів хосту](https://github.com/open-telemetry/opentelemetry-dotnet-contrib/blob/Resources.Host-1.15.1-beta.1/src/OpenTelemetry.Resources.Host/README.md)                                                        | [Експериментальний](/docs/specs/otel/versioning-and-stability) |
| `OPERATINGSYSTEM` | Детектор операційної системи    | [Документація детектора ресурсів операційної системи](https://github.com/open-telemetry/opentelemetry-dotnet-contrib/blob/Resources.OperatingSystem-1.15.1-beta.1/src/OpenTelemetry.Resources.OperatingSystem/README.md)                    | [Експериментальний](/docs/specs/otel/versioning-and-stability) |
| `PROCESS`         | Детектор процесів               | [Документація детектора ресурсів процесів](https://github.com/open-telemetry/opentelemetry-dotnet-contrib/blob/Resources.Process-1.15.1-beta.2/src/OpenTelemetry.Resources.Process/README.md)                                               | [Експериментальний](/docs/specs/otel/versioning-and-stability) |
...
Вибірники дозволяють контролювати потенційний шум і навантаження, що вводяться інструментацією OpenTelemetry, вибираючи, які трасування ви хочете збирати та експортувати. Дивіться [специфікацію OpenTelemetry](/docs/specs/otel/configuration/sdk-environment-variables/#general-sdk-configuration) для отримання додаткової інформації.

| Змінна середовища         | Опис                                                                      | Стандартне значення     | Статус                                                  |
| ------------------------- | ------------------------------------------------------------------------- | ----------------------- | ------------------------------------------------------- |
| `OTEL_TRACES_SAMPLER`     | Вибірник, який буде використовуватися для трасувань \[1\]                 | `parentbased_always_on` | [Стабільний](/docs/specs/otel/versioning-and-stability) |
| `OTEL_TRACES_SAMPLER_ARG` | Текстове значення, яке буде використовуватися як аргумент вибірника \[2\] |                         | [Стабільний](/docs/specs/otel/versioning-and-stability) |
...
\[2\]: Для вибірників `traceidratio` та `parentbased_traceidratio`: Ймовірність вибірки, число в діапазоні [0..1], наприклад "0.25". Стандартно 1.0.
...
Щоб увімкнути експортер OTLP, встановіть змінну середовища `OTEL_TRACES_EXPORTER`/`OTEL_METRICS_EXPORTER`/`OTEL_LOGS_EXPORTER` на `otlp`.

Щоб налаштувати експортер OTLP за допомогою змінних середовища, дивіться [документацію експорту OTLP](https://github.com/open-telemetry/opentelemetry-dotnet/tree/core-1.16.0/src/OpenTelemetry.Exporter.OpenTelemetryProtocol#environment-variables). Важливі змінні середовища включають:
...
| `OTEL_EXPORTER_OTLP_LOGS_PROTOCOL`                  | Еквівалентно `OTEL_EXPORTER_OTLP_PROTOCOL`, але застосовується тільки до логів.                                                                                                                              | `http/protobuf`                                                                      | [Стабільний](/docs/specs/otel/versioning-and-stability) |
...
| `OTEL_EXPORTER_OTLP_LOGS_TIMEOUT`                   | Еквівалентно `OTEL_EXPORTER_OTLP_TIMEOUT`, але застосовується тільки до логів.                                                                                                                               | `10000` (10с)                                                                        | [Стабільний](/docs/specs/otel/versioning-and-stability) |
| `OTEL_EXPORTER_OTLP_COMPRESSION`                    | Метод стиснення для експортера OTLP. Підтримувані значення: `gzip`, `none`.                                                                                                                                  | `none`                                                                               | [Стабільний](/docs/specs/otel/versioning-and-stability) |
| `OTEL_EXPORTER_OTLP_TRACES_COMPRESSION`             | Еквівалентно `OTEL_EXPORTER_OTLP_COMPRESSION`, але застосовується тільки до трасувань.                                                                                                                       | `none`                                                                               | [Стабільний](/docs/specs/otel/versioning-and-stability) |
| `OTEL_EXPORTER_OTLP_METRICS_COMPRESSION`            | Еквівалентно `OTEL_EXPORTER_OTLP_COMPRESSION`, але застосовується тільки до метрик.                                                                                                                          | `none`                                                                               | [Стабільний](/docs/specs/otel/versioning-and-stability) |
| `OTEL_EXPORTER_OTLP_LOGS_COMPRESSION`               | Еквівалентно `OTEL_EXPORTER_OTLP_COMPRESSION`, але застосовується тільки до логів.                                                                                                                           | `none`                                                                               | [Стабільний](/docs/specs/otel/versioning-and-stability) |
...
| `OTEL_EXPORTER_OTLP_LOGS_HEADERS`                   | Еквівалентно `OTEL_EXPORTER_OTLP_HEADERS`, але застосовується тільки до логів.                                                                                                                               |                                                                                      | [Стабільний](/docs/specs/otel/versioning-and-stability) |
...
| `OTEL_LOGRECORD_ATTRIBUTE_VALUE_LENGTH_LIMIT`       | Максимально допустимий розмір значення атрибута запису журналу.                                                                                                                                              | немає                                                                                | [Стабільний](/docs/specs/otel/versioning-and-stability) |
| `OTEL_LOGRECORD_ATTRIBUTE_COUNT_LIMIT`              | Максимально допустима кількість атрибутів запису журналу.                                                                                                                                                    | 128                                                                                  | [Стабільний](/docs/specs/otel/versioning-and-stability) |
...
Дивіться [документацію Prometheus Exporter HttpListener](https://github.com/open-telemetry/opentelemetry-dotnet/tree/coreunstable-1.16.0-beta.1/src/OpenTelemetry.Exporter.Prometheus.HttpListener) для отримання додаткової інформації.
...
Щоб налаштувати експортер Zipkin за допомогою змінних середовища, дивіться [документацію експортера Zipkin](https://github.com/open-telemetry/opentelemetry-dotnet/tree/core-1.16.0/src/OpenTelemetry.Exporter.Zipkin#configuration-using-environment-variables). Важливі змінні середовища включають:
...
| `OTEL_DOTNET_AUTO_NETFX_REDIRECT_ENABLED`           | **Застаріла. Тільки .NET Framework.** Запасний варіант для `OTEL_DOTNET_AUTO_REDIRECT_ENABLED`, якщо основна змінна не встановлена. Замість цього використовуйте `OTEL_DOTNET_AUTO_REDIRECT_ENABLED`.                                                                                                                                                                                                                                                                                                              |                                  | [Застаріла](/docs/specs/otel/versioning-and-stability)         |
| `OTEL_DOTNET_AUTO_REDIRECT_ENABLED`                 | Дозволяє перенаправляти посилання на збірки на версії, не нижчі за ті, що використовуються під час автоматичної інструментації. Стандартне значення — `true` для самостійних розгортань і `false` для інших розгортань (наприклад, розгортання пакетів NuGet).                                                                                                                                                                                                                                                     |                                  | [Експериментальний](/docs/specs/otel/versioning-and-stability) |
...
| `OTEL_DOTNET_AUTO_PLUGINS`                          | Список втулків інструментації OTel SDK, розділених двокрапкою, вказаних з [assembly-qualified name](https://docs.microsoft.com/en-us/dotnet/api/system.type.assemblyqualifiedname?view=net-6.0#system-type-assemblyqualifiedname). _Примітка: Цей список повинен бути розділений двокрапкою, оскільки імена типів можуть містити коми._ Дивіться більше інформації про те, як писати плагіни на [plugins.md](https://github.com/open-telemetry/opentelemetry-dotnet-instrumentation/blob/v1.16.0/docs/plugins.md). |                                  | [Експериментальний](/docs/specs/otel/versioning-and-stability) |
| `OTEL_DOTNET_AUTO_APP_DOMAIN_STRATEGY`              | Лише .NET Framework. Визначає, як автоматична інструментація обробляє нестандартні AppDomains. Підтримувані значення: `LoaderOptimizationSingleDomain`, `AssemblyRedirect` та `None`. Дивіться [Стратегія AppDomain .NET Framework](https://github.com/open-telemetry/opentelemetry-dotnet-instrumentation/blob/v1.16.0/docs/netfx-appdomain-strategy.md) для деталей.                                                                                                                                             | `LoaderOptimizationSingleDomain` | [Експериментальний](/docs/specs/otel/versioning-and-stability) |
...
## Клієнт OpAMP {#opamp-client}

| Змінна середовища                   | Опис                 | Стандартне значення               | Статус                                                         |
| ----------------------------------- | -------------------- | --------------------------------- | -------------------------------------------------------------- |
| `OTEL_DOTNET_AUTO_OPAMP_ENABLED`    | Вмикає клієнт OpAMP. | `false`                           | [Експериментальний](/docs/specs/otel/versioning-and-stability) |
| `OTEL_DOTNET_AUTO_OPAMP_SERVER_URL` | URL сервера OpAMP.   | `https://localhost:4320/v1/opamp` | [Експериментальний](/docs/specs/otel/versioning-and-stability) |
