...
Коли OBI пригнічує оригінальний рядок, файли контейнерних логів містять рядок з NUL байтів на його місці. Для записів до 8 KiB фільтруйте ці рядки-заповнювачі на етапі доставки за допомогою `^[\x00\s]*$`. Наприклад, з приймачем `file_log` колектора OpenTelemetry:
