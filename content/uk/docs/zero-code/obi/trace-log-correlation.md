...
OBI зберігає поля `trace_id` та `span_id`, які вже існують у JSON. Для сервісу, який виявлено як такий, що безпосередньо експортує трейси OpenTelemetry, OBI вставляє лише `trace_id`: його span ID, згенерований eBPF, не буде ідентифікувати span SDK.
