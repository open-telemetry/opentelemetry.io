...
| YAML    | Змінна середовища         | Тип                 | Стандартно |
| ------- | ------------------------- | ------------------- | ---------- |
| `cidrs` | `OTEL_EBPF_NETWORK_CIDRS` | list або рядки CIDR | (empty)    |
...
В YAML, кожен запис може бути або простим рядком CIDR, або обʼєктом з полями `cidr` та `name`. Змінна середовища `OTEL_EBPF_NETWORK_CIDRS` приймає лише список CIDR, розділених комами. Наприклад:
