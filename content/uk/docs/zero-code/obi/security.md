...
## Обовʼязкове монтування хосту для поширення контексту {#required-host-mount-for-context-propagation}

Починаючи з OBI v0.12.0, на версіях ядра 6.6.128+, 6.12.75+, 6.18.14+ та 6.19+, OBI вимагає, щоб тека `/sys/kernel/tracing` хосту була змонтована у контейнер OBI за тим самим шляхом. OBI використовує це для виконання своєї перевірки компенсації FIONREAD. Якщо це монтування відсутнє, OBI не може виконати цю перевірку та автоматично вимикає поширення контексту, щоб уникнути створення некоректних даних трасування.

Щоб забезпечити це монтування в Kubernetes, додайте наступне до специфікації вашого Podʼа:
