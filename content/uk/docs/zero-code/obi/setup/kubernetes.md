...
weight: 4
default_lang_commit: 83dbea34b16a9b3ddce3fa42857f2a35d2ace5b6
# prettier-ignore
...
> [!NOTE]
>
> На версіях ядра 6.6.128+, 6.12.75+, 6.18.14+ та 6.19+, OBI v0.12.0+ вимагає, щоб тека `/sys/kernel/tracing` хосту була змонтована у контейнер OBI за тим самим шляхом. Без цього монтування OBI не може виконати свою перевірку компенсації FIONREAD та автоматично вимикає поширення контексту трасування. Усі приклади розгортання в цьому документі включають це монтування. Дивіться [Безпека, дозволи та можливості](../../security/#required-host-mount-for-context-propagation) для деталей.
...
#### Інструментування всіх процесів в поді (рекомендовано){#instrumenting-all-processes-in-a-pod-recommended}

Коли OBI працює як контейнер sidecar з `shareProcessNamespace: true`, він використовує PID простору імен пода та може бачити процеси тільки в Podʼі. Це означає, що ви можете використовувати `OTEL_EBPF_AUTO_TARGET_EXE=*` для інструментування всіх процесів в поді без необхідності вказувати окремі назви виконавчих файлів або порти.

Це рекомендований підхід для розгортання sidecar, оскільки він забезпечує просту конфігурацію, яку можна використовувати повторно і яка працює в різних подах без модифікацій:
  ...
  template:
    metadata:
      labels:
        app: goblog
    spec:
      # Потрібно щоб інструментування sidecar мало доступ до процесів сервісу
      shareProcessNamespace: true
      serviceAccountName: obi # Потрібно якщо ви бажаєте мати декорування метаданих kubernetes
      containers:
        ...
        - name: goblog
          ...
          ports:
            - containerPort: 8443
              name: https
        # Контейнер sidecar з OBI - інструментом автоматичного внструментування eBPF
        - name: obi
          image: otel/ebpf-instrument:main
          securityContext: # Потрібні прівілеї для встановлення проб eBPF
            privileged: true
          env:
            # Інструментувати всі процеси в поді (за шаблоном)
            - name: OTEL_EBPF_AUTO_TARGET_EXE
              value: '*'
            - name: OTEL_EXPORTER_OTLP_ENDPOINT
              value: 'http://otelcol:4318'
            # Потрібно якщо ви бажаєте мати декорування метаданих kubernetes
            - name: OTEL_EBPF_KUBE_METADATA_ENABLE
              value: 'true'
          volumeMounts:
            # потрібно для поширення контексту трасування, див. документацію з безпеки
            - name: tracefs
...
Використання підходу з використанням символів-замінників є менш схильним до помилок, ніж вказання окремих імен виконуваних файлів або портів, оскільки вам не потрібно оновлювати конфігурацію OBI при додаванні нових сервісів до пода. OBI може отримати доступ тільки до процесів у межах того самого пода (через спільний PID простору імен), тому немає ризику випадкового інструментування процесів поза вашим подом.

#### Інструментування окремих процесів в поді {#instrumenting-specific-processes-in-a-pod}

Якщо вам потрібно більш детальне управління процесами, які необхідно інструментувати в рамках пода, ви можете вказати імʼя виконуваного файлу або відкритий порт замість використання символу-замінника.
...
Коли OBI запускається як DaemonSet, кожен Pod OBI відкриває власні зʼєднання `list` і `watch` до сервера API Kubernetes, щоб отримати метадані, необхідні для декорування метрик і трасувань, не лише локальні метадані вузла, але й метадані з усього кластера. Це робиться для збагачення інформації поза локальним вузлом, наприклад, для додавання атрибутів [peer](/docs/specs/semconv/registry/attributes/service/#service-attributes-for-peer-services) до відрізків, що роблять запити між вузлами кластера. У великих кластерах такий розподіл може створювати значне навантаження на сервер API, до точки, коли це може вплинути на весь кластер.

Щоб уникнути цього, OBI постачає додатковий сервіс з назвою `k8s-cache`. Він запускається як невеликий Deployment, спостерігає за API Kubernetes один раз від імені всіх Pod OBI і передає метадані їм через gRPC. Це усуває трафік інформерів OBI на кожен Pod до сервера API і значно зменшує навантаження на API, хоча OBI все ще може виконувати обмежені прямі запити до API Kubernetes для метаданих вузлів і кластера.

Використання `k8s-cache` завжди рекомендується, особливо якщо:

- Ви запускаєте OBI як DaemonSet на великому кластері.
- Ви запускаєте багато реплік OBI (великий `Deployment`, кілька sidecar тощо) на одному кластері.
- Сервер API Kubernetes під навантаженням або обмежений за швидкістю.

Якщо ви не налаштуєте адресу кешу, кожен екземпляр OBI зберігає власні локальні інформери в процесі, що підходить для невеликих кластерів.

`k8s-cache` актуальний лише при запуску OBI на Kubernetes; він не впливає на автономні або Docker-налаштування.

Щоб використовувати кеш, розгорніть його та вкажіть OBI адресу його `Service` за допомогою змінної середовища `OTEL_EBPF_KUBE_META_CACHE_ADDRESS` (або `attributes.kubernetes.meta_cache_address` у YAML). Найпростіший спосіб — використовувати Helm-чарт OBI, який налаштовує `Deployment`, `Service` та підключення OBI для вас, коли ви встановлюєте `k8sCache.replicas` на ненульове значення.

Якщо ви віддаєте перевагу ручному розгортанню, кеш публікується як контейнерний образ `ghcr.io/open-telemetry/opentelemetry-ebpf-instrumentation/opentelemetry-ebpf-k8s-cache`. Мінімальний маніфест виглядає так:
  ...
  template:
    metadata:
      labels:
        app: k8s-cache
    spec:
      serviceAccountName: obi # потребує list/watch на pods, nodes, services
      containers:
        - name: k8s-cache
...
Потім вкажіть OBI адресу кешу з DaemonSet:
...
Зазвичай достатньо однієї репліки. Для високої доступності запустіть кілька реплік за одним `Service` — кожен Pod OBI підключається до одного і підключається до іншого у разі відмови.
