...
| `max_path_segment_cardinality` | Максимальна кількість унікальних значень на сегмент шляху та сервіс у режимі `low-cardinality`. `0` вимикає обмеження. Див. [режим низької кардинальності](#low-cardinality-mode). | ціле число    | `10`       |
...
- `low-cardinality` застосовує евристичні правила, а також обмежує унікальні значення сегментів шляху на сервіс за допомогою `max_path_segment_cardinality`
...
Якщо ви явно встановите `unmatched: ''`, OBI обробляє порожнє значення як `wildcard`. Коли `unmatched` пропущено, стандартна конфігурація OBI використовує `heuristic`.
...
## Режим низької кардинальності {#low-cardinality-mode}

`unmatched: low-cardinality` спочатку застосовує евристичний класифікатор, а потім відстежує унікальні значення для кожного сегмента шляху на сервіс. Після досягнення сегментом `max_path_segment_cardinality`, OBI замінює нові унікальні значення на `wildcard_char`. Встановіть ліміт на `0`, щоб вимкнути це обмеження кардинальності.
