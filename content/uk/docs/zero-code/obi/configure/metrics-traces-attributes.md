...
# prettier-ignore
cSpell:ignore: kube kubecache kubeconfig OpenShift replicaset statefulset Зменшіть висококардинальну мультиплексує
---

> [!NOTE]
>
> Ця сторінка використовує імена полів Config v1 та приклади. Для Config v2 дивіться [довідку по Config v2](../config-v2/). Для конвертації наявного файлу використайте [посібник з міграції](../migrate-to-config-v2/).
...
За допомогою підрозділу `select` ви контролюєте атрибути метрик, опціональні атрибути трейсів та атрибути ресурсів. Кожен запис має дві властивості: `include` та `exclude`.
...
attributes:
  select:
    ...
    http_client_request_duration:
      ...
      exclude: ['k8s.pod.*']
    resource:
      # залишити стандартний набір ресурсів, крім цих висококардинальних полів
...
## Вибір ресурсів {#resource-selection}

Використовуйте спеціальний ключ `resource` для фільтрації атрибутів ресурсів на ресурсах OTLP та на метриках Prometheus `target_info` та `traces_target_info`. Коли `include` пропущено, OBI зберігає стандартні виявлені атрибути ресурсів та видаляє лише відповідні записи `exclude`. Атрибути ресурсів, виявлені під час виконання, включаючи Kubernetes та значення `OTEL_RESOURCE_ATTRIBUTES`, використовують той самий вибір. Вибір фільтрує лише атрибути, які в іншому випадку підлягають експорту; для атрибутів Prometheus, не відомих під час запуску, продовжуйте використовувати `prometheus_export.extra_resource_attributes`.

## Вибір трейсу {#trace-selection}

Для експортованих OpenTelemetry трейсів використовуйте ключ `traces` (не назву метрики) в `attributes.select`. Він контролює декорування трейсів, таке як `db.query.text`, `graphql.document`, `url.query`, атрибути GenAI payload та `db.response.error`.
...
`url.query` стандартно увімкнено, коли HTTP-запит містить рядок запиту. Щоб пропустити його, додайте `url.query` до `attributes.select.traces.exclude`. OBI також зберігає рядок запиту в `url.full` клієнтського відрізка та автоматично замінює значення відомих конфіденційних ключів на `REDACTED`.

Використовуйте `attributes.sensitive_query_params`, щоб розширити або звузити вбудований список чутливих параметрів (з урахуванням регістру):
...
Еквівалентними змінними середовища є `OTEL_EBPF_SENSITIVE_QUERY_PARAMS_ADD` та `OTEL_EBPF_SENSITIVE_QUERY_PARAMS_REMOVE` зі значеннями, розділеними комами. Вбудований список охоплює поширені облікові дані, токени автентифікації, паролі, платіжні ідентифікатори та підписані параметри URL AWS та Google Cloud.

Аргументи та результати викликів інструментів MCP доступні як опціональні атрибути трейсів `gen_ai.tool.call.arguments` та `gen_ai.tool.call.result`. Оскільки ці значення можуть містити конфіденційні дані, вони вимагають точного запису в `attributes.select.traces.include`; записи з символами підстановки, як-от `gen_ai.*`, їх не вмикають.
...
Колишній псевдонім `http` було видалено. Застаріле значення `ip` не має ефекту; використовуйте `tcp` для поширення через TCP-опції.
...
Поширення на мережевому рівні підтримує gRPC шляхом впровадження та аналізу потокових заголовків HPACK `traceparent`. Поширення через TCP-опції не використовується для gRPC, оскільки HTTP/2 мультиплексує кілька контекстів трасування через одне зʼєднання. Поширення контексту загального не-gRPC HTTP/2 залишається обмеженим інструментуванням бібліотеки Go. Для не-Go сервісів gRPC постійні зʼєднання, встановлені до запуску OBI, можуть не бути розпізнані для поширення.
...
| YAML<br>змінна середовища                                                   | Опис                                                                                                                                                                                                                  | Тип            | Стандартно     |
| --------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- | -------------- |
| `enable`<br>`OTEL_EBPF_KUBE_METADATA_ENABLE`                                | Вмикає чи вимикає декорацію метаданих Kubernetes. Встановіть `autodetect`, щоб увімкнути, якщо ви працюєте в Kubernetes. Для отримання додаткової інформації див. розділ [увімкнення Kubernetes](#enable-kubernetes). | boolean/string | `autodetect`   |
| `cluster_name`<br>`OTEL_EBPF_KUBE_CLUSTER_NAME`                             | Перевизначає виявлене імʼя кластера Kubernetes.                                                                                                                                                                       | string         | (empty)        |
...
| `meta_cache_address`<br>`OTEL_EBPF_KUBE_META_CACHE_ADDRESS`                 | Адреса зовнішнього сервісу `k8s-cache` звідки потрібно отримувати метадані Kubernetes. Для отримання додаткової інформації див. розділ [адреса кешу мета](#meta-cache-address).                                       | string         | (empty)        |
...
Коли `cluster_name` порожній, OBI намагається використати мітки вузлів Kubernetes, власний ресурс OpenShift Infrastructure та метадані хмарного провайдера для Amazon EC2, Google Cloud та Microsoft Azure, у такому порядку. Атрибут залишається порожнім, якщо жодне з цих джерел не надає імені.
...
### Адреса кешу метаданих {#meta-cache-address}

Коли встановлено, OBI отримує метадані Kubernetes з зовнішнього сервісу `k8s-cache` через gRPC замість запуску власних інформерів проти API-сервера Kubernetes. Це рекомендується для великих кластерів та розгортань DaemonSet, щоб уникнути перевантаження API Kubernetes.
