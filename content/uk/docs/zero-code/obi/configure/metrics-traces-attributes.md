...
# prettier-ignore
cSpell:ignore: kube kubecache kubeconfig replicaset statefulset Зменшіть збільшіть
---
...
### Розширення HTTP-заголовків для елементів відрізків {#http-header-enrichment-for-spans}

OBI може прикріплювати вибрані HTTP-заголовки до елементів відрізків через секцію конфігурації `ebpf.payload_extraction.http.enrichment`. Це корисно, коли ви хочете передавати заголовки бізнес-логіки або заголовки маршрутизації у трейсах без ручного інструментування застосунку.

Механізм розширення працює на основі правил:

- Встановіть `enabled: true`, щоб увімкнути розширення заголовків HTTP.
- Використовуйте `policy.default_action`, щоб визначити, чи слід включати заголовки, що не відповідають критеріям, чи виключати їх. Стандартним значенням є `exclude`.
- Використовуйте `policy.match_order`, щоб керувати оцінкою правил. Стандартним значенням є `first_match_wins`.
- Використовуйте правила `obfuscate`, щоб замаскувати конфіденційні значення, при цьому залишаючи ключ заголовка у фрагменті.
...
Наступні змінні середовища керують стандартними параметрами політики:
...
Правила налаштовуються в YAML. Якщо ви очікуєте великі заголовки, збільшіть `ebpf.buffer_sizes.http`, щоб OBI міг захоплювати відповідні значення.
