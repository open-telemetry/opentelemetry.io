...
> [!NOTE]
>
> Ця сторінка використовує імена полів Config v1 та приклади. У Config v2 налаштуйте ці настройки під `capture.engine` та специфічні для протоколу розділи; див. [довідку по Config v2](../config-v2/). Для конвертації наявного файлу використайте [посібник з міграції](../migrate-to-config-v2/).
...
| ------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------- | ---------- |
| `wakeup_len`<p>`OTEL_EBPF_BPF_WAKEUP_LEN`</p>                       | Встановлює, скільки повідомлень OBI накопичує в eBPF буфері викликів перед надсиланням запиту на пробудження в простір користувача. Див. [довжина пробудження](#wake-up-length).                                                                 | int     | 500        |
| `stats_wakeup_data_bytes`<p>`OTEL_EBPF_STATS_WAKEUP_DATA_BYTES`</p> | Встановлює мінімальну кількість байтів у черзі перед пробудженням споживача буфера виклику статистики TCP. Див. [поріг пробудження статистики](#stats-wake-up-threshold).                                                                        | int     | 4096       |
...
## Поріг пробудження статистики {#stats-wake-up-threshold}

`stats_wakeup_data_bytes` керує буфером виклику статистики TCP незалежно від `wakeup_len`. Вищі значення зменшують пробудження простору користувача під навантаженням за рахунок затримки доставки метрик. Встановіть значення `0`, щоб пробуджувати споживача для кожної події. Тримайте значення значно нижчим за розмір буфера виклику, щоб поставлені в чергу події були оброблені до заповнення буфера.
