...
> [!NOTE]
>
> Ця сторінка використовує імена полів Config v1, приклади та змінні середовища. Для Config v2 дивіться [довідку по Config v2](../config-v2/). Для конвертації наявного файлу використайте [посібник з міграції](../migrate-to-config-v2/).
...
## Точка доступу перевірки справності {#health-check-endpoint}

OBI може відкривати `/healthz` через TCP або сокет Unix. JSON-відповідь повідомляє версію схеми, поточний час Unix та час роботи процесу. Точка доступу вимкнена за замовчуванням.
...
| YAML<br>змінна середовища                                                    | Опис                                                                                                                                                  | Тип    | Стандартно |
| ---------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | ---------- |
| `health_check.port`<br>`OTEL_EBPF_HEALTH_CHECK_PORT`                         | Відкриває точку доступу перевірки справності на вказаному порту TCP. `0` вимикає перевірки справності через TCP.                                      | int    | `0`        |
| `health_check.unix_socket_path`<br>`OTEL_EBPF_HEALTH_CHECK_UNIX_SOCKET_PATH` | Привʼязує точку доступу перевірки справності до абсолютного шляху файлової системи або абстрактного сокета з префіксом `@`. Має пріоритет над `port`. | string | (unset)    |

Наприклад, `unix_socket_path: /var/run/obi-health.sock` створює сокет файлової системи, а `unix_socket_path: '@obi-health'` використовує абстрактний сокет.
