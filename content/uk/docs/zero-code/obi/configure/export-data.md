...
# prettier-ignore
cSpell:ignore: AsterixDB couchbase genai gonic jackc libcudart memcached nats pgxpool pyserver Qwen rerank segmentio spanmetrics Самопосилання
---
...
| gRPC          | `1.0+`                                                                             | Довготривалі зʼєднання, розпочаті до запуску OBI, можуть використовувати `*` для імен методів.                                                        |
...
| GenAI         | OpenAI, Anthropic, Gemini, AWS Bedrock, Qwen, MCP, embedding APIs, and rerank APIs | Для отримання корисного навантаження, специфічного для певного провайдера, необхідно встановити відповідний прапорець `ebpf.payload_extraction.http`. |
...
| NGINX           | Validated on NGINX `1.27.5` and `1.29.7` | Це версії NGINX, які наразі покриті документованою валідацією.                                                          |
...
- `genai`: GenAI метрики клієнтів (OpenAI, Anthropic, Gemini, AWS Bedrock, Qwen, MCP та інші підтримувані API для вбудовування та повторного ранжування)
...
- `genai`: GenAI трейси клієнтів (OpenAI, Anthropic, Gemini, AWS Bedrock, Qwen, MCP та підтримувані API для вбудовування та повторного ранжування)
...
- `genai`: Метрики клієнтів GenAI (OpenAI, Anthropic, Gemini, AWS Bedrock, Qwen, MCP та підтримувані API для вбудовування та повторного ранжування)
