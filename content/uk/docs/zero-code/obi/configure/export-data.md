...
# prettier-ignore
cSpell:ignore: Aerospike AsterixDB Chroma couchbase genai gonic jackc libcudart memcached Milvus nats Ollama pgxpool Pinecone pyserver Qdrant Qwen rerank segmentio spanmetrics sunrpc Weaviate Zilliz Самопосилання
---
...
## Сумісність інструментування {#instrumentation-compatibility}

OBI підтримує наступні версії протоколів та функцій для інструментування трасувань та метрик:

| Застосування  | Підтримувані версії                                                                                                                 | Примітки                                                                                                                          |
| :------------ | :---------------------------------------------------------------------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------- |
| HTTP          | `1.0/1.1`                                                                                                                           | Підтримується поширення контексту.                                                                                                |
| HTTP          | `2.0`                                                                                                                               | Поширення контексту вимагає інструментування на рівні бібліотеки Go.                                                              |
| gRPC          | `1.0+`                                                                                                                              | Поширення контексту підтримується. Довготривалі зʼєднання, розпочаті до запуску OBI, можуть використовувати `*` для імен методів. |
| MySQL         | All                                                                                                                                 | Підготовлені запити, створені до запуску OBI, можуть не включати текст запиту.                                                    |
| PostgreSQL    | All                                                                                                                                 | Підготовлені запити, створені до запуску OBI, можуть не включати текст запиту.                                                    |
| MSSQL         | All                                                                                                                                 | Підготовлені запити, створені до запуску OBI, можуть не включати текст запиту.                                                    |
| Redis         | All                                                                                                                                 | Наявні зʼєднання можуть не включати номер бази даних або `db.namespace`.                                                          |
| MongoDB       | `5.0+`                                                                                                                              | Стиснуті дані не підтримуються.                                                                                                   |
| Couchbase     | All                                                                                                                                 | Імена кошиків або колекцій можуть бути недоступні, якщо узгодження завершилось до запуску OBI.                                    |
| Aerospike     | All                                                                                                                                 | Стиснуті дані не розбираються; значення записів та блоків не захоплюються.                                                        |
| Memcached     | All                                                                                                                                 | Підтримується підмножина ASCII текстового протоколу, за винятком команд `quit` та meta.                                           |
| Kafka         | All                                                                                                                                 | Пошук імен тем може не вдаватися для версій API вибірки `13+`.                                                                    |
| MQTT          | `3.1.1/5.0`                                                                                                                         | Навантаження не захоплюється.                                                                                                     |
| NATS          | All                                                                                                                                 | Додаткових обмежень версій не задокументовано.                                                                                    |
| AMQP          | `1.0`                                                                                                                               | Лише перформативні операції створюють відрізки.                                                                                   |
| SunRPC        | All                                                                                                                                 | Підтримує ONC RPC через TCP; UDP не підтримується. RPCSEC_GSS приховує аргументи процедур.                                        |
| GraphQL       | All                                                                                                                                 | Додаткових обмежень версій не задокументовано.                                                                                    |
| Elasticsearch | `7.14+`                                                                                                                             | Додаткових обмежень версій не задокументовано.                                                                                    |
| OpenSearch    | `3.0.0+`                                                                                                                            | Додаткових обмежень версій не задокументовано.                                                                                    |
| AWS S3        | All                                                                                                                                 | Додаткових обмежень версій не задокументовано.                                                                                    |
| AWS SQS       | All                                                                                                                                 | Додаткових обмежень версій не задокументовано.                                                                                    |
| SQL++         | All                                                                                                                                 | Додаткових обмежень версій не задокументовано.                                                                                    |
| GenAI         | OpenAI, Anthropic, Gemini, AWS Bedrock, Qwen, Ollama, OpenAI-compatible gateways, MCP, embedding, rerank, and vector retrieval APIs | Для отримання корисного навантаження, специфічного для провайдера, потрібен відповідний прапорець `ebpf.payload_extraction.http`. |

Деякі інструментування на рівні застосунків також залежать від конкретних версій середовища виконання, бібліотек або серверів:

| Застосування    | Підтримувані версії                      | Примітки                                                                                                                |
| :-------------- | :--------------------------------------- | :---------------------------------------------------------------------------------------------------------------------- |
| Застосунки Go   | Go `1.17+`                               | Застосовується до інструментування на рівні бібліотек Go; поширення контексту на рівні бібліотек Go вимагає Go `1.18+`. |
| Застосунки Java | JDK `8+`                                 | Додаткових обмежень версій середовища виконання не задокументовано.                                                     |
| NGINX           | Перевірено на NGINX `1.27.5` та `1.29.7` | Це версії NGINX, які наразі покриті документованою валідацією.                                                          |

### Сумісність інструментування бібліотек Go {#go-library-instrumentation-compatibility}

OBI підтримує наступні бібліотеки Go та мінімальні версії для інструментування застосунків:

| Бібліотека                       | Підтримувані версії             |
| :------------------------------- | :------------------------------ |
| `net/http`                       | `>= 1.17`                       |
...
Версії, зазначені в цих таблицях, є версіями, які OBI явно підтримує. Інші версії також можуть працювати, але вони не входять до документованого обсягу підтримки, якщо не зазначено інше.

Щоб увімкнути аналіз корисного навантаження пошуку векторів, встановіть `ebpf.payload_extraction.http.genai.retrieval.enabled: true` або `OTEL_EBPF_HTTP_RETRIEVAL_ENABLED=true`. Підтримувані API включають Pinecone, Qdrant, Milvus та Zilliz, Chroma та Weaviate. Аналіз корисного навантаження також вимагає ненульового значення `ebpf.buffer_sizes.http`. У версії v0.10.0 максимальний розмір захоплення HTTP становить 262144 байти на напрямок запиту; захоплення залишається стандартно вимкненим.

### Сумісність інструментування GPU {#gpu-instrumentation-compatibility}

OBI підтримує інструментування GPU, коли середовище відповідає [вимогам сумісності OBI](/docs/zero-code/obi/#compatibility) і застосунок використовує підтримувану бібліотеку виконання CUDA.

| Вимога                    | Підтримується                  |
| :------------------------ | :----------------------------- |
| Операційна система        | Linux                          |
| Архітектура CPU           | `amd64`, `arm64`               |
| Бібліотека виконання CUDA | `libcudart.so` для CUDA `7.0+` |

OBI інструментує наступні операції CUDA:
...
Інструментування GPU застосовується лише до застосунків, які використовують підтримувану бібліотеку виконання CUDA та операції, зазначені вище. Інші API GPU, фреймворки або бібліотеки не входять до документованого обсягу підтримки, якщо не зазначено інше.
...
| ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------- | ----------------- |
| `features`<br>`OTEL_EBPF_METRICS_FEATURES` | Список груп метрик, для яких OBI експортує дані, див. [функції експорту метрик](#metrics-export-features). Прийнятні значення: `all`, `*`, `application`, `application_span`, `application_span_otel`, `application_span_sizes`, `application_host`, `application_runtime`, `application_service_graph`, `network`, `network_flow_packets`, `network_inter_zone`, `stats`, `stats_tcp_rtt`, `stats_tcp_failed_connections`, `stats_tcp_retransmits`, `stats_tcp_io` та `ebpf`. | список рядків | `["application"]` |
...
- `application_runtime`: Метрики середовища виконання Go, пам’яті JVM HotSpot та циклу подій Node.js, зібрані з інструментованих служб. Див. [метрики середовища виконання](#runtime-metrics).
- `application_jvm`: Метрики памʼяті HotSpot JVM. Також необхідно увімкнути `jvm_runtime_metrics`. Див. [метрики середовища виконання](#runtime-metrics).
...
- `application_span_otel`: метрики відрізків трасування на рівні застосунку в форматі OpenTelemetry (наприклад, `traces.span.metrics.calls`); `span_metrics` є окремим.
...
- `network_flow_packets`: Метрики лічильників мережевих пакетів. Для отримання додаткової інформації див. документацію з конфігурації [мережевих метрик](../../network/).
...
- `stats`: Усі TCP статистичні метрики.
- `stats_tcp_rtt`: Метрики гістограми часу кругового проходження TCP.
- `stats_tcp_failed_connections`: Лічильник невдалих TCP-зʼєднань, з мітками за причиною невдачі.
- `stats_tcp_retransmits`: Метрики лічильників повторної передачі TCP.
- `stats_tcp_io`: Байти TCP, передані на рівні сокетів, з мітками за напрямком введення/виведення. Ця функція спостерігає кожен виклик TCP send та recv і може мати більші накладні витрати, ніж інші функції статистики.
- `ebpf`: Метрики часу виконання eBPF для завантажених проб і карт, доступні через експортер Prometheus та внутрішній репортер метрик.

> [!NOTE]
>
> Починаючи з версії v0.11.0, при прямому експорті даних OTLP використовується нотація OpenTelemetry з крапками для таких назв метрик:
>
> - `target_info` тепер `target.info`.
> - `traces_target_info` тепер `traces.target.info`.
> - `traces_host_info` тепер `traces.host.info`.
> - `traces_span_metrics_calls_total` тепер `traces.span.metrics.calls`.
> - `traces_span_metrics_duration` тепер `traces.span.metrics.duration`.
>
> Назви Prometheus метрик залишаються незмінними. Оновіть OTLP запити та дашборди при оновленні до v0.11.0.
...
## Метрики середовища виконання {#runtime-metrics}

OBI може збирати метрики середовища виконання без змін SDK у цільовому процесі.

Увімкніть метрики середовища виконання Go, HotSpot JVM та Node.js за допомогою функції метрик `application_runtime`. Значення середовища виконання Go передаються після того, як цільовий процес завершить цикл очищення пам’яті, тому новий процес може не надсилати ці метрики одразу. Зміни параметрів `GOGC`, `GOMEMLIMIT` або `GOMAXPROCS` відображаються після завершення наступного циклу.

Для Node.js OBI зчитує показники часу роботи циклу подій, завантаженості та затримки один раз на секунду. Для зчитування показників часу роботи циклу подій та завантаженості потрібна версія Node.js 14.10 або пізніша. Для зчитування показників затримки потрібна версія Node.js 16.14 або пізніша. OBI зчитує показники лише для циклу подій головного потоку, а інспектор повинен бути доступним, щоб OBI міг вбудувати агент збору показників виконання.

Використовуйте `jvm_runtime_metrics.sampling_interval`, щоб керувати частотою, з якою OBI вимірює показники пам’яті HotSpot JVM:
...
| YAML<br>змінна середовища                                          | Опис                                                   | Тип      | Стандартно |
| ------------------------------------------------------------------ | ------------------------------------------------------ | -------- | ---------- |
| `sampling_interval`<br>`OBI_JVM_RUNTIME_METRICS_SAMPLING_INTERVAL` | Встановлює, як часто OBI вибірково збирає метрики JVM. | Duration | `1s`       |
...
| YAML<br>змінна середовища                                                                | Опис                                                                                                                                                                                                                                                                                                                                      | Тип             | Стандартно                          |
| ---------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------- | ----------------------------------- |
| `endpoint`<br>`OTEL_EXPORTER_OTLP_METRICS_ENDPOINT`                                      | Точка доступу, на яку OBI надсилає метрики. Підтримує HTTP(S), gRPC та сокети Unix, такі як `unix:///var/run/otel.sock` або `unix://@otel`.                                                                                                                                                                                               | URL             |                                     |
...
- `nats`: NATS метрики повідомлень publish/subscribe
- `amqp`: AMQP 1.0 метрики повідомлень publish/process
- `couchbase`: Метрики запитів Couchbase N1QL/SQL++ та протоколу метрик KV (Key-Value), що базується на протоколі memcached
- `memcached`: Метрики протоколу Memcached ASCII
- `genai`: GenAI метрики клієнтів (OpenAI, Anthropic, Gemini, AWS Bedrock, Qwen, MCP та підтримувані API для вбудовування, повторного ранжування та пошуку векторів)
- `gpu`: метрики продуктивності GPU
- `mongo`: метрики викликів клієнтів MongoDB
- `dns`: метрики DNS-запитів
- `sunrpc`: Метрики клієнта та сервера ONC RPC через TCP
...
| YAML<br>змінна середовища                                                           | Опис                                                                                                                                                                                                                                                                             | Тип             | Стандартно                                                                                                   |
| ----------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------- | ------------------------------------------------------------------------------------------------------------ |
| `endpoint`<br>`OTEL_EXPORTER_OTLP_TRACES_ENDPOINT`<br>`OTEL_EXPORTER_OTLP_ENDPOINT` | Точка доступу, на яку OBI надсилає трейси. Підтримує сокети Unix, такі як `unix:///var/run/otel.sock` або `unix://@otel`. Коли використовується `OTEL_EXPORTER_OTLP_ENDPOINT`, OBI дотримується стандарту OpenTelemetry і автоматично додає шлях `/v1/traces` для HTTP експорту. | URL             |                                                                                                              |
| `protocol`<br>`OTEL_EXPORTER_OTLP_TRACES_PROTOCOL`<br>`OTEL_EXPORTER_OTLP_PROTOCOL` | Протокол транспорту/кодування OpenTelemetry, зверніться до [протоколу експорту трейсів](#traces-export-protocol). [Прийняті значення](/docs/languages/sdk-configuration/otlp-exporter/#otel_exporter_otlp_protocol) `http/json`, `http/protobuf` та `grpc`.                      | string          | Визначається за використанням порту                                                                          |
...
| `instrumentations`<br>`OTEL_EBPF_TRACES_INSTRUMENTATIONS`                           | Список інструментації, для якої OBI збирає дані, зверніться до розділу [інструментації трейсів](#traces-instrumentation).                                                                                                                                                        | list of strings | `http`, `grpc`, `sql`, `redis`, `kafka`, `mqtt`, `nats`, `amqp`, `mongo`, `couchbase`, `memcached`, `sunrpc` |
...
- `nats`: NATS трейси повідомлень publish/subscribe
- `amqp`: AMQP 1.0 трейси повідомлень publish/process
- `couchbase`: Трейси запитів Couchbase N1QL/SQL++ та трейси протоколу KV (Key-Value), з текстом запиту та деталями операцій
- `memcached`: Трейси протоколу Memcached ASCII
- `genai`: GenAI трейси клієнтів (OpenAI, Anthropic, Gemini, AWS Bedrock, Qwen, MCP та підтримувані API для вбудовування, повторного ранжування та пошуку векторів)
- `gpu`: GPU трейси продуктивності
- `mongo`: MongoDB трейси викликів клієнтів
- `dns`: DNS трейси запитів
- `sunrpc`: Трейси клієнта та сервера ONC RPC через TCP
...
##### Протокол KV (Key-Value) {#kv-key-value-protocol}

**Що відстежується**:

| Атрибут                   | Джерело                    | Приклад             |
| ------------------------- | -------------------------- | ------------------- |
| `db.system.name`          | Стала                      | `couchbase`         |
| `db.operation.name`       | Код операції               | `GET`, `SET`        |
| `db.namespace`            | Кошик                      | `travel-sample`     |
| `db.collection.name`      | Область дії + Колекція     | `inventory.airline` |
| `db.collection.name`      | Колекція                   | `airline`           |
| `db.response.status_code` | Код стану (у разі помилки) | `1`                 |
| `server.address`          | Інформація про зʼєднання   | Server hostname     |
| `server.port`             | Інформація про зʼєднання   | `11210`             |
...
- Якщо `SELECT_BUCKET` відбувається до запуску OBI, імʼя кошика для цього зʼєднання невідоме
...
##### Операції SQL++ (N1QL) {#sql-n1ql-operations}
...
| Атрибут                   | Джерело                                   | Приклад                                |
| ------------------------- | ----------------------------------------- | -------------------------------------- |
| `db.system.name`          | Заголовок версії N1QL                     | `couchbase` або `other_sql`            |
| `db.operation.name`       | Парсер SQL                                | `SELECT`, `INSERT`, `UPDATE`           |
| `db.namespace`            | Шлях до таблиці / `query_context`         | `travel-sample`                        |
| `db.collection.name`      | Шлях до таблиці                           | `inventory.airline`                    |
| `db.query.text`           | Тіло запиту                               | Повний текст запиту SQL++              |
| `db.response.status_code` | Код помилки (у разі помилки)              | `12003`                                |
| `error.type`              | Повідомлення про помилку (у разі помилки) | Повідомлення про помилку від Couchbase |
...
3. Одного ідентифікатора: розглядається як імʼя колекції (з `query_context`) або імʼя bucket (без `query_context`, у старій версії)
...
- `memcached`: Метрики протоколу Memcached ASCII
- `genai`: Метрики клієнтів GenAI (OpenAI, Anthropic, Gemini, AWS Bedrock, Qwen, MCP та підтримувані API для вбудовування, повторного ранжування та пошуку векторів)
- `gpu`: метрики продуктивності GPU
- `mongo`: метрики викликів клієнтів MongoDB
- `dns`: метрики DNS-запитів
- `sunrpc`: Метрики клієнта та сервера ONC RPC через TCP
