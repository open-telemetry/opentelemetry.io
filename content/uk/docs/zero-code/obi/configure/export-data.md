...
# prettier-ignore
cSpell:ignore: AsterixDB Chroma couchbase genai gonic jackc libcudart memcached Milvus nats pgxpool Pinecone pyserver Qdrant Qwen segmentio spanmetrics sunrpc Weaviate Zilliz Самопосилання
---
...
| GenAI         | OpenAI, Anthropic, Gemini, AWS Bedrock, Qwen, MCP, API вбудовування, повторного ранжування та пошуку векторів | Для отримання корисного навантаження, специфічного для певного провайдера, необхідно встановити відповідний прапорець `ebpf.payload_extraction.http`. |
...
| ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- | ----------------- |
| `features`<br>`OTEL_EBPF_METRICS_FEATURES` | Список груп метрик, для яких OBI експортує дані, див. [функції експорту метрик](#metrics-export-features). Прийнятні значення: `all`, `*`, `application`, `application_span`, `application_span_otel`, `application_span_sizes`, `application_host`, `application_runtime`, `application_jvm`, `application_service_graph`, `network`, `network_flow_packets`, `network_inter_zone`, `stats`, `stats_tcp_rtt`, `stats_tcp_failed_connections`, `stats_tcp_retransmits`, `stats_tcp_io` та `ebpf`. | список рядків | `["application"]` |
...
- `application_runtime`: Метрики середовища виконання Go, зібрані з інструментованих Go-сервісів. Див. [метрики середовища виконання](#runtime-metrics).
...
Увімкніть метрики середовища виконання Go за допомогою функції метрик `application_runtime`. OBI звітує про значення після завершення цільовим процесом циклу збирання сміття, тому новий процес може не генерувати ці метрики одразу. Зміни `GOGC`, `GOMEMLIMIT` або `GOMAXPROCS` зʼявляються після наступного завершеного циклу.

Метрики середовища виконання JVM вимагають як функції метрик `application_jvm`, так і `jvm_runtime_metrics.enabled: true`:
...
| YAML<br>змінна середовища                                          | Опис                                                             | Тип      | Стандартно |
| ------------------------------------------------------------------ | ---------------------------------------------------------------- | -------- | ---------- |
| `enabled`<br>`OBI_JVM_RUNTIME_METRICS_ENABLED`                     | Вмикає збір метрик середовища виконання HotSpot JVM.             | boolean  | `false`    |
| `sampling_interval`<br>`OBI_JVM_RUNTIME_METRICS_SAMPLING_INTERVAL` | Встановлює, як часто OBI вибірково збирає увімкнені метрики JVM. | Duration | `1s`       |
