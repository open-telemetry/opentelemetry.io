...
# prettier-ignore
cSpell:ignore: filestorecsi kube-node-lease kube-system rdns replicaset statefulset testserver volumepopulator самоінструментування мапінг
---

> [!NOTE]
>
> Ця сторінка використовує імена полів Config v1 та приклади. У Config v2 використовуйте `capture.policy` та `capture.rules`; дивіться [довідку по Config v2](../config-v2/). Для конвертації наявного файлу використайте [посібник з міграції](../migrate-to-config-v2/).
...
| `process_context_poll_interval`<br>`OTEL_EBPF_PROCESS_CONTEXT_POLL_INTERVAL`                                     | Встановлює, як часто OBI перевіряє інструментований процес на наявність оновлених атрибутів ресурсів та метаданих, опублікованих через експериментальний мапінг контексту процесу `OTEL_CTX`. Встановіть `0`, щоб перевіряти лише коли OBI вперше виявляє процес.                                                                                                              | duration        | `1s`                                                                                                  |
...
4. У Kubernetes атрибути ресурсу обчислюються з метаданих власника Pod, у такому порядку (згідно з їх доступністю):
   ...
   - `k8s.pod.name`
   - `k8s.container.name`
5. Для Java застосунків, перше доступне значення з імені застосунку Spring Boot, заголовка JAR маніфесту або базової назви JAR.
6. Виконуване імʼя інструментованого процесу.

Ви можете перевизначити мітки Kubernetes з попереднього пункту 3 через конфігурацію.

## Збагачення контексту процесу

Починаючи з OBI v0.12.1, OBI читає рядкові атрибути ресурсів та метадані
з процесів, що публікують експериментальний мапінг контексту процесу OpenTelemetry `OTEL_CTX`. OBI перевіряє при першому виявленні процесу та потім перевіряє на оновлення раз на секунду. Явно сконфігуровані імена сервісів та просторів імен продовжують мати пріоритет.

У Config v1 використовуйте `discovery.process_context_poll_interval` або
`OTEL_EBPF_PROCESS_CONTEXT_POLL_INTERVAL` для зміни інтервалу опитування. Встановіть
інтервал на `0`, щоб зберегти початкову перевірку та вимкнути пізніші опитування. Config
v2 використовує стандартний інтервал у v0.12.1 і не виставляє цю настройку.
