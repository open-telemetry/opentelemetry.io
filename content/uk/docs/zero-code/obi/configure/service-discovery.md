...
# prettier-ignore
cSpell:ignore: beyla filestorecsi kube-node-lease kube-system rdns replicaset statefulset volumepopulator самоінструментування
---
...
| `default_exclude_instrument`                                                                                     | Вимикає інструментування самого OBI, OpenTelemetry Collector та інших компонентів спостереження. Встановіть в пусте значення, щоб дозволити OBI інструментувати себе та ці інші компоненти. Див. розділ [стандартне виключення сервісів з інструментування](#default-exclude-services-from-instrumentation) для отримання деталей. | list of objects | Шлях: `{*beyla,*alloy,*prometheus-config-reloader,*obi,*otelcol,*otelcol-contrib,*otelcol-contrib[!/]*}` та певні простори імен Kubernetes |
...
| `cmd_args`             | Вибирає процеси для інструментування за їх аргументами командного рядка.                                                                               | string (glob)                | (unset)    |
| `languages`            | Вибирає процеси для інструментування за виявленим мовами програмування виконуваного файлу. Використовуйте малі літери, наприклад `go` або `{go,java}`. | string (glob)                | (unset)    |
| `target_pids`          | Дозволяє вибирати процеси за PID. Коли не порожній, PID процесу повинен бути у цьому списку (на додаток до будь-яких критеріїв шляху/порту).           | comma separated list of pids | (unset)    |
...
- Сервіси, виключені за `exe_path`: `*beyla`, `*alloy`, `*obi`, `*otelcol`, `*otelcol-contrib`, `*otelcol-contrib[!/]*`.
