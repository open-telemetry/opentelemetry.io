...
# prettier-ignore
cSpell:ignore: asyncio chanrecv chansend HPACK uvloop самопосилання
---
...
Для gRPC OBI впроваджує потоковий заголовок HPACK `traceparent`. Це працює для різних мов програмування та зберігає окремі контексти трасування для одночасних потоків HTTP/2. OBI не використовує TCP-опції для gRPC, оскільки TCP-опції обмежені зʼєднанням і не можуть представляти кілька мультиплексованих потоків. Поширення контексту загального не-gRPC HTTP/2 залишається обмеженим інструментуванням бібліотеки Go.
