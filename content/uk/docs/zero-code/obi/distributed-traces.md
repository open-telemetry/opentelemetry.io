...
| :----------------------------------------- | :---------------------------------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Поширення контексту на рівні мережі        | Середовища Linux, які відповідають [вимогам сумісності OBI](/docs/zero-code/obi/#compatibility) | Працює для HTTP-трафіку між різними мовами програмування. Для HTTPS поширення обмежене іншими сервісами, інструментованими OBI, і може бути порушене проксі або балансувальниками навантаження L7. gRPC та HTTP/2 не підтримуються на рівні мережі. |
...
gRPC та HTTP/2 наразі не підтримуються на мережевому рівні.

Якщо вам потрібен більш точний контроль, `context_propagation` також приймає значення `headers`, `tcp` та `headers,tcp`. `http` приймається як псевдонім для `headers`.
