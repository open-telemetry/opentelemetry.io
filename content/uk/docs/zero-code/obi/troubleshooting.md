...
Починаючи з OBI v0.6.0, `telemetry.sdk.name` відображає базовий SDK, коли це можливо, а OBI ідентифікує себе за допомогою `telemetry.distro.name`.
...
## Міграція до v0.7.0: Зміни в визначенні мережевих портів {#migration-to-v070-network-port-guessing-changes}

OBI v0.7.0 вводить критичну зміну: **визначення мережевих портів тепер стандартно вимкнено**. Ця зміна покращує точність мережевих метрик, не роблячи припущень про невідомих ініціаторів у мережевих потоках.

### Що змінилося {#what-changed}

У версіях v0.6.0 і раніше OBI намагався визначити, яка точка доступу була клієнтом, а яка сервером у мережевих потоках, де ініціатор не міг бути визначений. Це здогадування базувалося на порядкових евристиках (зазвичай припускаючи, що нижчий номер порту був сервером, а вищий номер порту був клієнтом).

У версії v0.7.0 це здогадування стандартно вимкнено, що означає:

- Атрибути `client.port` і `server.port` можуть бути порожніми для потоків, де OBI не може визначити ініціатора.
- Мережеві метрики будуть точнішими, але можуть втратити інформацію для невідомих потоків.

### Як мігрувати {#how-to-migrate}

Якщо ви залежите від старої поведінки і хочете, щоб `client.port` і `server.port` визначалися навіть тоді, коли ініціатор невідомий, повторно увімкніть здогадування портів за допомогою порядкових евристик:
...
[документацію з конфігурації мережі](../network/config/).

### Рекомендація {#recommendation}

Ми рекомендуємо залишати здогадування портів вимкненим, якщо у вас немає конкретного випадку використання, який цього вимагає. Поведінка за замовчуванням забезпечує більш чисті, точні мережеві метрики, які менш схильні до неправильного класифікування.

### Клієнтські метрики або відрізки, що помилково приписуються неправильному сервісу {#client-metrics-or-spans-attributed-to-the-wrong-service}

Коли OBI працює з доступом до простору імен PID хосту (наприклад, `pid: host` у Docker Compose або `hostPID: true` у Kubernetes) і ви вибираєте сервіси за [відкритим портом](../configure/service-discovery/#open-ports), ви можете побачити неочікувані вихідні (клієнтські) метрики або відрізки, що приписуються одному з ваших сервісів. Поширеним симптомом є виклики Docker Engine API, такі як `GET /v1.43/containers/json`, що звітуються як клієнтські запити з беззмістовним `server.port` та `telemetry.sdk.language=go`, навіть коли цільовий сервіс не написаний на Go:
...
Коли ви публікуєте порт контейнера (наприклад, `-p 7773:7773` у Docker або запис `ports:` у Compose), процес переадресації на стороні хосту прослуховує цей порт у просторі імен мережі хосту. Залежно від середовища виконання контейнера це може бути `docker-proxy` (Docker із увімкненим проксі на рівні користувача) або еквівалентний агент.

Оскільки OBI бачить процеси хосту, а проксі-сервер прослуховує той самий порт, який ви вибрали, OBI зіставляє проксі-сервер із вашими критеріями `open_ports` і інструментує його під ідентифікатором вашого сервісу. І проксі-сервер (у просторі імен мережі хосту), і ваш реальний сервіс (у просторі імен мережі контейнера) дійсно прослуховують цей порт, тому їх неможливо відрізнити лише за портом. Передавач зазвичай є бінарним файлом на Go, саме тому відрізки містять
`telemetry.sdk.language=go`. Будь-який трафік, який генерує сам передавач, наприклад, виклики API Docker Engine, які він здійснює через сокет Docker, потім приписується вашому сервісу.

**Рішення:**

Виключіть передавача з інструментування за шляхом виконуваного файлу:
...
Або вибирайте сервіси з більш специфічним критерієм, ніж відкритий порт, щоб передавач на стороні хосту, який випадково використовує той самий порт, не був зіставлений. Наприклад, використовуйте [шлях виконуваного файлу](../configure/service-discovery/#executable-path).
