...
##### Використовуйте Gunicorn post-fork для програмного автоінструментування {#use-gunicorn-post-fork-for-programmatic-auto-instrumentation}

Якщо ви використовуєте Gunicorn, ви можете ініціалізувати OpenTelemetry з [програмним автоінструментуванням](https://github.com/open-telemetry/opentelemetry-python-contrib/blob/main/opentelemetry-instrumentation/README.rst#programmatic-auto-instrumentation) як частину ваших post-fork хуків, замість `opentelemetry-instrument`. Наприклад, у файлі `gunicorn_config.py`, що знаходиться поруч з `your_app.py`:
