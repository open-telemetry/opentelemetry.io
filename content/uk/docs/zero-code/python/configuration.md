...
- `OTEL_PYTHON_LOG_AUTO_INSTRUMENTATION`: контролює, чи обробник логів налаштований автоматично (true, false), стандартно увімкнено. Дивіться [Автоінструментування Логів](/docs/zero-code/python/logs-example/)
- `OTEL_PYTHON_LOG_CODE_ATTRIBUTES`: для вимикання додаткових атрибутів у `code` (`code.file.path`, `code.function.name`, `code.line.number`) в логах (true, false)
...
> До OpenTelemetry Python 1.40.0 автоматичне інструментування логів було стандартно вимкнено і реалізовано в пакунку `opentelemetry-sdk`. Встановлення `OTEL_PYTHON_LOGGING_AUTO_INSTRUMENTATION_ENABLED` на `true` вмикало цю функцію.
