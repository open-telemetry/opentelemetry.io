...
- використання `GlobalConfigProvider` для доступу до декларативних значень конфігурації у власному коді (перетворюючи `GlobalOpenTelemetry.get()` на `ExtendedOpenTelemetry`)
