...
Якщо ви хочете почати міграцію з Prometheus до OpenTelemetry, або якщо вас цікавить використання OpenTelemetry Collector для збору метрик Prometheus, дивіться [Prometheus Receiver](../collector/components/#prometheus-receiver).
...
Для встановлення колектора ми будемо використовувати [Helm-чарт OpenTelemetry Collector](../helm/collector/), який має кілька опцій конфігурації, що полегшать налаштування колектора. Якщо ви не знайомі з Helm, ознайомтеся з [сайтом проєкту Helm](https://helm.sh/uk/). Якщо вас цікавить
використання оператора Kubernetes, дивіться [OpenTelemetry Operator](../operator/), але це керівництво зосередиться на Helm-чарті.
...
- [Kubernetes Attributes Processor](../collector/components/#kubernetes-attributes-processor): для додавання метаданих Kubernetes до вхідної телеметрії застосунків.
- [Kubeletstats Receiver](../collector/components/#kubeletstats-receiver): для отримання метрик вузлів, podʼів та контейнерів з API-сервера на kubelet.
- [Filelog Receiver](../collector/components/#filelog-receiver): для збору логів Kubernetes та логів застосунків, записаних у stdout/stderr.
...
- [Kubernetes Cluster Receiver](../collector/components/#kubernetes-cluster-receiver): для збору метрик на рівні кластера та подій обʼєктів.
- [Kubernetes Objects Receiver](../collector/components/#kubernetes-objects-receiver): для збору обʼєктів, таких як події, з API-сервера Kubernetes.

Розгляньмо їх детальніше.

### Kubernetes Cluster Receiver

[Kubernetes Cluster Receiver](../collector/components/#kubernetes-cluster-receiver) є рішенням колектора для збору метрик про стан кластера в цілому. Цей приймач може збирати метрики про умови вузлів, фази podʼів, перезапуски контейнерів, доступні та бажані розгортання та багато іншого.

### Kubernetes Objects Receiver

[Kubernetes Objects Receiver](../collector/components/#kubernetes-objects-receiver) є рішенням колектора для збору обʼєктів Kubernetes як логів. Хоча можна збирати будь-які обʼєкти, загальним та важливим випадком використання є збір подій Kubernetes.
