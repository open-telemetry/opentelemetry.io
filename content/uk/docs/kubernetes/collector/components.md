...
Процесор атрибутів Kubernetes використовує API Kubernetes для виявлення всіх podʼів, що працюють у кластері, і зберігає запис їх IP-адрес, UID podʼів та цікавих метаданих. Стандартно дані, що проходять через процесор, асоціюються з podʼом через IP-адресу вхідного запиту, але можуть бути налаштовані різні правила. Оскільки процесор використовує API Kubernetes, він потребує спеціальних дозволів (див. приклад нижче). Якщо ви використовуєте [Helm чарт OpenTelemetry Collector](../../helm/collector/), ви можете використовувати [пресет `kubernetesAttributes`](../../helm/collector/#kubernetes-attributes-preset) для початку роботи.
...
Існують різні методи автентифікації, але зазвичай використовується службовий обліковий запис. Службовий обліковий запис також потребує правильних дозволів для отримання даних з Kubelet (див. нижче). Якщо ви використовуєте [Helm чарт OpenTelemetry Collector](../../helm/collector/), ви можете використовувати [пресет `kubeletMetrics`](../../helm/collector/#kubelet-metrics-preset) для початку роботи.
...
Приймач filelog складається з операторів, які зʼєднуються разом для обробки журналу. Кожен оператор виконує просту відповідальність, таку як аналіз мітки часу або JSON. Налаштування приймача filelog не є тривіальним. Якщо ви використовуєте [Helm чарт OpenTelemetry Collector](../../helm/collector/), ви можете використовувати [пресет `logsCollection`](../../helm/collector/#logs-collection-preset) для початку роботи.
...
Існують різні методи автентифікації, але зазвичай використовується службовий обліковий запис. Службовий обліковий запис також потребує правильних дозволів для отримання даних з API сервера Kubernetes (див. нижче). Якщо ви використовуєте [Helm чарт OpenTelemetry Collector](../../helm/collector/), ви можете використовувати [пресет `clusterMetrics`](../../helm/collector/#cluster-metrics-preset) для початку роботи.
...
Наразі для автентифікації можна використовувати лише службовий обліковий запис. Службовий обліковий запис також потребує правильних дозволів для отримання даних з API сервера Kubernetes (див. нижче). Якщо ви використовуєте [Helm чарт OpenTelemetry Collector](../../helm/collector/) і хочете отримувати події, ви можете використовувати [пресет `kubernetesEvents`](../../helm/collector/#cluster-metrics-preset) для початку роботи.
...
Приймач метрик хосту збирає метрики з хосту, використовуючи різні скрепери. Існує деякий перетин з [Приймачем kubeletstats](#kubeletstats-receiver), тому якщо ви вирішите використовувати обидва, можливо, варто відключити ці дублюючі метрики.

У Kubernetes приймач потребує доступу до тому `hostfs` для коректної роботи. Якщо ви використовуєте [Helm чарт OpenTelemetry Collector](../../helm/collector/), ви можете використовувати [пресет `hostMetrics`](../../helm/collector/#host-metrics-preset) для початку роботи.

Доступні скрепери:

| Скрепер    | Підтримувані ОС       | Опис                                            |
| ---------- | --------------------- | ------------------------------------------------|
| cpu        | Усі, крім Mac[^1]     | Метрики використання CPU                        |
| disk       | Усі, крім Mac[^1]     | Метрики вводу/виводу диска                      |
...
| processes  | Linux, Mac            | Метрики кількості процесів                      |
| process    | Linux, Windows, Mac   | Метрики CPU, памʼяті та вводу/виводу диска для кожного процесу |

[^1]:
    Не підтримується на Mac, коли компілюється без cgo, що є стандартним для образів, випущених SIG колектора.
