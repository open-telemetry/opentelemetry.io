...
- Обробка недоступності точки доступу/колектора:
  - Якщо колектор-споживач (Gateway) не працює, повідомлення просто накопичуються в топіку Kafka (в межах лімітів зберігання Kafka). Поки Kafka працює, це не впливає на колектора (Агента), що відправляє повідомлення, поки Kafka працює.
...
1.  Відсутність мережі + тайм-аут: Кінцева точка нижчого рівня недоступна довше, ніж заданий `max_elapsed_time` у налаштуваннях `retry_on_failure`.
