...
- Сценарій втрати даних:
  ...
  - Таймаут повторів: Якщо точка доступу залишається недоступною довше, ніж налаштована максимальна тривалість повторної спроби (стандартно 5 хвилин), колектор припинить повторні спроби для найстаріших даних у черзі і видалить їх.
...
{{% alert title="Порада: Використовуйте WAL для вибраних колекторів" %}}
...
- Як це працює: Один екземпляр колектора (агент) експортує дані до топіку Kafka за допомогою експортера Kafka. Інший екземпляр Колектора (Gateway) споживає дані з топіка Kafka за допомогою приймача Kafka.
- Обробка недоступності точки доступу/колектора:
  - Якщо споживаючий колектор (Gateway) не працює, повідомлення просто накопичуються в топіку Kafka (в межах лімітів зберігання Kafka). Поки Kafka працює, це не впливає на колектора (Агента), що відправляє повідомлення, поки Kafka працює.
...
1.  Відсутність мережі + таймаут: Кінцева точка нижчого рівня недоступна довше, ніж заданий `max_elapsed_time` у налаштуваннях `retry_on_failure`.
2.  Відсутність мережі + переповнення черги: Кінцева точка недоступна, а черга надсилання (в памʼяті або постійна) переповнюється до того, як кінцева точка відновлюється. Нові дані втрачаються.
3.  Збій колектора (без постійного збереження): Екземпляр колектора аварійно завершує свою роботу, а він використовував лише чергу відправлення в памʼяті. Дані в памʼяті втрачено.
...
3.  Налаштуйте розмір черги та повторні спроби: Налаштуйте параметри `queue_size` і `retry_on_failure` відповідно до очікуваного навантаження, ресурсів памʼяті/диска та допустимого часу простою кінцевих точок.
