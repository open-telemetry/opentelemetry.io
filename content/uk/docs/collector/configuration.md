...
Ви можете налаштувати один або кілька конекторів за допомогою розділу `connectors` файлу конфігурації Collector. За замовчуванням жодні конектори не налаштовані. Кожен тип конектора призначений для роботи з однією або кількома парами типів даних і може використовуватися лише для зʼєднання конвеєрів відповідно.
...
## Розділ сервісу {#service}

Розділ `service` використовується для налаштування, які компоненти увімкнені в Collector на основі конфігурації, знайденої в розділах приймачів, процесорів, експортерів і розширень. Якщо компонент налаштований, але не визначений у розділі `service`, то він не увімкнений.

Розділ сервісу складається з трьох підрозділів:
...
Ви можете використовувати один і той самий приймач, процесор або експортер у більше ніж одному конвеєрі. Коли процесор посилається в кількох конвеєрах, кожен конвеєр отримує окремий екземпляр процесора.
...
На стороні агента, це приклад, який робить OTLP експортер отримувати OIDC токени, додаючи їх до кожного RPC, зробленого до віддаленого Collector:
