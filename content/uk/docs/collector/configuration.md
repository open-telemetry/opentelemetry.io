...
# prettier-ignore
cSpell:ignore: cfssl cfssljson configtls fluentforward gencert genkey initca oidc pprof prodevent prometheusremotewrite spanevents unredacted upsert zpages напр невалідну
---
...
> [!WARNING]
>
> Хоча зазвичай краще привʼязувати точки доступу до `localhost`, коли всі клієнти локальні, наші приклади конфігурацій використовують "невизначену" адресу `0.0.0.0` для зручності. Collector стандартно використовує `localhost`. Для отримання деталей щодо будь-якого з цих виборів як значення конфігурації кінцевої точки, див. [Захист від атак на відмову в обслуговуванні][Safeguards against denial of service attacks].
