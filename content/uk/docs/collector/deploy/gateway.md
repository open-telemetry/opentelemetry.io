...
# prettier-ignore
cSpell:ignore: hostnames loadbalancer loadbalancing resourcedetectionprocessor субключ
---
...
У загальному випадку ви можете використовувати готовий балансувальник навантаження для розподілу навантаження між Колекторами. У цій однорівневій схемі балансувальник навантаження є зовнішнім щодо екземплярів Колектора:

![Однорівневий шлюз, де застосунок надсилає OTLP через зовнішній балансувальник навантаження до реплік Колектора, а потім до бекенду](../../img/otel-gateway-sdk.svg)
...
На наступній діаграмі показано цю дворівневу конфігурацію. На відміну від зовнішнього балансувальника навантаження на попередній діаграмі, компонент із позначкою **Load Balancer** тут є Колектором першого рівня, який використовує експортер з балансуванням навантаження:

![Дворівневий шлюз, де застосунок надсилає OTLP до Колектора першого рівня, який використовує експортер з балансуванням навантаження, що маршрутизує до реплік Колектора другого рівня, а потім до бекенду](../../img/otel-gateway-lb-sdk.svg)

1. У застосунку SDK налаштовано на надсилання даних OTLP до центрального місця.
2. Колектор першого рівня налаштований на використання експортера з балансуванням навантаження для розподілу сигналів між групою Колекторів другого рівня.
3. Колектори другого рівня надсилають телеметричні дані до одного або кількох бекендів.
...
Припустимо, у вас є три колектори (`collector1`, `collector2` та `collector3`), налаштовані, і ви хочете балансувати трафік між ними за допомогою NGINX, ви можете використовувати наступну конфігурацію:
...
- Метрика `M1` отримана в `T2` з міткою часу 13:56:24 зі значенням `120`
- Метрика `M1` отримана в `T3` з міткою часу 13:56:04 зі значенням `110`
- Метрика `M1` отримана в 13:56:24 зі значенням `120`
- Метрика `M1` отримана в 13:56:04 зі значенням `110`
