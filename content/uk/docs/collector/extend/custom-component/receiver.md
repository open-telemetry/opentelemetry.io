...
Спочатку скористайтесь підручником [Створення власного Collectorʼа](/docs/collector/extend/ocb/), щоб створити екземпляр Collector з назвою `otelcol-dev`; все, що вам потрібно, це скопіювати `builder-config.yaml`, описаний у [Кроці 2](/docs/collector/extend/ocb/#step-2---create-a-builder-manifest-file), і запустити збирач. У результаті у вас повинна зʼявитися структура тек, як показано нижче:
