...
Тут ви дізнаєтеся, як усувати проблеми, повʼязані зі станом і продуктивністю OpenTelemetry Collector.
...
- Як телеметрія змінюється (наприклад, чи застосовується вибірка або редагування)?
...
## Усунення несправностей у середовищах Kubernetes {#troubleshooting-in-kubernetes-environments}

Під час запуску OpenTelemetry Collector у Kubernetes ви можете використовувати [ефемерні контейнери для налагодження](https://kubernetes.io/docs/tasks/debug/debug-application/debug-running-pod/#ephemeral-container) для розслідування проблем, повʼязаних із Collector.
...
Часто проблеми з експортом даних повʼязані з мережею, наприклад, через налаштування брандмауера, DNS або проксі. Колектор підтримує [роботу через проксі](https://github.com/open-telemetry/opentelemetry-collector/tree/main/exporter#proxy-support).
