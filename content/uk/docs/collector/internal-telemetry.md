...
#### Префікс `otelcol_` {#otelcol_-prefix}
...
#### Крапки (`.`) чи підкреслення (`_`) {#dots--v-underscores-_}
...
{{% alert title="Зміни рівня метрик пакетних процесорів" %}}
...
{{% alert title="Примітка" color="info"%}} Метрики `http*` та `rpc*` не охоплюються наведеними нижче рівнями зрілості, оскільки вони не перебувають під контролем Collector SIG.
