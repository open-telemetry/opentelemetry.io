...
Всі доступні опції дивіться в [Опції експортера OTLP](#otlp-exporter-options).
...
Додаткові параметри наведено у [example configuration][kitchen-sink-config]; зверніть увагу, що розділ `tracer_provider` там відповідає `traces` тут. Деталі про опції експортера OTLP дивіться [нижче](#otlp-exporter-options).

[kitchen-sink-config]: https://github.com/open-telemetry/opentelemetry-configuration/blob/v0.3.0/examples/kitchen-sink.yaml

### Опції експортера OTLP {#otlp-exporter-options}

Наступні [опції](https://github.com/open-telemetry/opentelemetry-go-contrib/blob/otelconf/v0.23.0/otelconf/v0.3.0/generated_config.go#L256) доступні для експортера OTLP для всіх трьох сигналів. Деякі додаткові опції доступні [для метрик](#otlp-exporter-options-metrics).
...
| Назва поля           | Стандартне значення                                       | Опис                                                                                                                                                                                                                                                                            |
| -------------------- | --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `endpoint`           | `localhost:4317` (gRPC), `localhost:4318` (http/protobuf) | Цільова URL-адреса для надсилання телеметрії, наприклад `https://backend:4318`. Для `http/protobuf` будь-який шлях у URL передається експортеру; якщо шлях не вказано, використовується стандартний шлях, специфічний для сигналу (`/v1/traces`, `/v1/metrics` або `/v1/logs`). |
| `protocol`           | (обовʼязкове)                                             | Транспортний протокол. Підтримувані значення: `grpc`, `http/protobuf`.                                                                                                                                                                                                          |
| `compression`        |                                                           | Алгоритм стиснення, що застосовується перед надсиланням. Підтримувані значення: `gzip`, `none`.                                                                                                                                                                                 |
| `timeout`            | `10000`                                                   | Час очікування в мілісекундах для кожної спроби експорту.                                                                                                                                                                                                                       |
| `headers`            |                                                           | Список пар ключ-значення, що надсилаються як заголовки запиту. Кожен запис вимагає поля `name` та `value`.                                                                                                                                                                      |
| `headers_list`       |                                                           | Заголовки у форматі [W3C Baggage](https://www.w3.org/TR/baggage/) (наприклад, `key1=value1,key2=value2`). Коли встановлені одночасно `headers` та `headers_list`, `headers` має пріоритет для окремих заголовків.                                                               |
| `certificate`        |                                                           | Шлях до файлу сертифіката CA у форматі PEM, що використовується для перевірки сертифіката сервера.                                                                                                                                                                              |
| `client_certificate` |                                                           | Шлях до файлу сертифіката клієнта у форматі PEM для mTLS. Обовʼязкове, коли встановлено `client_key`.                                                                                                                                                                           |
| `client_key`         |                                                           | Шлях до файлу закритого ключа у форматі PEM для сертифіката клієнта. Обовʼязкове, коли встановлено `client_certificate`.                                                                                                                                                        |
| `insecure`           | `false`                                                   | Застосовується лише до протоколу `grpc`. Коли `true`, вимикає TLS для gRPC зʼєднань, де схема адреси не є `http` або `https`. Для `http/protobuf` TLS буде увімкнено, якщо адреса не використовує схему `http`, незалежно від цієї опції.                                       |

> [!NOTE]
>
> Внутрішній експортер OTLP реалізовано в Go SDK, який використовується Колектором. Хоча Go SDK підтримує [конфігурацію через змінні середовища](/docs/languages/sdk-configuration/otlp-exporter/), програмна конфігурація колектора має пріоритет, тому рекомендується використовувати YAML-конфігурацію колектора, щоб уникнути неочікуваної поведінки.

#### Додаткові опції для метрик {#otlp-exporter-options-metrics}

Наступні [опції](https://github.com/open-telemetry/opentelemetry-go-contrib/blob/otelconf/v0.23.0/otelconf/v0.3.0/generated_config.go#L288) застосовуються лише до експортера метрик OTLP (`metrics::readers[*].periodic.exporter.otlp`).

| Назва поля               | Стандартне значення | Опис                                                                                                                                                                                                                                                                                            |
| ------------------------ | ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `temporality_preference` | `cumulative`        | Темпоральність агрегації для інструментів метрик. Підтримувані значення: `cumulative` (всі інструменти), `delta` (дельта для лічильників, гістограм та спостережуваних лічильників; кумулятивна для всіх інших), `lowmemory` (дельта для лічильників та гістограм; кумулятивна для всіх інших). |
...
Рівні зрілості телеметрії Колектора застосовуються до всієї власної телеметрії, створеної Колектором. Бібліотеки сторонніх розробників, у тому числі OpenTelemetry Go, не охоплюються цими рівнями зрілості.
