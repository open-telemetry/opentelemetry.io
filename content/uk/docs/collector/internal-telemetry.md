...
> [!NOTE]
>
> При ручному налаштуванні експортера Prometheus для внутрішніх метрик (за допомогою `readers`) `otelcol_process_uptime` може бути експортований як `otelcol_process_uptime_seconds_total`, якщо `without_type_suffix` і `without_units` не встановлені в значення `true`. Незалежно від цього, у переглядах використовуйте значення `instrument_name` `otelcol_process_uptime` (ім'я OTLP). Щоб контролювати суфікси, специфічні для Prometheus, див. [Суфікси одиниць виміру](#unit-suffixes).
...
Стандартно і тільки для Prometheus, експортер Prometheus додає суфікс `_total` до сумарних метрик, щоб дотримуватися правил іменування Prometheus, наприклад `otelcol_exporter_send_failed_spans_total`. Цю поведінку можна вимкнути, встановивши `without_type_suffix: true` у конфігурації експортера Prometheus.
...
#### `_seconds` та інші суфікси одиниць виміру {#unit-suffixes}

Експортер Prometheus додає суфікс одиниці виміру до метрик, які містять одиницю виміру. Наприклад, `otelcol_process_uptime` (одиниця виміру: секунди) може бути експортований як `otelcol_process_uptime_seconds_total` — спочатку додається суфікс одиниці виміру `_seconds`, а потім суфікс лічильника `_total`.

Експортер Prometheus, стандартно налаштований колектором (коли не вказано `readers`), вже встановлює `without_type_suffix` та `without_units` на `true` для зворотної сумісності, тому `otelcol_process_uptime` використовується без змін.

Однак, коли ви вручну налаштовуєте експортер Prometheus під `service::telemetry::metrics::readers`, ці опції не встановлюються стандартно. Щоб зберегти оригінальні, коротші назви метрик, явно встановіть обидві опції на `true`:
...
З цією конфігурацією `otelcol_process_uptime_seconds_total` експортується як `otelcol_process_uptime`.
...
| `otelcol_exporter_in_flight_requests`                  | Кількість запитів на експорт, які наразі обробляються, включаючи повторні спроби.          | UpDownCounter |
...
| `rpc.client.call.duration`                            | Вимірює тривалість вихідних викликів віддалених процедур (RPC). | Histogram |
| `rpc.server.call.duration`                            | Вимірює тривалість вхідних викликів віддалених процедур (RPC).  | Histogram |

#### Приналежність метрик, що генеруються {#ownership-of-emitted-metrics}

Деякі метрики не належать до SIG Collector, а деякі обмежені певними
компонентами.

**`http*` та `rpc` метрики**

Ці метрики не перебувають під контролем SIG Collector, і, отже, не охоплюються наведеними нижче рівнями зрілості.

**`rpc` метрики**

Внутрішні RPC метрики Колектора походять з upstream [`otelgrpc`](https://github.com/open-telemetry/opentelemetry-go-contrib/tree/main/instrumentation/google.golang.org/grpc/otelgrpc) інструментування, яке відстежує [семантичні домовленості RPC OpenTelemetry](/docs/specs/semconv/rpc/rpc-metrics/). Набір RPC метрик, що генеруються Колектором, змінювався з випуску в випуск:

| Версія Колектора     | Згенеровані RPC метрики                                                                                                                                      |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
...
| v0.147.0             | `rpc.client.call.duration`, `rpc.server.call.duration`, `rpc.*.request.size`, `rpc.*.response.size` (метрики `*_per_rpc` застарілі та більше не генеруються) |
| v0.148.0 and later   | тільки `rpc.client.call.duration`, `rpc.server.call.duration`                                                                                                |

Метрики RPC size більше не генеруються Колектором з версії v0.148.0 або пізнішої. [RPC semantic conventions v1.40.0](https://github.com/open-telemetry/semantic-conventions/releases/tag/v1.40.0) застаріли їх через неоднозначні визначення та непослідовну реалізацію.

**`otelcol_processor_batch_*` метрики**

Ці метрики унікальні для `batchprocessor`.

**метрики пакунку `helper`**

Метрики `otelcol_receiver_`, `otelcol_scraper_`, `otelcol_processor_` та `otelcol_exporter_` походять з відповідних пакунків `helper`. Таким чином, деякі компоненти, які не використовують ці пакунки, можуть не генерувати їх.
