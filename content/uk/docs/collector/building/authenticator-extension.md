...
Автентифікатори є звичайними розширеннями, які також задовольняють один або більше інтерфейсів, повʼязаних з механізмом автентифікації:
...
Серверний автентифікатор — це, по суті, розширення з функцією `Authenticate`, яка отримує заголовки корисного навантаження як параметр. Якщо автентифікатор може автентифікувати вхідне зʼєднання, він повинен повернути помилку `nil`, або конкретну помилку, якщо не може. Як розширення, автентифікатор повинен переконатися, що ініціалізує всі необхідні ресурси під час фази [`Start`](https://pkg.go.dev/go.opentelemetry.io/collector/component#Component), і очікується, що він очистить їх під час фази `Shutdown`.
...
Клієнтський автентифікатор — це той, який реалізує один або більше з наступних інтерфейсів:
...
Власні автентифікатори повинні бути частиною того ж бінарного файлу, що й основний колектор. При створенні власного автентифікатора, вам, ймовірно, доведеться створити власний дистрибутив або надати засоби для ваших користувачів, щоб використовувати ваше розширення як частину їхніх власних дистрибутивів. На щастя, створення власного дистрибутиву можна зробити за допомогою утиліти [OpenTelemetry Collector Builder](https://github.com/open-telemetry/opentelemetry-collector/tree/main/cmd/builder).
