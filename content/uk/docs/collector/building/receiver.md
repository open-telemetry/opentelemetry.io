...
- `context.Context`: посилання на `context.Context` колектора, щоб ваш приймач трейсіів міг належним чином керувати контекстом виконання.
...
- Додано поле `zap.Logger` з іменем `logger`, щоб мати доступ до посилання на логгер збирача з приймача.
...
- Додано змінну з назвою `logger` та ініціалізовано її логгером колектора, який доступний у вигляді поля з назвою `Logger` у посиланні `receiver.Settings`.
...
Хоча це і трейс Jaeger, але він був згенерований конвеєром тресів у Колекторі, тому ви можете використати його, щоб дізнатися деякі речі про модель даних тресів OTel:

- Трейс складається з одного або декількох відрізків, структурованих в рамках ієрархії для представлення залежностей.
- Проміжки можуть представляти операції в межах сервісу та/або між сервісами.
...
Перш ніж визначати `Resource`, вам потрібно створити `ptrace.Traces`, який відповідатиме за поширення трейсів у конвеєрі колектора, і ви можете скористатися допоміжною функцією `ptrace.NewTraces()` для його екземплярування. Вам також потрібно створити екземпляри типів `Atm` і `BackendSystem`, щоб мати дані для представлення джерел телеметрії, задіяних у вашій трасі.
...
Ці настанови відомі як [Семантичні домовдленості щодо Resources](/docs/specs/semconv/resource/) і задокументовані у специфікації OTel.
...
Зверніть увагу, що ми не додали атрибут 2atm.name" або "backendsystem.name" до `pcommon.Resource`, що представляє імена сутностей `Atm` та `BackendSystem`, тому що більшість (не кажучи вже про всі) розподілених систем бекенда трасування, сумісних зі специфікацією трасування OTel, інтерпретують `pcommon.Resource`, описаний у трейсі, як `Service`, тому вони очікують, що `pcommon.Resource` буде містити обовʼязковий атрибут з іменем `service.name`, як це передбачено семантичною угодою про ресурси.

Ми також використаємо необовʼязковий атрибут із назвою `ervice.version` для представлення інформації про версію як для сутностей `Atm`, так і для `BackendSystem`.
...
- Встановити той самий контекст трасування, що й у викликаючої операції, викликавши метод `SetTraceID()` і передавши `pcommon.TraceID` батьківського/викликаючого `ptrace.Span` як параметр.
