---
title: Зведення статусу специфікації
linkTitle: Статус
aliases: [otel/status]
weight: -10
---
...
Щоб дізнатися більше про сигнали та компоненти, дивіться специфікацію OTel [Огляд](/docs/specs/otel/overview/).
...
Для повних визначень життєвих циклів та довгострокової підтримки дивіться [Версіонування та стабільність](/docs/specs/otel/versioning-and-stability/).
...
- {{% spec_status "Протокол" "otlp" "/document-status/.*for.*trace" %}}
- Примітки:
  ...
  - Клієнти OpenTelemetry версіонуються до v1.0 після завершення їх реалізації трасування.
...
- {{% spec_status "Протокол" "otlp" "/document-status/.*for.*metric" %}}
- Примітки:
...
- {{% spec_status "Міст API" "otel/logs/api" "Status" %}}
- {{% spec_status "SDK" "otel/logs/sdk" "Status" %}}
- {{% spec_status "Події API" "otel/logs/event-api" "Status" %}}
- {{% spec_status "Протокол" "otlp" "/document-status/.*for.*log" %}}
- Примітки:
  ...
  - Міст API OpenTelemetry дозволяє писати апендери, які переносять логи з наявних фреймворків логування у OpenTelemetry. Міст API логів не призначений для прямого виклику кінцевими користувачами. Апендери логів розробляються багатьма мовами.
  ...
  - API подій OpenTelemetry дозволяє випускати записи логів, які відповідають [семантичним домовленості подій][семантичні конвенції подій]. На відміну від Мосту API логів, API подій призначений для виклику кінцевими користувачами. API подій активно розробляється.

### Профілі {#profiles}

- [Специфікація][profiles]
- {{% spec_status "Протокол" "otlp" "/document-status/.*for.*profiles" %}}
