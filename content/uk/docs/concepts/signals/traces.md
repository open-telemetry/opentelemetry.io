...
Відрізки можуть бути вкладеними, як це передбачено наявністю ідентифікатора батьківського відрізка: дочірні відрізки представляють підоперації. Це дозволяє відрізкам точніше відображати виконану роботу в застосунку.
...
Контекст відрізка є частиною відрізка, яка серіалізується та поширюєтьсяя разом з [Розподіленим Контекстом](#context-propagation) та [Багажем](/docs/concepts/signals/baggage).
...
Коли статус відрізка `Ok`, це означає, що відрізок був явно позначений розробником застосунку як безпомилковий. Хоча це не інтуїтивно, не обов'язково встановлювати статус відрізка як `Ok`, коли відомо, що відрізок завершився без помилки, оскільки це покривається `Unset`. Що робить `Ok`, так це представляє однозначний "остаточний виклик" щодо статусу відрізка, який був явно встановлений користувачем. Це корисно в будь-якій ситуації, коли розробник бажає, щоб не було іншої інтерпретації відрізка, окрім "успішного".
