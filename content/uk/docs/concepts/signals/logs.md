...
## Перевірка того, чи ввімкнено логування {#checking-whether-logging-is-enabled}

Поширене питання: чи має код застосунку перевіряти, чи ввімкнено логування, перед його викликом, наприклад:
...
У більшості випадків така перевірка непотрібна та не рекомендована. SDK OpenTelemetry розроблені для ефективної роботи — виклик API логування має мінімальні накладні витрати, коли логер не ввімкнено. Додатковий виклик `logger.Enabled` знижує продуктивність і ускладнює ваш код.

API `Enabled` корисне лише тоді, коли _обчислення аргументів_, переданих у виклик логування, саме по собі є дорогим, і ви хочете уникнути цих витрат, коли логер не ввімкнено. Наприклад, якщо тіло або атрибут потрібно отримати з бази даних або обчислити за допомогою дорогої операції:
...
Захищайте лише вирази, які не мають побічних ефектів, оскільки захищений код виконується лише тоді, коли логування ввімкнено. Захист коду, який має побічні ефекти або від якого залежить інша логіка, робить поведінку вашого застосунку залежною від конфігурації логування, що зазвичай є джерелом прихованих помилок.

Навіть тоді памʼятайте, що результат `Enabled` не є статичним: він може змінюватися з часом у міру зміни конфігурації, тому його слід обчислювати для кожного запису логу, а не кешувати.

Щодо нормативних вказівок API дивіться [специфікацію API логів](/docs/specs/otel/logs/api/#enabled).
...
| EventName            | Імʼя, що ідентифікує клас або тип події.              |
