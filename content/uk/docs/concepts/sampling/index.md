...
Важливо використовувати послідовну термінологію при обговоренні вибірки. Трейс або відрізок вважають "відібраним" або "не відібраним":

- **Відібрані**: Трейс або відрізок обробляються та експортуються. Оскільки вони обрані механізмом вибірки як представники популяції, вони вважаються "відібраними".
- **Не відібрані**: Трейс або відрізок не обробляються та не експортуються. Оскільки вони не обрані механізмом вибірки, вони вважаються "не відібраними".

Іноді визначення цих термінів плутають. Ви можете почути, що хтось говорить, що вони "відбирають дані" або що дані, які не обробляються або не експортуються, вважаються "відібраними". Це неправильні твердження.
...
- Ви можете описати деякі загальні правила, які визначають, чи повинні дані бути відібрані або відкинуті.
- Ви маєте спосіб розрізняти ваші служби, щоб служби з високим і низьким обсягом даних відібралися по-різному.
- Ви маєте можливість направляти невідібрані дані (для "про всяк випадок" сценаріїв) до систем з низькою вартістю зберігання.
...
- Ви обмежені обставинами, такими як регулювання, що забороняє відкидання даних (і не можете направляти невідібрані дані до систем з низькою вартістю зберігання).
...
Наприклад, найпоширеніша форма вибірки на початку — це [Консистентна ймовірнісна вибірка](/docs/specs/otel/trace/tracestate-probability-sampling/#consistent-sampling-decision). Це також може називатися детермінованою вибіркою. У цьому випадку рішення про вибірку приймається на основі ідентифікатора трейса та бажаного відсотка трейсів для вибірки. Це забезпечує, що цілі трейси відібрані, без пропущених відрізків, з постійною частотою, наприклад, 5% всіх трейсів.
...
Основний недолік вибірки на початку полягає в тому, що неможливо прийняти рішення про вибірку на основі даних у всьому трейсі. Наприклад, ви не можете гарантувати, що всі трейси з помилкою в них будуть відібрані лише за допомогою вибірки на початку. Для цієї ситуації та багатьох інших вам потрібна вибірка наприкінці.

## Вибірка наприкінці {#tail-sampling}

Вибірка наприкінці — це коли рішення про вибірку трейсу приймається, враховуючи всі або більшість відрізків у трейсі. Вибірка наприкінці дає вам можливість вибирати ваші трейси на основі конкретних критеріїв, отриманих з різних частин трейсів, що не є можливим з вибіркою на початку.

![Ілюстрація показує, як відрізки походять від кореневого відрізку. Після завершення відрізків процесор вибірки наприкінці приймає рішення про вибірку.](tail-sampling-process.svg)
...
1. Вибірка наприкінці може бути складною для реалізації. Залежно від типу доступних вам технік вибірки, це не завжди "налаштуй і забудь". Як змінюються ваші системи, так само змінюватимуться ваші стратегії вибірки. Для великої та складної розподіленої системи правила, що реалізують стратегії відбору, також можуть бути великими та складними.
...
- [Процесор ймовірнісної вибірки](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/processor/probabilisticsamplerprocessor)
...
Для окремих мовних реалізацій API та SDK OpenTelemetry ви знайдете підтримку вибірки на відповідних сторінках документації:

{{% sampling-support-list " " %}}

### Постачальники {#vendors}

Багато [постачальників](/ecosystem/vendors) пропонують комплексні рішення для вибірки, які включають вибірку на початку, вибірку наприкінці та інші функції, які можуть підтримувати складні потреби відбору. Ці рішення також можуть бути оптимізовані спеціально для бекенду постачальника. Якщо ви надсилаєте телеметрію постачальнику, розгляньте використання їх рішень для відбору.
