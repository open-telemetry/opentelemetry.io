...
Важливо використовувати послідовну термінологію при обговоренні вибірки. Трейс або відрізок вважають "семплованими" або "не семплованими":

- **Семпловані**: Трейс або відрізок обробляються та експортуються. Оскільки вони обрані семплером як представники популяції, вони вважаються "семплованими".
- **Не семпловані**: Трейс або відрізок не обробляються та не експортуються. Оскільки вони не обрані семплером, вони вважаються "не семплованими".

Іноді визначення цих термінів плутають. Ви можете почути, що хтось говорить, що вони "семплують дані" або що дані, які не обробляються або не експортуються, вважаються "семплованими". Це неправильні твердження.
...
- Ви маєте спосіб розрізняти ваші служби, щоб служби з високим і низьким обсягом даних семплувалися по-різному.
- Ви маєте можливість направляти несемпловані дані (для "про всяк випадок" сценаріїв) до систем з низькою вартістю зберігання.
...
Вибірка може бути не підходящою для вас. Ви можете уникати вибірки, якщо відповідаєте будь-якому з наступних критеріїв:
...
- Ви обмежені обставинами, такими як регулювання, що забороняє відкидання даних (і не можете направляти несемпловані дані до систем з низькою вартістю зберігання).
...
Наприклад, найпоширеніша форма вибірки на початку — це [Консистентний ймовірнісний семплінг](/docs/specs/otel/trace/tracestate-probability-sampling-experimental/#consistent-probability-sampling). Це також може називатися детермінованим семплінгом. У цьому випадку рішення про вибірку приймається на основі ідентифікатора трейса та бажаного відсотка трейсів для
вибірки. Це забезпечує, що цілі трейси семпловані, без пропущених відрізків, з постійною частотою, наприклад, 5% всіх тресів.
...
Основний недолік вибірки на початку полягає в тому, що неможливо прийняти рішення про вибірку на основі даних у всьому трейсі. Наприклад, ви не можете гарантувати, що всі трейси з помилкою в них будуть семпловані лише за допомогою вибірки на початку. Для цієї ситуації та багатьох інших вам потрібна вибірка наприкінці.

## Вибірка наприкінці {#tail-sampling}

Вибірка наприкінці — це коли рішення про вибірку трейсу приймається, враховуючи всі або більшість відрізкув у трейсі. Вибірка наприкінці дає вам можливість семплувати ваші трейси на основі конкретних критеріїв, отриманих з різних частин
трейсів, що не є можливим з вибіркою на початку.
...
Нарешті, для деяких систем вибірка наприкінці може використовуватися разом із вибіркою на початку. Наприклад, набір служб, які генерують надзвичайно великий обсяг даних трейсів, може спочатку використовувати вибірку на початку для відбору лише невеликого відсотка трейсів, а потім пізніше в ланцюжку телеметрії використовувати вибірку наприкінці для прийняття більш
складних рішень про відбір перед експортом до бекенду. Це часто робиться в інтересах захисту ланцюжка телеметрії від перевантаження.
