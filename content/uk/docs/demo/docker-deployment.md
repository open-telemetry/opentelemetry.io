...
cSpell:ignore: Firepit otelcollector span_metrics
---
...
```
    ...
    > [!NOTE]
    >
    > Голий `docker compose up` завантажує лише `compose.yaml`. Це запускає вебмагазин, але без Kafka чи будь-яких бекендів спостережуваності, тому телеметрію ніде переглядати. Передавайте файли явно, як показано вище, або використовуйте `make start`.
...
```
    ...
    ### Запуск з AI агентом {#run-with-the-ai-agent}

    Агент, MCP сервер та чатбот типовою є запускаються. Щоб додати їх[^1]:
...
```
    ...
    Це додає інтерфейс чатботу за адресою <http://localhost:8080/chatbot/>. Зазвичай агент відтворює записані відповіді LLM (`USE_VCR=True`), тому API-ключ не потрібен. Щоб спілкуватися з реальною LLM, встановіть `LLM_BASE_URL`, `LLM_MODEL` та `API_KEY` у `.env.override`.

    ### Запуск з безперервним профілюванням {#run-with-continuous-profiling}

    Щоб додати eBPF профайлер та інтерфейс профілювання Firepit[^1]:
...
```
    ...
    Профілі тоді доступні за адресою <http://localhost:8080/profiles/>.
...
5. (Опціонально) Запустіть фронтенд end-to-end тести[^1]:

    Cypress фронтенд тести запускаються для вже запущеного демо:
...
- Інтерфейс Load Generator: <http://localhost:8080/loadgen/>
...
OpenTelemetry Collector можна використовувати для експорту даних телеметрії до кількох бекендів. Колектор у демонстраційному застосунку нашаровує свою конфігурацію з кількох файлів, кожен з яких обʼєднується поверх попереднього. Які файли завантажуються, залежить від того, як ви запускаєте демо:

- `otelcol-config.yml` — базова конфігурація, завжди завантажується
- `otelcol-config-full.yml` — додає приймачі для сервісів, які працюють лише у повному демо, наприклад Kafka
- `otelcol-config-observability.yml` — підʼєднує вбудовані бекенди (Jaeger, Prometheus та OpenSearch)
- `otelcol-config-extras.yml` — ваші власні доповнення, завжди завантажується останньою

`make start` та `make start-minimal` завантажують усі чотири файли. Запуск демо без стеку спостережуваності завантажує менше з них, але `otelcol-config-extras.yml` завжди застосовується останнім, тому ваші зміни мають пріоритет у кожному режимі.
