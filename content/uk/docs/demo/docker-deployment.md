...
- Потім перевизначте `exporters` для телеметричних конвеєрів, які ви хочете використовувати для вашого бекенду.
...
{{% alert title="Примітка" color="info" %}} При обʼєднанні значень YAML з Collector, обʼєкти обʼєднуються, а масиви замінюються. Експортер `spanmetrics` повинен бути включений у масив експортерів для конвеєра `traces`, якщо він перевизначений. Не додавання цього експортера призведе до помилки. {{% /alert %}}
