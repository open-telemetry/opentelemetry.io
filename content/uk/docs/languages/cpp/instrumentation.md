...
### Ініціалізація експортера та процесора {#initialize-exporter-and-processor}

Ініціалізуйте експортер та процесор. У цьому випадку ви ініціалізуєте OStream Exporter, який за замовчуванням друкує записи журналу в stdout. Процесор відповідає за перетворення LogRecords у експортоване представлення перед тим, як вони досягнуть експортера.
...
### Реєстрація постачальника журналів {#register-a-logger-provider}

Створіть `LoggerProvider` за допомогою `LoggerProviderFactory` та зареєструйте його як глобального постачальника. Зверніть увагу, що фабрика повертає `unique_ptr`, тоді як `SetLoggerProvider` очікує `shared_ptr`. Використовуйте постачальника для отримання об'єктів `Logger`. Логери можуть мати імена для ідентифікації компонентів, що їх видають.

```cpp
// LoggerProviderFactory::Create повертає unique_ptr;
// обгорніть його в shared_ptr для SetLoggerProvider
...
### Випуск запису журналу {#emit-a-log-record}

Використовуйте отриманий логер для випуску структурованих записів журналу. Підтримувані рівні критичності включають: `kTrace`, `kDebug`, `kInfo`, `kWarn`, `kError`, `kFatal`.

За бажанням, можна збагачувати записи журналу контекстом трасування або **неявно**, додаючи відрізок до області та ведучи журнал у межах цієї області, або **явно**, передаючи ідентифікатор трасування, ідентифікатор відрізка та прапори.

#### З активною областю відрізка {#with-active-span-scope}

Додайте відрізок до області, щоб встановити поточний активний контекст виконання, і будь-які записи журналу, випущені в межах цієї області, автоматично отримують метадані контексту відрізка:

```cpp
{
  auto span = get_tracer()->StartSpan("HandleRequest");
  auto scope = opentelemetry::trace::Scope{span};
  // Цей запис журналу отримує trace_id, span_id та trace_flags тощо
  // з активної області
  logger->Info("Handling request");
}
```

#### З явним контекстом трасування {#with-explicit-trace-context}

Ви також можете передати контекст трасування явно:
