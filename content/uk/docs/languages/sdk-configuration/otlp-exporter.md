...
default_lang_commit: 2983de042738a2fae9d81beb78be88405dae050b
...
Для OTLP/gRPC значення endpoint є gRPC цілями. Не додавайте шляхи сигналів OTLP/HTTP, як-от `/v1/traces`, `/v1/metrics` або `/v1/logs`, до значень gRPC endpoint; gRPC експортери використовують метод служби protobuf OTLP для кожного сигналу після підключення до налаштованої цілі.
...
Заголовки представлені у форматі, що відповідає [W3C Baggage](https://www.w3.org/TR/baggage/#header-content), наприклад `key1=value1,key2=value2`. Метадані, розділені крапкою з комою, [metadata](https://www.w3.org/TR/baggage/#property), не підтримуються. Для отримання додаткових відомостей див. [Вказівка заголовків через змінні середовища](/docs/specs/otel/protocol/exporter/#specifying-headers-via-environment-variables) у специфікації протоколу.
