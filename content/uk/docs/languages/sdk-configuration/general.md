...
Вказує Вибірник, який використовується SDK для вибірки трейсів.
...
Вказує аргументи, якщо це застосовується, до вибірника, визначеного в `OTEL_TRACES_SAMPLER`. Вказане значення буде використано лише якщо `OTEL_TRACES_SAMPLER` встановлено. Кожен тип вибірника визначає свій власний очікуваний вхід, якщо такий є. Неправильний або невпізнаний вхід реєструється як помилка.
...
- Для вибірників `traceidratio` та `parentbased_traceidratio`: Ймовірність вибірки, число в діапазоні [0..1], наприклад "0.25". Стандартно 1.0, якщо не встановлено.
- Для `jaeger_remote` та `parentbased_jaeger_remote`: Значення є розділений комою список:
  ...
  - `pollingIntervalMs`: у мілісекундах, що вказує, як часто вибірник буде опитувати бекенд для оновлень стратегії вибірки.
