...
Потім включіть код конфігурації, який виконується під час ініціалізації вашої програми. Переконайтеся, що `service.name` встановлено шляхом налаштування імені сервісу (наприклад, через змінну середовища `OTEL_SERVICE_NAME`).

Щоб інструментувати багато поширених бібліотек без увімкнення кожного інструментування окремо, використовуйте `opentelemetry-instrumentation-all`:

```sh
gem install opentelemetry-instrumentation-all
```

Потім налаштуйте SDK для увімкнення доступних інструментувань:
...
> [!TIP]
>
> У застосунку Ruby on Rails ви можете розмістити цю конфігурацію у файлі `config/initializers/opentelemetry.rb`.

Цей підхід автоматично увімкне підтримувані інструментування бібліотек (тому вам зазвичай не потрібно виконувати кроки для кожного інструментування окремо у розділі [Трейси](#traces) нижче). Вам все ще може знадобитися налаштувати експортер та змінні середовища, відповідні для вашого розгортання.
...
Якщо виняток виходить за межі блоку `in_span`, трейсер стандартно записує виняток у відрізок, встановлює статус відрізка в `Error` і повторно сповіщає про виняток. Ви можете вимкнути автоматичний запис винятків, передавши `record_exception: false` як аргумент методу `in_span`.

Якщо ви перехоплюєте виняток всередині блоку і не сповіщаєте про нього повторно, встановіть статус відрізка та запишіть виняток вручну, коли це доречно:

```ruby
MyAppTracer.in_span("do_work") do |span|
  begin
    # виконайте роботу, яка може викликати виняток
  rescue StandardError => e
    span.status = OpenTelemetry::Trace::Status.error(e.message)
    span.record_exception(e)

    # Обробіть виняток без повторного сповіщення, наприклад, поверніть резервне значення.
  end
end
```
...
>
> &#9888; Рішення про вибірку приймаються в момент створення відрізка. Якщо ваш вибірник враховує атрибути відрізка під час прийняття рішення про відбір відрізка, то ви _повинні_ передати ці атрибути як частину створення відрізка. Будь-які атрибути, додані після створення, не будуть враховані вибірником, оскільки рішення про відбір вже прийнято.
...
Щоб збирати журнали додатків, що генеруються за допомогою стандартного модуля Ruby `Logger`, встановіть пакет інструментування для журналів:

```sh
gem install opentelemetry-instrumentation-logger
```

Після встановлення переконайтеся, що ви налаштували експортер/точку доступу, що підтримує журнали, наприклад, за допомогою конфігурації експортера, описаної в наступному розділі.
