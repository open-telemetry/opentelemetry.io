...
{{% alert title="Примітка" %}} Ця функція залежить від платформи, оскільки вона покладається на `System.Runtime.InteropServices.Marshal.GetExceptionPointers`. {{% /alert %}}
...
{{% alert title="Обережно" %}} Використовуйте `AppDomain.UnhandledException` з обережністю. Викидання винятку в цьому обробнику ставить процес у стан, з якого неможливо відновитися. {{% /alert %}}
