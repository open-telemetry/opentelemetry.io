...
Дотримуйтесь інструкцій з [Початку роботи][getting started], щоб мати робочий .NET застосунок, який експортує дані до консолі.
...
Наступний приклад базується на [початковому прикладі][getting started] і додає два власні ресурси, `environment.name` та `team.name` у коді:
