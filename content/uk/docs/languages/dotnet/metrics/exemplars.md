...
// Звільнити провайдери до завершення роботи програми.
// Це призведе до очищення залишків телеметричних даних та закриття конвеєрів.
...
Екземпляри не потребують окремого API для інструментування. Продовжуйте використовувати звичайні API інструментів `System.Diagnostics.Metrics`; коли фільтрація екземплярів увімкнена і підтримка бекенду існує, SDK та шлях експорту автоматично прикріплюють екземпляри. Для звичайного використання API дивіться [Metrics API](/docs/languages/dotnet/metrics-api/).

## Екземпляри та фільтрація атрибутів на основі View {#exemplars-and-view-based-attribute-filtering}

> [!IMPORTANT]
>
> Обмеження `TagKeys` (або інше видалення атрибутів через `View`) видаляє їх з потоку **агрегованих метрик**, але не видаляє їх з будь-яких екземплярів, записаних для того ж інструмента. Видалені атрибути зберігаються в екземплярах як _фільтровані теги_.
>
> Якщо метою видалення атрибута є **редагування даних** (наприклад, видалення атрибута, що містить конфіденційні дані), одного лише `View` недостатньо, коли екземпляри увімкнені. Або залиште значення за замовчуванням `ExemplarFilterType.AlwaysOff`, щоб повністю вимкнути екземпляри, або налаштуйте власний `ExemplarFilter` / `ExemplarReservoir`, щоб контролювати, які вимірювання вибірково зберігати. Див. [opentelemetry-specification#5073](https://github.com/open-telemetry/opentelemetry-specification/pull/5073) для відповідного уточнення специфікації.
