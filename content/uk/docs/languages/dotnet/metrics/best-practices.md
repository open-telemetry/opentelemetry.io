...
> [!NOTE]
>
> Для синхронних інструментів з кумулятивною тимчасовістю, кожен набір атрибутів, який коли-небудь був зафіксований, продовжує експортуватися на кожному циклі збору, навіть якщо нові вимірювання не були зафіксовані (як показано у інтервалі `(T0, T2]` вище). Для **спостережуваних (асинхронних) інструментів** (наприклад, `ObservableGauge`, `ObservableCounter`) поведінка відрізняється: експортуються лише набори атрибутів, повідомлені зворотним викликом під час поточного циклу збору. Якщо зворотний виклик перестає повідомляти певний набір атрибутів, він буде пропущений у наступних експортів. Це означає, що користувачі несуть відповідальність за управління станом у своєму зворотному виклику - щоб припинити експорт серії, просто припиніть її повідомлення.
...
> [!WARNING] **Уникайте використання `TraceId` та `SpanId` як атрибутів у метриках.**
>
> Включення контексту трасування (`TraceId`, `SpanId`) як атрибутів метрик може здатися інтуїтивним способом кореляції метрик з трасуванням, але цей підхід неефективний і може зробити метрики практично непридатними. Включення контексту трасування як атрибутів призводить до вибуху кардинальності, оскільки кожен унікальний контекст трасування стає новою комбінацією атрибутів. Це може швидко призвести до досягнення [обмеження кардинальності](#cardinality-limits), в результаті чого вимірювання будуть складені в кошик з переповненням і втрачається інформація про кореляцію, яку ви намагалися досягти.

<!-- markdownlint-disable-next-line MD028 -->

> [!TIP] **Використовуйте екземпляри для кореляції метрик з трасуванням.**
>
> [Екземпляри](/docs/specs/otel/metrics/sdk/#exemplar) надають механізм для кореляції метрик з [трасуванням](/docs/languages/dotnet/traces/) шляхом вибіркового збирання конкретних вимірювань та прикріплення до них контексту трасування. Цей підхід зберігає кардинальність метрик, одночасно дозволяючи кореляцію трасування для підмножини вимірювань. Перегляньте навчальний посібник з [Екземплярів](/docs/languages/dotnet/metrics/exemplars/), щоб дізнатися більше.
