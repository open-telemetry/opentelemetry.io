...
Щоб увімкнути це, встановіть `OTEL_CONFIG_FILE=/path/to/config.yaml`, де `config.yaml` — це файл, який відповідає схемі декларативної конфігурації, наприклад:
...
Наявність `OTEL_CONFIG_FILE` призведе до того, що автозавантажувач спробує налаштуватись за допомогою наданого конфігураційного файлу, а інші змінні оточення буде проігноровано.
...
| `OTEL_PHP_AUTOLOAD_ENABLED`            | `false`             | `true`, `false`                                                                       | `true`                       | Увімкнення/вимкнення автозавантаження SDK                                                                                           |
| `OTEL_PHP_LOG_DESTINATION`             | `default`           | `error_log`, `trigger_error`, `stderr`, `stdout`, `psr3`, `none`                      | `stderr`                     | Куди будуть надсилатися внутрішні помилки та попередження. `error_log` та `trigger_error` відповідають функціям PHP з тією ж назвою |
| `OTEL_PHP_INTERNAL_METRICS_ENABLED`    | `false`             | `true`, `false`                                                                       | `true`                       | Чи повинен SDK надсилати метрики про свій внутрішній стан (наприклад, batch processors)                                             |
| `OTEL_PHP_DISABLED_INSTRUMENTATIONS`   | `[]`                | Імена інструментів або `all`                                                          | `psr15,psr18`                | Вимкнення одного або кількох встановлених авто-інструментів                                                                         |
| `OTEL_PHP_EXCLUDED_URLS`               | ``                  | Розділені комами регулярні вирази                                                     | `client/.*/info,healthcheck` | Не завантажувати SDK, якщо URL запиту відповідає одному з наданих регулярних виразів                                                |
| `OTEL_PHP_DEBUG_SCOPES_DISABLED`       | `false`             | `true`, `false`                                                                       | `true`                       | Увімкнення або вимкнення налагодження відключення областей.                                                                         |
| `OTEL_PHP_EXPERIMENTAL_AUTO_ROOT_SPAN` | `false`             | `true`, `false`                                                                       | `true`                       | Почати кореневий відрізок під час автозавантаження composer                                                                         |
