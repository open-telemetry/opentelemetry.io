...
### Dslhsprb в окремих процесах {#spans-in-separate-processes}
...
Щоб передати відрізки між процесами, нам потрібно запустити відрізок, який не повʼязаний з конкретним процесом. Це можна зробити за допомогою макросу `start_span`. На відміну від `with_span`, макрос `start_span` не встановлює новий відрізок як поточно активний відрізок у контексті словника процесу.

Звʼязування відрізка як пращура до нащадка в новому процесі можна зробити шляхом прикріплення контексту та встановлення нового відрізка як поточно активного в процесі. Весь контекст повинен бути прикріплений, щоб не втратити інші дані телеметрії, такі як [baggage](/docs/specs/otel/baggage/api/).
...
[Відрізок](/docs/concepts/signals/traces/#spans) може бути створений з нульовою або більше кількістю Span Links, які причинно повʼязують його з іншим Span. [Link](/docs/concepts/signals/traces/#span-links) потребує контексту Span для створення.
