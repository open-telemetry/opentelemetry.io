...
JMX (Java Management Extensions) — це технологія Java, яка надає інструменти для керування та моніторингу застосунків через JMX MBeans (Managed Beans). Ці MBeans надають атрибути та операції керування, які можна спостерігати ззовні.

Модуль OpenTelemetry JMX Metric Insight дає змогу інтегрувати метрики JMX в OpenTelemetry, а саме:
...
- Використовувати попередньо визначені зіставлення метрик для популярних цільових систем (Tomcat, Jetty, Wildfly, ...)
...
Збирання метрик JMX вмикається шляхом налаштування одного (або обох) з таких параметрів конфігурації:
...
| `otel.jmx.config`        | `OTEL_JMX_CONFIG`        | Шлях до користувацького YAML для зіставлення метрик                             | немає      |
...
- [Попередньо визначені цільові системи Java агента](https://github.com/open-telemetry/opentelemetry-java-instrumentation/blob/main/instrumentation/jmx-metrics/README.md#predefined-metrics)
- [Попередньо визначені цільові системи JMX Scraper](https://github.com/open-telemetry/opentelemetry-java-contrib/tree/main/jmx-scraper#configuration-reference)

Ви можете вказати кілька цільових систем, розділивши їх комами.

### Віддалені JMX-зʼєднання {#remote-jmx-connections}

Щоб збирати метрики з віддаленого JVM, потрібно використовувати JMX Scraper. Це потребує двох окремих JVM:

1. **Цільовий JVM** — Застосунок, за яким ведеться спостереження
