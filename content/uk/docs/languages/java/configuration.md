...
Властивості для телеметрії самодіагностики SDK:

| Системна властивість                      | Опис                                                                                              | Стандартно |
| ----------------------------------------- | ------------------------------------------------------------------------------------------------- | ---------- |
| `otel.experimental.sdk.telemetry.version` | Виберіть схему телеметрії самодіагностики. Допустимі значення: `legacy` та `latest`. Див. **[1]** | `legacy`   |

**[1]**: Вибирає схему для всієї телеметрії самодіагностики SDK, а не лише для експортерів. Для пакетних процесорів відрізків та лог-записів вибирає назви метрик; для провайдерів трейсерів та логерів і періодичного читача метрик визначає, чи записувати метрики самодіагностики взагалі. Див. [Метрики самодіагностики SDK](../sdk/#sdk-self-monitoring-metrics) для деталей конфігурації та назв метрик, які видає кожен компонент.
...
#### Властивості: трейсинг {#properties-traces}
