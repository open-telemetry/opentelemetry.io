...
Властивості для [вибірників](../sdk/#sampler):

| Системна властивість      | Опис                                                                                                                                                                                                                  | Стандартно              |
| ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| `otel.traces.sampler`     | Вибірник, який потрібно використовувати. Відомі значення включають `always_on`, `always_off`, `traceidratio`, `parentbased_always_on`, `parentbased_always_off`, `parentbased_traceidratio`, `jaeger_remote`. **[1]** | `parentbased_always_on` |
| `otel.traces.sampler.arg` | Аргумент для налаштованого вибірника, якщо підтримується, наприклад, коефіцієнт.                                                                                                                                      |                         |

**[1]**: Відомі вибірники та артефакти (див. [sampler](../sdk/#sampler) для координат артефакту):
...
| `otel.metrics.exporter`          | Список експортерів метрик через кому. Відомі значення включають `otlp`, `prometheus`, `console`, `none`. **[1]**                                                                                          | `otlp`          |
...
Реалізуйте інтерфейс [ConfigurableSamplerProvider](https://www.javadoc.io/doc/io.opentelemetry/opentelemetry-sdk-extension-autoconfigure-spi/latest/io/opentelemetry/sdk/autoconfigure/spi/traces/ConfigurableSamplerProvider.html) для дозволу користувацького механізму вибірки брати участь у автоконфігурації. Наприклад:
...
- [Документація з використання](https://github.com/open-telemetry/opentelemetry-java/tree/main/sdk-extensions/declarative-config)
