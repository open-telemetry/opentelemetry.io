...
- **Аналіз:** Це дієслово (`INSERT`), що діє на об'єкт (`my_database.users`). Конкретні значення, які вставляються, мають високу кардинальність і справедливо виключаються з назви.
...
- **Аналіз:** Хоча формат відрізняється, принцип залишається тим же. Він описує метод (`GetUser`), який є дієсловом, в межах сервісу (`com.example.UserService`), який є об'єктом або ресурсом.
