...
default_lang_commit: c4e5b35b265c240fde82c5f4aa9035424db42860
...
**Використовується для**: Пакунків, які реалізують API OpenTelemetry для мови: інтерфейси та no-op реалізації, від яких залежать інструментований код та бібліотеки, незалежно від будь-якого SDK.

**Приклади**: Ruby-gems `opentelemetry-api`, `opentelemetry-metrics-api` та `opentelemetry-logs-api`.

### `connector`

**Використовується для**: Компонентів конекторів OpenTelemetry Collector, які зʼєднують два конвеєри, діючи як експортер одного та приймач іншого, можливо, перетворюючи типи сигналів.

**Приклади**: Лічильні конектори, конектори span-to-metrics або конектори відмовостійкості (failover).
...
**Використовується для**: Компонентів SDK, які налаштовують генерацію ідентифікаторів трейсів та відрізків.

**Приклади**: Генератори ідентифікаторів, сумісні з AWS X-Ray.
...
**Використовується для**: Компонентів SDK, які переносять метрики зі сторонніх джерел у читач метрик SDK.
...
**Використовується для**: Поширювачів контексту, які переносять контекст трейсу та baggage через межі процесів у певному форматі передачі.

**Приклади**: Поширювачі B3, Jaeger або AWS X-Ray.
...
**Використовується для**: Вибірників SDK, які визначають, які відрізки записуються та експортуються.

**Приклади**: Віддалені вибірники AWS X-Ray або вибірники на основі правил.

### `sdk`

**Використовується для**: Пакунків, які реалізують SDK OpenTelemetry для мови.

**Приклади**: Ruby-геми `opentelemetry-sdk`, `opentelemetry-metrics-sdk` та `opentelemetry-logs-sdk`.

### `semantic-convention`

**Використовується для**: Пакунків, які надають константи семантичних домовленостей для мови.

**Приклади**: Ruby-gem `opentelemetry-semantic_conventions`.
