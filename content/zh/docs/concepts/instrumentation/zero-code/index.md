...
自动插桩通常以代理或类似代理的安装方式，将 OpenTelemetry API 和 SDK 的能力添加到你的应用中。
所涉及的具体机制可能因语言而异，包括字节码操作、猴子补丁或 eBPF，用于将对
OpenTelemetry API 和 SDK 的调用注入你的应用中。

通常，自动插桩会为你所使用的库添加插桩。这意味着对请求和响应、数据库调用、消息队列调用等操作进行了插桩。
而你的应用代码本身通常不会被插桩。若要对你的代码进行插桩，
你需要使用[基于代码的插桩](/docs/concepts/instrumentation/code-based)。

此外，自动插桩允许你配置加载的[插桩库](/docs/concepts/instrumentation/libraries)和[导出器](/docs/concepts/components/#exporters)。

你可以通过环境变量和其他特定语言的机制（如系统属性或传递给初始化方法的参数）来配置自动插桩。
