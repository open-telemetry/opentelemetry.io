...
如果你打算构建和调试自定义的 Collector 接收器、处理器、插件或导出器，你需要有自己的 Collector 实例。
...
帮助开发者组装自己的 Collector 发行版，便于构建包含自定义组件和公开组件的 Collector。

在此过程中，`ocb` 将生成 Collector 的源代码，你可以利用这些源代码来构建和调试你自己的自定义组件。现在我们开始吧。
...
| version:     | 自定义 OpenTelemetry Collector 的版本                    | 是   | `1.0.0`                                       |
...
要查看可以添加到自定义 Collector 中的组件列表，请参见
...
这一步使用 `ocb` 可执行文件构建你的自定义 Collector 发行版。如果你希望将其部署到容器编排系统（例如 Kubernetes），
...
你现在可以使用生成的代码来启动你的组件开发项目，并轻松构建和分发包含自定义组件的 Collector 发行版。
