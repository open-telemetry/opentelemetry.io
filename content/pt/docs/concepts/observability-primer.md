...
tratamento de novos problemas, ou seja, "desconhecidos desconhecido". Também
...
instrumentada quando os desenvolvedores não precisam adicionar mais
...
**Confiabilidade** responde à perguntas: "O serviço está fazendo o que os
...
**Métricas** são agregações ao longo de um período de tempo, de dados numéricos
sobre sua infraestrutura ou aplicação. Exemplos incluem: taxa de erro do
...
**SLO**, ou _Service Level Objective_, representa a forma como a confiabilidade
...
para sistemas distribuídos, que comumente têm problemas não determinísticos ou
são muito complicados para reproduzir localmente.

Para entender o rastreamento distribuído, você precisa compreender a função de
cada um de seus componentes: logs, trecho e rastros.
...
apresentam informações contextuais, como e de onde foram chamados.
...
para fornecer informações sobre a operação que é rastreada.
...
OpenTelemetry, confira [Trecho](/docs/concepts/signals/traces/#spans).
...
ocorre durante uma solicitação (ou quais etapas compõem uma solicitação).

Sem o rastreamento, encontrar a causa raiz dos problemas de desempenho em um
sistema distribuído pode ser desafiador. O rastreamento torna a depuração e o
entendimento de sistemas distribuídos menos assustador, analisando o que
