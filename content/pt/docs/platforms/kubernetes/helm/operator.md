...
operator que gerencia [OpenTelemetry Collectors](/docs/collector) e
auto-instrumentação de workloads. Uma das formas de instalar o OpenTelemetry
...
Para instalar o chart com o nome de release `my-opentelemetry-operator`, execute
...
No Kubernetes, para que o servidor de API se comunique com o componente webhook,
o webhook requer um certificado TLS que o servidor de API esteja configurado
para confiar. Existem algumas formas diferentes de gerar/configurar o
certificado TLS necessário.

- O método mais fácil e padrão é instalar o
  ...
  para mais detalhes.
- Você pode fornecer seu próprio Issuer configurando o valor
  `admissionWebhooks.certManager.issuerRef`. Você precisará especificar o `kind`
  (Issuer ou ClusterIssuer) e o `name`. Note que esse método também requer a
  instalação do cert-manager.
- Você pode usar um certificado autoassinado gerado automaticamente definindo
  `admissionWebhooks.certManager.enabled` como `false` e
  `admissionWebhooks.autoGenerateCert.enabled` como `true`. O Helm criará um
  cert autoassinado e um secret para você.
- Você pode usar seu próprio certificado autoassinado definindo tanto
  `admissionWebhooks.certManager.enabled` quanto
  `admissionWebhooks.autoGenerateCert.enabled` como `false`. Você deve fornecer
  os valores necessários para `admissionWebhooks.cert_file`,
  `admissionWebhooks.key_file` e `admissionWebhooks.ca_file`.
- Você pode carregar webhooks e certificados personalizados desabilitando
  `.Values.admissionWebhooks.create` e `admissionWebhooks.certManager.enabled`
  enquanto define o nome do secret do cert personalizado em
  `admissionWebhooks.secretName`.
- Você pode desabilitar webhooks completamente desabilitando
