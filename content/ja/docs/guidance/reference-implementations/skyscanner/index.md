...
[Tristan Sloughter](https://github.com/tsloughter)（コミュニティ）、
[Neil Fordyce](https://github.com/neilfordyce)（Skyscanner）著 | 2026年4月21日

このリファレンス実装は、スコットランドのエジンバラに拠点を置くグローバルな旅行検索プラットフォームである [Skyscanner](https://www.skyscanner.net/) が、OpenTelemetry を大規模に運用している方法について説明します。

全世界に 1,400 人の従業員を抱え、24 の本番 Kubernetes クラスターにわたって 1,000 以上のマイクロサービスを運用している Skyscanner の OpenTelemetry 導入事例は、大規模に運用する組織にとって貴重な教訓を提供します。

## 組織構成 {#organizational-structure}

6 人のプラットフォームエンジニアで構成される Hubble チームが、Skyscanner の Collector の大部分を管理しています。
より広いプラットフォームエンジニアリング組織の一部として、主に Java ベースのマイクロサービスアーキテクチャで動作する Skyscanner のコンピュートプラットフォームを担当しています。
...
Python と Node.js サービスの場合、プラットフォームチームが環境とリソース属性に基づいて適切なデフォルトを設定するラッパーライブラリを提供します。
これらのアプローチにより、ボイラープレートのセットアップが最小限に抑えられ、深い OpenTelemetry の知識を必要とせずに、サービスチームはすぐに使えるオブザーバビリティを手に入れることができます。

## OpenTelemetry の導入 {#opentelemetry-adoption}

Skyscanner の OpenTelemetry の取り組みは 2021 年に始まりました。
同社は社内で構築したオープンソーススタックから商用ベンダーへの移行を進めていました。
しかし、ベンダーロックインを避けたいと考えていました。

> 「ベンダーに依存しない形でベンダーに移行したかったのです。」と、Skyscanner の Hubble プラットフォームチームのソフトウェアエンジニアである [Neil Fordyce](https://github.com/neilfordyce) は説明します。
...
サービスがグローバルのどこで実行されていても、どのクラスターにいても、テレメトリーをこの単一のアドレスに送信します。
Istio が最も近い利用可能な Collector へのリクエストルーティングを処理します。

デプロイメントは 2 つの異なる Collector パターンで構成されています。

**ゲートウェイ Collector（レプリカセット）**：大部分の OTLP トラフィック（トレースとメトリクス）をほとんどのサービスから処理し、大半の処理が行われます。
...
Skyscanner が 2021 年に Collector を初めてデプロイした際、設定は最小限でした。メモリリミッター、バッチプロセッサー、そしてトレース用の OTLP エクスポーターです。

時間の経過とともに、設定は自然に進化しました。メトリクスパイプラインの追加、Istio スパン取り込みの統合、スパンからメトリクスへの変換の実装、そしてノイズの削減とコスト管理のためのフィルタープロセッサーの追加などです。
...
そこでの解決策は、Istio にスパンを出力するよう設定し（当初は Zipkin フォーマットでしたが、現在 Istio は OTLP をサポートしています）、Zipkin レシーバーで Collector に取り込み、セマンティック規約に合うように変換し、[スパンメトリクスコネクター](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/e8a502371ea1d2c3534235d623c1b1eb3b6b4b58/connector/spanmetricsconnector?from_branch=main)を使用して、アプリケーション計装なしで一貫したメトリクスを生成することでした。

> 「アプリケーションオーナーがコードを計装する必要なく、プラットフォームレベルでそれが実現できるのです。」と Neil は述べています。

スパンメトリクスコネクターの設定は、スパンから主要なディメンションを抽出します。
...
次に Collector はこれらのメトリクスを `http.client.duration` や `http.server.duration` などのセマンティック規約名に変換し、クラスター、サービス名、HTTP ステータスコードごとに集計します。
これにより、コード変更なしですべてのサービスに対してプラットフォームレベルの HTTP メトリクスが提供され、セマンティック規約に準拠した一貫した命名と、ネイティブ Istio メトリクスよりも低いカーディナリティが実現されます。

### 404 エラーの課題 {#the-404-error-challenge}

Collector 設定に関する注目すべき課題の 1 つは、キャッシュサービスがキャッシュにエントリが存在しないことを示すために HTTP 404 を返すケースでした。
...
このプロセッサーは、特定のキャッシュサービスからの 404 ステータスコードを持つスパンを照合し、そのエラーステータスを解除することで、エラーベースのサンプリングがトリガーされるのを防ぎます。

> 「最初からそのフィルタープロセッサーがあれば、より高品質で使いやすいトレースが得られていたでしょう。」と Neil は振り返ります。

しかし、Neil は OpenTelemetry SDK の[宣言的設定](/docs/languages/sdk-configuration/declarative-configuration/)が最近導入されたことで、このようなフィルタリングは中央の Collector 設定を変更するのではなく、サービスチーム自身が分散的に設定できるようになったと述べています。
...
- スパンメトリクスコネクターを使用して Istio スパンからメトリクスを生成する
- 広範な変換プロセッサーを使用して Istio 属性をセマンティック規約にマッピングする
- キャッシュサービスの 404 フィルタリングロジックを実装する
- メトリクスとトレースを OTLP 経由でオブザーバビリティベンダーにエクスポートする

下図は、OTLP メトリクスとトレース、および Istio スパンがこれらのゲートウェイ Collector にどのように到達するかを示しています。

![Skyscanner アーキテクチャ（ゲートウェイ Collector）図](skyscanner-architecture-gateway.png)

#### エージェント Collector {#agent-collector}

[エージェント Collector][agent-otelbin] は、各ノードからインフラストラクチャおよびプラットフォームレベルのメトリクスの収集に焦点を当てています。

- さまざまなソース（node exporter、kube-state-metrics、kubelet）から Prometheus エンドポイントをスクレイピングする
- 最小限の処理（メモリ制限、バッチ処理、属性のクリーンアップ）を行う
...
ベース Docker イメージに事前設定された Java エージェントは、HTTP と gRPC のスパン生成をすぐに使える形で提供します。

### 強い意見を持った自動計装 {#opinionated-auto-instrumentation}

チームは自動計装に対して、意図的に強い意見を持ったアプローチを取っています。
デフォルトですべてを有効にするのではなく、逆の方向から始めます。共有ベース Docker イメージですべての計装を無効にし、厳選されたセットのみを明示的に有効にします。

> 「逆のアプローチなのです。すべてを無効にしてから、必要なものだけを有効にします。」と Neil は説明します。

ベースイメージの環境変数を使用して、Skyscanner はランタイム、HTTP、gRPC 関連の計装の厳選されたセットをデフォルトで有効にしています。
これには JAX-RS、gRPC、Jetty、一般的な HTTP クライアント、executor 計装、およびロギングコンテキスト伝搬が含まれます。
サービスチームはこれらのデフォルトを自動的に継承しますが、必要に応じて独自のサービス定義でオーバーライドしたり、追加の計装を有効にしたりすることもできます。

このモデルにより、エッジでの柔軟性を維持しながら、数百のサービス間で一貫性が確保されます。
...
# サービス起動時に環境変数から検出できるものを設定
# これらの変数はデプロイメントシステムによって自動的に設定される
# 一部の環境変数は重複を避けるために省略されている
setup_otel_agent() {
...
# my-service が他のデフォルトではない計装を有効にしたい場合、簡単に拡張可能
...
HTTP と gRPC の計装は有効になっていますが、チームは SDK が生成する HTTP および RPC メトリクスのほとんどを意図的に破棄しています。
これは、先に説明したように、Istio サービスメッシュスパンから一貫した低カーディナリティのプラットフォームメトリクスをすでに導出しているためです。

計装全体を無効にする（スパンも削除されてしまう）のではなく、OpenTelemetry SDK ビューを使用してメトリクス集計を破棄しながらトレースを維持しています。

- HTTP と RPC のメトリクスはグローバルに破棄される
- スパンは通常通り出力され続ける
- サービスチームは、Istio が提供する以上の追加粒度が必要な場合、特定のメトリクス（たとえば、サーバーサイドのレイテンシー）を選択的に再有効化できる

チームが SDK メトリクスにオプトインする場合、既存の Istio 由来のメトリクスとの衝突や二重カウントを避けるため、多くの場合メトリクスの名前を変更します。
...
典型的なユースケースは `http.route` ごとにリクエストを分解することです。

```yaml
# この破棄の挙動は、リストを拡張してさらにビューを追加し、
# 保持するメトリクスを明示的に選択することで変更できる
# 例：http.server.request.duration メトリクスは保持するが、
# http.client.* メトリクスは引き続き破棄する
- selector:
    instrument_name: http.server.request.duration
  view:
    # Istio メトリクスに http.server.request.duration という名前が既にあるため、
    # 衝突と二重カウントを避けるためにリネーム
...
このアプローチにより、Skyscanner は高価値の分散トレースを維持し、メトリクスの重複を避け、カーディナリティを制御し、取り込みコストを削減できます。これらすべてが、サービスオーナーが OpenTelemetry の内部を深く理解する必要なく実現されています。

全体として、この戦略は強いプラットフォームマインドセットを反映しています。大規模に機能する適切なデフォルトを提供し、ノイズを最小限に抑え、「正しいこと」を簡単にできるようにしつつ、高度なニーズを持つチームがさらに先に進む余地を残しています。
...
チームは Contrib が本番利用に推奨されていないことを知り、必要なコンポーネントのみを含むカスタム Collector イメージのビルドを検討する予定です。

Skyscanner は約 6 ヶ月ごとに Collector をアップデートしていますが、特定の機能や重要な修正を追跡している場合はより頻繁にアップグレードします。
リリース情報を把握するために、RSS フィードや CNCF Slack チャンネルをフォローしています。

ロールアウト戦略は、クラスターティア間での段階的なプロモーションを使用しています。Dev クラスター、次に 3 つの Alpha 本番クラスター、続いて 8 つの Beta 本番クラスター、最後に残りの 13 の本番クラスターです。
Argo CD を使用してデプロイメントし、ティア間のプルリクエストを通じて変更をプロモーションします。

> 「開発テストクラスターで確かに問題を起こしたことがありますが、それ以上プロモーションする前に修正しました。」と Neil は言います。

この段階的なアプローチにより、設定の問題が本番に到達する前に発見されています。
OpenTelemetry Collector デプロイメントの自動テストとロールバック機能はまだありませんが、これらの改善は今後予定されています。

## うまくいっていること {#what-works-well}

本番で OpenTelemetry を採用してから、チームの経験は非常にポジティブなものとなっています。

> 「本当にほとんど問題がありませんでした。」と Neil は振り返ります。

柔軟性が Collector の最大の強みとして際立っています。

> 「やろうとしたことはすべて実現できました。それがどれほど柔軟かを物語っていると思います。」と Neil は説明します。

その他のハイライトとしては、シンプルな設定によるベンダー非依存を実現する OTLP プロトコル、明確で整理されたリリースノート、そしてチームメンバーが Collector コンポーネントのメモリリークを発見して修正を貢献した際のコミュニティの対応力などがあります。

## 教訓と課題 {#lessons-and-pain-points}

Skyscanner は一部のパイプラインでまだ古い不安定な HTTP セマンティック規約を使用しています。
アップグレードには、Istio 属性をセマンティック規約名にマッピングする複数の変換プロセッサールールの更新が必要であり、ドキュメントを手動で相互参照して設定文字列を入力する作業が伴います。

チームはセマンティック規約管理のための [Weaver](https://github.com/open-telemetry/weaver) を知っていますが、まだワークフローに統合していません。

6 ヶ月ごとのアップグレードは、複数の破壊的変更に一度に遭遇することを意味します。
リリースノートはよく書かれており、変更が明確に文書化されていますが、6 ヶ月分のアップデートを一度にレビューすることは、リリースに追従し続ける場合と比較して摩擦が増えます。

## 他の方へのアドバイス {#advice-for-others}

本番での経験に基づき、Skyscanner チームは以下のアドバイスを提供しています。

- **シンプルに始める**：メモリリミッター、バッチプロセッサー、基本的なエクスポーターだけから始めましょう。複雑さはニーズが生じたときにのみ追加しましょう。
- **メモリリミッターは初日から**：スケール時のメモリ問題を防ぐため、すぐにセットアップしましょう。
- **フィルタープロセッサーの早期検討**：アプリケーションのステータスコードのセマンティクスを理解し、高頻度の「偽陽性」をフィルタリングしてコストを管理しましょう。
- **回復性を過剰に設計しない**：テレメトリーデータの場合、シンプルなインメモリバッチ処理で十分なことが多いです。
- **段階的なロールアウトで問題を発見**：環境ティア間での段階的なプロモーションは、価値ある検証を提供します。

## まとめ {#takeaways}

Skyscanner の事例は、適度な規模のプラットフォームエンジニアリングチームが、比較的低い運用オーバーヘッドで、大規模な OpenTelemetry Collector インフラストラクチャを成功裏に管理できることを実証しています。
