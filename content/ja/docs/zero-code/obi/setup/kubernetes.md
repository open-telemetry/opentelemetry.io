...
たとえばDebianベースのディストリビューションでは、`kernel.perf_event_paranoid=3` [も使用](https://lwn.net/Articles/696216/)しており、これにより `CAP_SYS_ADMIN` なしでの `perf_event_open()` へのアクセスは拒否されます。
