...
Collector を使用するシナリオでは、Collector もローカルで実行され、ホストの CPU リソースを共有しています。
...
| OpenTelemetry PHP Distro + ローカル Collector                        |                             24.37 |                  7.01 |
| バニラ OpenTelemetry PHP + protobuf（C エクステンション）+ Collector |                             25.76 |                  8.40 |
| バニラ OpenTelemetry PHP pure-PHP protobuf エクスポート + Collector  |                             49.02 |                 31.66 |
...
- ローカルの Collector 配置は、PHP ワーカーと CPU を取り合う場合にオーバーヘッドを増加させる可能性があります。
...
- レイテンシーに敏感なアプリケーションワーカーと、負荷の高い Collector ワークロードの同居は避けてください。
