...
[メトリクス](#metric)で特に使われる用語。[属性](#attribute)を参照してください。
...
重大度やトレース情報など、他のフィールドも`メタデータ`とみなされるかもしれません。
...
生測定値または事前定義された集計値のいずれかのデータポイントを、[メタデータ](#metadata)付きの時系列として記録します。
...
テレメトリーを生成するエンティティに関する情報を[属性](#attribute)として取得します。
