...
一般的に**トレース**として知られている**分散トレース**は、マイクロサービスやサーバーレスアプリケーションのようなマルチサービスアーキテクチャを伝搬する単一のリクエスト（アプリケーションまたはエンドユーザーによって行われる）が辿った経路を記録します。
...
たとえば、ユーザーがウェブページを読み込むとき、最初の HTTP リクエストは API ゲートウェイ、バックエンドサービス、データベースを順に通過するかもしれません。これらの各ステップはスパンとして表され、それら全体で 1 つのトレースを形成し、リクエストのエンドツーエンドの流れを示します。
