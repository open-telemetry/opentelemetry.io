...
weight: 30
default_lang_commit: 5336b383467f539be5f01645182a1c345797127e # patched
...
ブログ記事は、不安定な GitHub `blob`/`tree` リンクを防止するために markdownlint (`gh-url-hash`) によってチェックされます。
...
ブログ投稿は履歴として扱われ、サイトのビルドを確実にするための重要な変更を除き、1 年ほど経過すると更新されません。
古いブログ投稿には、コンテンツが古くなっている可能性があり、一部のリンクが無効である可能性があることを読者に警告するバナーが上部に表示されます。

古い投稿は [lint もリンクチェックもされない][pr-checks]ことにも注意してください。
