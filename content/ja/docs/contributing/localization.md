...
linkTitle: ローカリゼーション
weight: 25
default_lang_commit: f7cf24c34806b8f98683f5cb9d82b35c2a26029d
...
最初の PR がマージされた後、メンテナーはイシューラベルと組織レベルのチームを設定します。
...
新しいロケールを [`data/locale-teams.yaml`][] に追加し、`npm run fix:codeowners` を実行して CODEOWNERS のロケールセクションを再生成してください。
これらの変更をロケールの最初の PR に含めてください。
CI はレジストリが `content/` 配下のロケールディレクトリと一致することを要求します。
新しいロケールは通常[未配置](#locale-teams)の状態で開始されるため、CODEOWNERS の行にはロケールが[卒業](#locale-teams)するまで `docs-approvers` のフォールバックが設定されます。
...
### ロケールチーム、コードオーナー、および人員配置 {#locale-teams}

各ロケールには `docs-LANG_ID-approvers` と `docs-LANG_ID-maintainers` の GitHub チームがあり、メンテナーは承認者も兼ねます。
承認者チームはロケールのパスの[コードオーナー][code owner]であるため、ロケール承認者のレビューはロケール限定の PR に必要なコードオーナーレビューを満たします。
ロケールメンテナーはこのような PR を[自動マージ](#auto-merge)することもできます。

ロケールチームの想定メンバーシップは [`data/locale-teams.yaml`][] レジストリに記録されており、このレジストリはリポジトリの [CODEOWNERS][] ファイルのロケールセクションも生成します。
メンバーシップの変更はレジストリに対する PR として提案され、監査証跡が残り、組織管理者によって実際のチームに適用されます。
ジェネレーターとそれがエンコードする規約については、[locale-codeowners README][] を参照してください。

メンテナーがいないロケールは**未配置**です。
その CODEOWNERS の行にはフォールバックオーナーとして `@open-telemetry/docs-approvers` が記載されるため、ドキュメント承認者がそのロケールの PR をレビューしてブロックを解除できます。
ドキュメントメンテナーがこのような PR を手動でマージします。
[自動マージ](#auto-merge)にはロケールメンテナーが必要だからです。

未配置のロケールがメンテナーを獲得した場合、メンテナーを記録し CODEOWNERS のロケールセクションを再生成するレジストリ PR を提出してください（`npm run fix:codeowners`）。
これによりロケールの行から `docs-approvers` のフォールバックが削除され、以降はロケールチームが自分たちの PR を管理します。
このロケールは**卒業**したことになります。
例として [zh の卒業 PR][zh graduation PR] を参照してください。
ロケールチームが後に活動停止した場合、逆のレジストリ変更によりフォールバックが復元されます。
