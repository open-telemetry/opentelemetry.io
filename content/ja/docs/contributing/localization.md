...
- 画像そのもののテキストをローカライズしない限り、画像、動画、その他のコンテンツ以外のアセットファイルのコピーを **作成しない** でください。
  - Hugo は、サイトのローカライゼーション間で共有される画像ファイルのレンダリングについてスマートです。
    つまり、Hugo は _単一の_ 画像ファイルを出力し、それをロケール間で共有します。
    詳細については [Page bundles][] を参照してください。
  - これは [`LOCALIZATION` ガイドライン][l10n-check]チェックによって強制されます。
