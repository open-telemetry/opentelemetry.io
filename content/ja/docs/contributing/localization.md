...
[ビルドとチェックの修正](#keep-checks-green)では、ローカリゼーションページを英語の対応ページと同期せずに編集する必要がある場合があります。
たとえば、英語のページが移動された後にリンクの変更先を変更する場合です。
...
`npm run fix:i18n:status` を実行して、乖離したローカリゼーション対象ページのフロントマターフィールド `drifted_from_default` を設定します。
このフィールドはページの上部に「outdated」バナーを表示し、リンクチェッカーがそのページをスキップするようにします。
これにより、乖離したページの古いリンクが CI を失敗させることがなくなります。
リンクチェッカーが乖離したページをどのように処理するかの詳細は、[リンクチェック](/site/build/link-checking/)を参照してください。
...
> メンテナーは、サイト全体のツール、設定、フロントマター、マークアップの更新を含む、ロケールをまたがざるを得ないコンテンツに影響しない変更を提出することがあります。
> これには自動化された[乖離状況](#track-changes)の記録管理 PR も含まれます。
> このような変更はローカリゼーションページの意味を変更しません。

#### ビルドとチェックをグリーンに保つ {#keep-checks-green}

ローカリゼーションページの**コンテンツ**を変更する PR が複数のロケールにまたがることが許されるのは、サイトのビルドとチェックをグリーンに保つために厳密に必要な場合のみです。

- **リンクの修正**：英語のページが移動または削除された後、あるいは外部リソースが移動した後に、ローカリゼーションページのリンクチェックの失敗を修復します。
  [リンクの修正とリソースの更新](#link-fixes-and-resource-updates)を参照してください。
- **ビルドの修正**：共有のショートコード、インクルードファイル、データソースの変更後などに、ローカリゼーションページのサイトビルドの破損を修復します。
  ページの[乖離状況](#track-changes)はリンクチェックからのみ保護し、Hugo のビルドからは保護しません。

いずれの場合も、修正したすべてのローカリゼーションページを[パッチ済み](#patched)としてマークしてください。
...
移動または削除されたページのローカリゼーションされたコピー自体は変更する必要はありません。
[乖離の追跡](#track-changes)がそれらのロケールチームに対してフラグを立てます。
修正が必要になるのは、そのようなターゲットに**リンクしている**ローカリゼーションページです。
リンクターゲットの状況に応じて対処してください。

- **ページが移動された場合**：
  1. 移動された英語ページが古いパスの [alias][aliases] を宣言していることを確認してください。
     alias は以前公開されたリンクを機能させ続けますが、サイト訪問者に対してのみです。
     alias はサーバーサイドリダイレクトとして公開され、リンクチェッカーはビルドされたサイトの正規ページパスに対してリンクを解決します。
     そのため、古いパスへのリンクは修正が必要です。
  2. リンクチェックに失敗する各非英語ページのリンクを新しいパスに更新し、編集した各ページを[パッチ済み](#patched)としてマークしてください。
     （[乖離した](#track-changes)ページはリンクチェッカーによってスキップされるため、これは通常、同期済みのページに適用されます。）

- **セクションが移動された場合**：alias はページパスをリダイレクトするため、フラグメントには対応できません。
  セクションがページ内で移動した場合は、セクションへのリンクが機能し続けるよう[見出し ID](#headings) を保持してください。
  それ以外の場合は、リンクチェックに失敗する各ページのセクションへのリンクを更新し、編集した各ローカリゼーションページを[パッチ済み](#patched)としてマークしてください。

- **ページまたはセクションが削除された場合**：代替を選択するかリファレンスを削除することは、影響を受ける各ロケールにとって[意味的な変更](#semantic-changes)であるため、そのようなリンクにパッチを適用しないでください。
  削除されたターゲットにリンクしていた英語ページを修正すると、そのローカリゼーションされたコピーが乖離します。
  英語の編集がコミットされたら、`npm run fix:i18n:status -- <PATHS-TO-LOCALIZED-COPIES>` を実行して、それらのコピーの[乖離状況](#drift-status)を更新してください。
  リンクチェッカーはそれらのページをスキップし、調整は各ページのロケールチームに委ねられます。
  まれに、ローカリゼーションページにのみ存在する失敗リンクがある場合は、そのロケールチームと連携して修正してください。

- **外部リソースが移動された**が、それ以外は意味的に変更されていない場合（GitHub ファイルの移動など）：リンクチェックに失敗する各ページのリンクを更新し、編集した各ローカリゼーションページを[パッチ済み](#patched)としてマークしてください。

すべてのケースで、`npm run check:links` を再実行して、リンクの失敗が残っていないことを確認してください。
