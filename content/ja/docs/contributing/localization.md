...
- **翻訳**
  ...
  - （ `inline code example` などの）コードブロックやインラインコードを含むコード
...
生成 AI ツール (ChatGPT、Gemini など) を使用して翻訳を支援する場合は、OpenTelemetry の[生成 AI コントリビューションポリシー][genai-policy]および Linux Foundation の[生成 AI ポリシー][lf-ai-policy]に従う必要があります。
特に、以下の点に注意してください。

- [プルリクエストテンプレート][pull request template]の該当するチェックボックスにチェックを入れ、生成 AI ツールの使用を**開示する**こと
- AI が生成したすべての翻訳の正確性を**レビューし、検証する**こと。
  提出する内容について、責任を負うこと
- 自身でレビューおよび検証ができない AI 生成の翻訳は**提出しない**こと。
  これにより、レビューの深刻なボトルネックが生じ、メンテナーのリソースを守るため、PR がクローズされる可能性があること
...
> [!IMPORTANT] 小さなPRを提出する <a id="small-prs"></a>
>
...
- `config/_default/module-template.yaml` の `module.mounts`。
  少なくとも、`content` に単一の `source`-`target` エントリを追加します。
  ロケールに十分なコンテンツがある場合にのみ、`en` フォールバックページのエントリを追加することを検討してください。
...
- **辞書が利用できる場合**：
  ...
  - [`.cspell.yml`][] の `import:` に、そのパッケージの `cspell-ext.json` を追加し、`dictionaries:` にその辞書の ID (たとえば `bn`, `es-es`, `pl_pl`) を追加します。
- その言語の**辞書が利用できない場合**、`import` を追加しないでください。
  [`.cspell.yml`][] の `ignorePaths` に `content/LANG_ID` を追加し、cSpell がそのロケールの Markdown を英語としてスペルチェックしないようにします。
