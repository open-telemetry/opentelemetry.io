...
各サービスは、分散アプリケーションにおけるテレメトリーがどのように問題解決に役立つかを示すために使用できる障害を有効化/無効化できるように、フィーチャーフラグサービスと通信できる必要があります。
...
すべてのサービスは Kubernetes 上で実行します。OpenTelemetry コレクターは OpenTelemetry Operator を用いてデプロイされる必要があり、サイドカー + ゲートウェイモードで動作する必要があります。各 Pod からのテレメトリーはエージェントからゲートウェイへルーティングされる必要があり、ゲートウェイはデフォルトでオープンソースのトレース + メトリクス可視化ツールへテレメトリーをエクスポートする必要があります。
