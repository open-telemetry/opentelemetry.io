...
唯一の例外は、特定の言語名を持ち、その言語にのみ関連するパラメーターです（たとえば`instrumentation/development.java.spring_batch`パラメーター）。
...
さらに、宣言的ファイルは必要に応じてパラメーターを拡張および追加する柔軟性を提供し、オブザーバビリティ設定を高度にカスタマイズおよび細かく制御できます。
