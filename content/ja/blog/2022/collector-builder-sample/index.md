...
余分な依存関係が重量とセキュリティの負担を増やしますが、カスタムビルドではそれらの余分なコンポーネントは存在すらしないため、容量を占有しません。
...
その後、他のテレメトリーシグナルを任意のバックエンドに処理しエクスポートする柔軟性が生まれます。
...
これにより Collector デプロイメントの新しいロールアウトがトリガーされ、更新されたイメージにコンパイルされた設定変更と新しいコンポーネントが反映されます。
