---
title: Skyscanner が OpenTelemetry をスケールさせる方法：24 の本番クラスターにまたがるコレクター管理
...
default_lang_commit: a50a77347a3525e3821626dfd6a11604503226ba
# prettier-ignore
...
この記事では、スコットランドのエディンバラに拠点を置くグローバルな旅行検索プラットフォームである [Skyscanner](https://www.skyscanner.net/) を紹介します。

世界中に 1,400 人の従業員を擁し、24 の本番 Kubernetes クラスターで 1,000 以上のマイクロサービスを運用している Skyscanner の OpenTelemetry への取り組みは、大規模に運用する組織にとって貴重な教訓を提供します。

## 組織構成 {#organizational-structure}

6 人のプラットフォームエンジニアで構成される Hubble チームが、Skyscanner のコレクターの大部分を管理しています。
より大きなプラットフォームエンジニアリング組織の一部として、Skyscanner の主に Java ベースのマイクロサービスアーキテクチャを実行するコンピュートプラットフォームを担当しています。

サービスチーム自体は、デプロイメントやテレメトリー収集インフラストラクチャから抽象化されています。
Java サービスの場合、チームは事前設定された OpenTelemetry Java エージェントを含むベース Docker イメージを継承します。
Python や Node.js のサービスについては、プラットフォームチームが環境やリソース属性に基づいた適切なデフォルトを設定するラッパーライブラリを提供しています。
これらのアプローチにより、ボイラープレートのセットアップを最小限に抑え、サービスチームが OpenTelemetry の深い知識を必要とせずにすぐにオブザーバビリティを利用できるようになっています。

## OpenTelemetry の採用 {#opentelemetry-adoption}

Skyscanner の OpenTelemetry への取り組みは 2021 年に始まりました。
同社は社内で構築したオープンソースのスタックから商用ベンダーへの移行を進めていました。
...
Skyscanner のコレクターアーキテクチャは、Istio ベースのインテリジェントルーティングを備えた中央 DNS エンドポイントを特徴としています。
サービスがグローバルのどこで実行されているか、どのクラスターにいるかに関係なく、この単一のアドレスにテレメトリーを送信します。
Istio がリクエストを最も近い利用可能なコレクターにルーティングします。

デプロイメントは 2 つの異なるコレクターパターンで構成されています。

**Gateway Collector（Replica Set）**：ほとんどのサービスからの大量の OTLP トラフィック（トレースとメトリクス）を処理し、処理の大部分がここで行われます。

**Agent Collector（DaemonSet）**：まだネイティブに OTLP をサポートしていないオープンソースおよびプラットフォームサービスから Prometheus エンドポイントをスクレイピングします。

![Skyscanner のアーキテクチャ図](skyscanner-architecture.png)

## 設定：シンプルに始めて、徐々に進化させる {#configuration-start-simple-evolve-gradually}

Skyscanner が 2021 年にコレクターを初めてデプロイしたとき、設定は最小限でした。メモリリミッター、バッチプロセッサー、そしてトレース用の OTLP エクスポーターだけでした。

時間の経過とともに、設定は自然に進化していきました。メトリクスパイプラインの追加、Istio スパンの取り込みの統合、スパンからメトリクスへの変換の実装、そしてノイズの削減とコスト管理のためのフィルタープロセッサーの追加などです。

### Istio サービスメッシュスパンからプラットフォームメトリクスへの変換 {#turning-istio-service-mesh-spans-into-platform-metrics}

Skyscanner のコレクターの最も革新的な使い方の 1 つは、Istio サービスメッシュスパンからメトリクスを生成することです。

Istio のネイティブメトリクスには、Prometheus デプロイメントを圧迫するカーディナリティ爆発の問題がありました。
さらに、Skyscanner はコードを所有していないが一貫したメトリクスが必要な多くの既製サービスを運用しています。

彼らの解決策：Istio がスパンを発行するように設定し（元々は Zipkin 形式でしたが、現在 Istio は OTLP をサポートしています）、Zipkin レシーバーを介してコレクターに取り込み、セマンティック規約に合うように変換し、[span metrics connector](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/e8a502371ea1d2c3534235d623c1b1eb3b6b4b58/connector/spanmetricsconnector?from_branch=main) を使用して、アプリケーション計装なしで一貫したメトリクスを生成するというものです。

> 「アプリケーションオーナーがコードを計装する必要なく、プラットフォームレベルでそれを実現できます」と Neil は述べました。
...
コレクターはこれらのメトリクスを `http.client.duration` や `http.server.duration` のようなセマンティック規約の名前を使用するように変換し、クラスター、サービス名、HTTP ステータスコードごとに集約します。
これにより、コード変更なしですべてのサービスに対してプラットフォームレベルの HTTP メトリクスが提供され、セマンティック規約に準拠した一貫した命名と、ネイティブの Istio メトリクスよりも低いカーディナリティが実現されます。

### 404 エラーの課題 {#the-404-error-challenge}

コレクター設定に関する注目すべき課題の 1 つは、キャッシュサービスがキャッシュにエントリが存在しないことを示すために HTTP 404 を返すケースでした。
コレクターはこれらの 404 をエラーとして扱い、実際には正常な高ボリュームの動作に対して 100% のトレースサンプリングをトリガーしていました。

解決策は、これらの特定の 404 レスポンスに対してエラーステータスをリセットするフィルタープロセッサーを追加することでした。
...
このプロセッサーは、特定のキャッシュサービスからの 404 ステータスコードを持つスパンにマッチし、エラーステータスをリセットすることで、エラーベースのサンプリングがトリガーされるのを防ぎます。

> 「最初からそのフィルタープロセッサーがあれば、もっと高品質で使いやすいトレースが得られたでしょう」と Neil は振り返りました。

ただし、Neil は OpenTelemetry SDK の[宣言的設定](/docs/languages/sdk-configuration/declarative-configuration/)が最近導入されたことで、このようなフィルタリングは、中央のコレクター設定を変更する必要なく、サービスチーム自身が分散的に設定できるようになったと指摘しています。

### 設定の詳細解説 {#configuration-deep-dive}

Skyscanner は、これらのパターンを実際に理解するために、本番のコレクター設定を共有しています。
...
- キャッシュサービス向けの 404 フィルタリングロジックを実装
- OTLP を介してオブザーバビリティベンダーにメトリクスとトレースをエクスポート

この図は、OTLP メトリクスとトレース、および Istio スパンがこれらの Gateway collector にどのように到達するかを示しています。

![Skyscanner のアーキテクチャ（Gateway Collector）図](skyscanner-architecture-gateway.png)

#### Agent collector {#agent-collector}

[Agent collector][agent-otelbin] は、各ノードからのインフラストラクチャおよびプラットフォームレベルのメトリクス収集に焦点を当てています。

- さまざまなソース（node exporter、kube-state-metrics、kubelet）から Prometheus エンドポイントをスクレイピング
- 最小限の処理（メモリ制限、バッチ処理、属性のクリーンアップ）を実行
- OTLP を介してオブザーバビリティベンダーにメトリクスをエクスポート

## 計装戦略 {#instrumentation-strategy}

Skyscanner の Java 中心の環境は、OpenTelemetry の自動計装機能から大きな恩恵を受けています。
ベース Docker イメージに事前設定された Java エージェントにより、HTTP と gRPC のスパン生成がすぐに利用できます。

### 意見を持った自動計装 {#opinionated-auto-instrumentation}

チームは自動計装に対して意図的に意見を持ったアプローチを取っています。
デフォルトですべてを有効にするのではなく、逆方向から始めます。共有ベース Docker イメージですべての計装を無効にし、厳選されたセットのみを明示的に有効にします。

> 「逆のアプローチです。すべてを無効にしてから、必要なものだけを有効にしています」と Neil は説明しました。

ベースイメージの環境変数を使用して、Skyscanner はランタイム、HTTP、gRPC に関連する計装の焦点を絞ったセットをデフォルトで有効にしています。
これには JAX-RS、gRPC、Jetty、一般的な HTTP クライアント、executor の計装、ロギングコンテキスト伝搬が含まれます。
サービスチームはこれらのデフォルトを自動的に継承しますが、必要に応じて独自のサービス定義でオーバーライドしたり、追加の計装を有効にしたりすることもできます。

このモデルにより、何百ものサービスにわたる一貫性を確保しつつ、周辺での柔軟性も維持しています。
...
# すべての Java マイクロサービスが拡張するための共通ベースイメージを定義
...
ランチャースクリプト `run.sh` は、デプロイメントが提供する環境変数から `-javaagent` フラグと `otel.resource.attributes` を構築します。

```bash run.sh
# これを使用して OTel リソース属性を設定します
# サービス起動時に環境変数から検出できるものを対象としています
# これらの変数はデプロイメントシステムによって自動的に設定されます
# 繰り返しを避けるため、一部の環境変数は省略されています
setup_otel_agent() {
...
最後に、個別のサービスの Dockerfile は同じベースを拡張し、そのサービスが必要とする追加の計装のみを追加します。
...
# my-service が他のデフォルト以外の計装を有効にしたい場合も簡単に拡張できます
...
### スパンは有効、メトリクスは無効（デフォルトで） {#spans-yes-metrics-no-by-default}

Skyscanner の戦略で特に興味深い点は、メトリクスとトレースの扱い方です。
HTTP と gRPC の計装は有効になっていますが、チームは意図的に SDK が生成する HTTP および RPC メトリクスの大部分をドロップしています。
これは、前述のように Istio サービスメッシュスパンから一貫した低カーディナリティのプラットフォームメトリクスをすでに導出しているためです。

計装を完全に無効にすると（スパンも削除されてしまうため）、かわりに OpenTelemetry SDK のビューを使用してメトリクスの集約をドロップし、トレースを維持しています。

- HTTP と RPC のメトリクスはグローバルにドロップ
- スパンは通常通り発行を継続
- サービスチームは、Istio が提供する以上の粒度が必要な場合、特定のメトリクス（たとえばサーバー側のレイテンシ）を選択的に再有効化できる

チームが SDK メトリクスをオプトバックする場合、既存の Istio 由来のメトリクスとの衝突や二重カウントを避けるために、名前を変更することが多いです。

前述の [Java ベースイメージ](#setting-up-the-java-agent) では、`OTEL_EXPERIMENTAL_METRICS_VIEW_CONFIG` が Skyscanner のデフォルトの `otel-view.yaml` を指しており、[view file configuration](https://github.com/open-telemetry/opentelemetry-java/blob/65f7412a986cb474314b093c1bbba77955b52031/sdk-extensions/incubator/README.md#view-file-configuration) を使用しています。

```yaml
# Skyscanner のデフォルトメトリクスビュー設定
# OTEL_EXPERIMENTAL_METRICS_VIEW_CONFIG が指すファイルに保存
# Istio からのメトリクスがすでにあるため、http と rpc のメトリクスをドロップ
...
同じファイルは、サービスが特定のメトリクスを保持する必要がある場合に拡張できます。
典型的なユースケースは、`http.route` でリクエストを分類することです。

```yaml
# このドロップ動作は、リストを拡張してより多くのビューを追加し、
# 保持するメトリクスを明示的に選択することで変更できます。
# たとえば、http.server.request.duration メトリクスを保持しつつ、
# http.client.* メトリクスのドロップは継続する場合
- selector:
    instrument_name: http.server.request.duration
  view:
    # Istio メトリクスにすでに http.server.request.duration という名前があるため、
...
このアプローチにより、Skyscanner は高価値の分散トレースを維持し、メトリクスの重複を回避し、カーディナリティを制御し、取り込みコストを削減できます。これらすべてが、サービスオーナーが OpenTelemetry の内部を深く理解する必要なく実現されています。

全体として、この戦略は強力なプラットフォームマインドセットを反映しています。大規模に機能する適切なデフォルトを提供し、ノイズを最小限に抑え、「正しいこと」を「簡単なこと」にする一方で、高度なニーズを持つチームがさらに先に進む余地を残しています。

## デプロイメントとリリース管理 {#deployment-and-release-management}

Skyscanner は OpenTelemetry Collector Contrib ディストリビューションを使用しており、必要なものがすべて含まれていたため採用しました。
チームはインタビュー中に Contrib が本番使用には推奨されていないことを知り、必要なコンポーネントのみを含むカスタムコレクターイメージのビルドを検討する予定です。

Skyscanner はコレクターを約 6 か月ごとに更新していますが、特定の機能や重要な修正を追跡している場合は、より頻繁にアップグレードします。
RSS フィードや CNCF Slack チャンネルをフォローして、リリース情報を入手しています。

彼らのロールアウト戦略は、クラスターのティア全体にわたる段階的なプロモーションを使用しています。Dev クラスター、次に 3 つの Alpha 本番クラスター、その後 8 つの Beta 本番クラスター、最後に残りの 13 の本番クラスターです。
Argo CD を使用してデプロイし、ティア間のプロモーションはプルリクエストを介して行われます。

> 「開発テストクラスターで確かに問題を起こしたことがありますが、さらにプロモーションする前に修正しました」と Neil は語りました。

この段階的なアプローチにより、設定の問題が本番に到達する前に検出されています。
OpenTelemetry Collector デプロイメントの自動テストとロールバック機能はまだありませんが、これらの改善は今後予定されています。

## うまくいっていること {#what-works-well}

本番で OpenTelemetry を採用して以来、チームの体験は非常にポジティブなものでした。

> 「正直なところ、かなり苦労のないものでした」と Neil は振り返りました。

柔軟性がコレクターの最大の強みとして際立っています。

> 「私たちがやろうとしたことはすべて、実際に実現できました。それがどれだけ柔軟であるかを物語っていると思います」と Neil は説明しました。

その他のハイライトには、OTLP プロトコルがシンプルな設定を通じてベンダー非依存を提供していること、明確でよく整理されたリリースノート、そしてチームメンバーがコレクターコンポーネントのメモリリークを発見して修正に貢献した際のコミュニティの応答性が挙げられます。

## 教訓と課題 {#lessons-and-pain-points}

Skyscanner は一部のパイプラインでまだ古い不安定な HTTP セマンティック規約を使用しています。
アップグレードするには、Istio 属性をセマンティック規約の名前にマッピングする複数の transform プロセッサールールを更新する必要があり、ドキュメントを手動で相互参照して設定文字列を埋める必要があります。

チームはセマンティック規約管理のための [Weaver](https://github.com/open-telemetry/weaver) を認識していますが、まだワークフローに統合していません。

6 か月ごとのアップグレードでは、複数の破壊的変更を一度に処理することになります。
リリースノートはよく書かれており、変更が明確に記載されていますが、6 か月分の更新を一度にレビューすることは、リリースに追従するよりも手間がかかります。

## 他の方へのアドバイス {#advice-for-others}

本番での経験に基づき、Skyscanner チームは次のアドバイスを提供しています。

- **シンプルに始める**：メモリリミッター、バッチプロセッサー、基本的なエクスポーターだけから始めましょう。
  必要性が生じたときにのみ複雑さを追加してください。
- **メモリリミッターは初日から**：スケールする際のメモリ問題を防ぐために、すぐに設定しましょう。
- **フィルタープロセッサーの早期検討**：アプリケーションのステータスコードのセマンティクスを理解し、高ボリュームの「偽陽性」をフィルタリングしてコストを管理しましょう。
- **復元力を過剰設計しない**：テレメトリーデータには、シンプルなインメモリバッチ処理で十分なことが多いです。
- **段階的なロールアウトで問題を検出**：環境ティア全体への段階的なプロモーションは、貴重な検証を提供します。

## 今後の展望 {#whats-next}

このストーリーは、適度な規模のプラットフォームエンジニアリングチームが、比較的低い運用オーバーヘッドで大規模な OpenTelemetry Collector インフラストラクチャを成功裏に管理できることを示しています。

このシリーズの今後のストーリーもご期待ください。さまざまな規模の組織が、それぞれ独自の課題と創造的なソリューションを持ちながら、本番で OpenTelemetry Collector をどのように活用しているかを引き続き紹介していきます。

あなた自身の OpenTelemetry Collector のストーリーを共有しませんか？
CNCF の [#otel-devex](https://cloud-native.slack.com/archives/C01S42U83B2) Slack チャンネルにぜひ参加してください。
本番での OpenTelemetry の活用方法や、開発者体験をさらに改善する方法について、ぜひお聞かせください。
