---
title:
  'Skyscanner が OpenTelemetry をスケールさせる方法: 24 の本番クラスターにまたがる
  Collector の管理'
linkTitle: Skyscanner が OpenTelemetry をスケールさせる方法
date: 2026-04-21
author: >-
...
Developer Experience SIG は、さまざまな業界や規模の企業による実際の OpenTelemetry デプロイ事例を紹介するブログシリーズを公開しています。
この記事では、スコットランドのエディンバラに拠点を置くグローバルな旅行検索プラットフォーム [Skyscanner](https://www.skyscanner.net/) を取り上げます。

世界中に 1,400 人の従業員を擁し、24 の本番 Kubernetes クラスターで 1,000 以上のマイクロサービスを運用する Skyscanner の OpenTelemetry 導入の歩みは、大規模に運用する組織にとって貴重な教訓を提供しています。

## 組織構造 {#organizational-structure}

Hubble チームは 6 人のプラットフォームエンジニアで構成され、Skyscanner の Collector の大部分を管理しています。
より広いプラットフォームエンジニアリング組織の一部として、Skyscanner の主に Java ベースのマイクロサービスアーキテクチャを実行するコンピューティングプラットフォームを担当しています。

サービスチーム自体は、デプロイメントとテレメトリー収集インフラストラクチャから抽象化されています。
Java サービスの場合、チームは事前設定済みの OpenTelemetry Java エージェントを含むベース Docker イメージを継承します。
Python および Node.js サービスの場合、プラットフォームチームは環境やリソース属性に基づいて適切なデフォルト値を設定するラッパーライブラリを提供しています。
これらのアプローチにより、定型的なセットアップが最小化され、サービスチームは OpenTelemetry の深い知識を必要とせずに、すぐにオブザーバビリティを利用できるようになります。

## OpenTelemetry の採用 {#opentelemetry-adoption}

Skyscanner の OpenTelemetry 導入は 2021 年に始まりました。
同社は社内で構築したオープンソーススタックから商用ベンダーへの移行を進めていましたが、ベンダーロックインは避けたいと考えていました。

> 「ベンダー非依存な形でベンダーに移行したかったのです」と、Skyscanner の Hubble プラットフォームチームのソフトウェアエンジニアである [Neil Fordyce](https://github.com/neilfordyce) は説明しました。

このベンダー非依存のアプローチにより、テレメトリーインフラストラクチャの中核として OpenTelemetry Collector を採用することになりました。
...
サービスがグローバルのどこで実行されていても、どのクラスターにあっても、この単一のアドレスにテレメトリーを送信します。
Istio が最も近い利用可能な Collector にリクエストをルーティングします。

デプロイメントは 2 つの異なる Collector パターンで構成されています。

**ゲートウェイ Collector（Replica Set）**: 大部分の OTLP トラフィック（トレースとメトリクス）をほとんどのサービスから処理し、処理の大部分がここで行われます。

**エージェント Collector（DaemonSet）**: OTLP をネイティブにサポートしていないオープンソースおよびプラットフォームサービスから Prometheus エンドポイントをスクレイピングします。

![Skyscanner アーキテクチャ図](skyscanner-architecture.png)

## 設定: シンプルに始めて、段階的に進化させる {#configuration-start-simple-evolve-gradually}

Skyscanner が 2021 年に最初に Collector をデプロイしたとき、設定は最小限でした。メモリリミッター、バッチプロセッサー、そしてトレース用の OTLP エクスポーターだけです。

時間の経過とともに、設定は自然に進化しました。メトリクスパイプラインの追加、Istio スパンの取り込みの統合、スパンからメトリクスへの変換の実装、そしてノイズを減らしコストを制御するためのフィルタープロセッサーの追加が行われました。

### Istio サービスメッシュのスパンをプラットフォームメトリクスに変換する {#turning-istio-service-mesh-spans-into-platform-metrics}

Skyscanner の Collector の最も革新的な使い方の一つは、Istio サービスメッシュのスパンからメトリクスを生成することです。

Istio のネイティブメトリクスはカーディナリティ爆発の問題を抱えており、Prometheus デプロイメントを圧倒してしまう可能性がありました。
さらに、Skyscanner はコードを所有していないが一貫したメトリクスが必要な、多くの既製サービスを運用しています。

彼らの解決策は次の通りです。Istio にスパンを出力するよう設定し（元々は Zipkin 形式でしたが、現在 Istio は OTLP もサポートしています）、Zipkin レシーバーを使って Collector にスパンを取り込み、セマンティック規約に準拠するよう変換し、[span metrics connector](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/e8a502371ea1d2c3534235d623c1b1eb3b6b4b58/connector/spanmetricsconnector?from_branch=main) を使って、アプリケーション計装なしで一貫したメトリクスを生成します。

> 「アプリケーションオーナーがコードを計装する必要なく、プラットフォームレベルでそれを実現できるのです」と Neil は述べました。
...
その後、Collector はこれらのメトリクスを `http.client.duration` や `http.server.duration` といったセマンティック規約の名前に変換し、クラスター、サービス名、HTTP ステータスコードごとに集約します。
これにより、コード変更なしですべてのサービスにプラットフォームレベルの HTTP メトリクスが提供され、セマンティック規約に準拠した一貫した命名規則と、ネイティブの Istio メトリクスよりも低いカーディナリティが実現されます。

### 404 エラーの課題 {#the-404-error-challenge}

Collector 設定における注目すべき課題の一つは、キャッシュサービスがキャッシュにエントリが存在しないことを示すために HTTP 404 を返すケースでした。
Collector はこれらの 404 をエラーとして扱い、実際には正常で高頻度の動作に対して 100% のトレースサンプリングをトリガーしていました。

解決策は、これらの特定の 404 レスポンスに対するエラーステータスを解除するフィルタープロセッサーを追加することでした。
...
このプロセッサーは、特定のキャッシュサービスからの 404 ステータスコードを持つスパンに一致し、エラーステータスを解除することで、エラーベースのサンプリングのトリガーを防止します。

> 「最初からそのフィルタープロセッサーがあれば、より高品質で使いやすいトレースが得られていたでしょう」と Neil は振り返りました。

ただし、Neil は、最近導入された OpenTelemetry SDK の[宣言的設定](/docs/languages/sdk-configuration/declarative-configuration/)により、このようなフィルタリングは中央の Collector 設定を変更するのではなく、サービスチーム自身が分散的に設定できるようになったと指摘しています。

### 設定の詳細 {#configuration-deep-dive}

Skyscanner は、これらのパターンを実際に理解するのに役立つよう、本番の Collector 設定を共有しています。

#### ゲートウェイ Collector {#gateway-collector}

[ゲートウェイ Collector][gateway-otelbin] は処理の大部分を担います。
...
- キャッシュサービスの 404 フィルタリングロジックを実装
- メトリクスとトレースを OTLP 経由でオブザーバビリティベンダーにエクスポート

この図は、OTLP メトリクスとトレース、および Istio スパンがどのようにこれらのゲートウェイ Collector に到達するかを示しています。

![Skyscanner アーキテクチャ（ゲートウェイ Collector）図](skyscanner-architecture-gateway.png)

#### エージェント Collector {#agent-collector}

[エージェント Collector][agent-otelbin] は、各ノードからインフラストラクチャおよびプラットフォームレベルのメトリクスを収集することに焦点を当てています。
...
ベース Docker イメージに事前設定された Java エージェントにより、HTTP および gRPC のスパン生成がすぐに利用可能です。

### 独自の方針に基づく自動計装 {#opinionated-auto-instrumentation}

チームは自動計装に対して意図的に独自の方針に基づいたアプローチを取っています。
デフォルトですべてを有効にするのではなく、逆のアプローチを取ります。共有ベース Docker イメージですべての計装を無効にし、厳選されたセットのみを明示的に有効にします。

> 「逆のアプローチなのです。すべてを無効にしてから、必要なものだけを有効にします」と Neil は説明しました。

ベースイメージの環境変数を使用して、Skyscanner はランタイム、HTTP、gRPC に関連する計装のフォーカスされたセットをデフォルトで有効にしています。
これには JAX-RS、gRPC、Jetty、一般的な HTTP クライアント、executor の計装、およびロギングコンテキスト伝搬が含まれます。
サービスチームはこれらのデフォルトを自動的に継承しますが、必要に応じて独自のサービス定義でオーバーライドしたり、追加の計装を有効にしたりする自由があります。

このモデルにより、何百ものサービスにわたる一貫性を確保しつつ、エッジケースでの柔軟性も維持されています。
...
このランチャースクリプト `run.sh` は、デプロイメントが提供する環境変数から `-javaagent` フラグと `otel.resource.attributes` を構築します。

```bash run.sh
# これを使用して OTel リソース属性をセットアップします
# サービス起動時に環境変数から検出できるもののために使用
# これらの変数はデプロイメントシステムによって自動的に設定されます
# 繰り返しを避けるため一部の環境変数は省略されています
setup_otel_agent() {
...
最後に、個別のサービス Dockerfile は同じベースを拡張し、そのサービスに必要な追加の計装のみを追加します。
...
# my-service がデフォルト以外の計装を有効にしたい場合、簡単に拡張できます
...
HTTP および gRPC の計装は有効になっていますが、チームは SDK が生成するほとんどの HTTP および RPC メトリクスを意図的にドロップしています。
これは、前述の通り、Istio サービスメッシュのスパンから一貫した低カーディナリティのプラットフォームメトリクスをすでに導出しているためです。

計装自体を無効にするとスパンも削除されてしまうため、OpenTelemetry SDK のビューを使用して、トレースを維持しながらメトリクスの集約をドロップしています。

- HTTP および RPC メトリクスはグローバルにドロップ
- スパンは通常通り出力を継続
- サービスチームは、Istio が提供するものを超える追加の粒度が必要な場合、特定のメトリクス（たとえばサーバーサイドのレイテンシ）を選択的に再有効化可能

チームが SDK メトリクスをオプトインする場合、既存の Istio 由来メトリクスとの衝突や二重カウントを避けるために、名前を変更することが多いです。
...
# Istio からのメトリクスがすでにあるため、http および rpc メトリクスをドロップ
# 計装を無効にするのではなく、トレースは引き続き機能させたい
- selector:
...
同じファイルを拡張して、サービスが特定のメトリクスを保持する必要がある場合に対応できます。
典型的なユースケースは、`http.route` ごとにリクエストを分類することです。

```yaml
# このドロップ動作は、リストを拡張してさらにビューを追加することで変更できます。
# 保持するメトリクスを明示的に選択します。
# 例: http.server.request.duration メトリクスは保持し、
# http.client.* メトリクスは引き続きドロップする場合
- selector:
    instrument_name: http.server.request.duration
  view:
    # Istio メトリクスに http.server.request.duration という名前がすでにあるため、
    # 衝突や二重カウントを避けるために名前を変更
...
このアプローチにより、Skyscanner は高価値の分散トレースを維持し、メトリクスの重複を回避し、カーディナリティを制御し、インジェストコストを削減できます。これらすべてを、サービスオーナーが OpenTelemetry の内部構造を深く理解する必要なく実現しています。

全体として、この戦略は強力なプラットフォームマインドセットを反映しています。スケールで機能する適切なデフォルトを提供し、ノイズを最小化し、「正しいこと」を簡単にできるようにしつつ、高度なニーズを持つチームがさらに先に進む余地を残しています。

## デプロイメントとリリース管理 {#deployment-and-release-management}

Skyscanner は、必要なすべてが含まれていたため、OpenTelemetry Collector Contrib ディストリビューションを採用しました。
チームはインタビュー中に Contrib が本番環境での使用には推奨されていないことを知り、必要なコンポーネントだけを含むカスタム Collector イメージの構築を検討する予定です。

Skyscanner は約 6 か月ごとに Collector を更新していますが、特定の機能や重要な修正を追跡している場合は、より頻繁にアップグレードします。
リリース情報を把握するために、RSS フィードや CNCF Slack チャンネルをフォローしています。

ロールアウト戦略は、クラスターの階層ごとの段階的なプロモーションを採用しています。Dev クラスター、次に 3 つの Alpha 本番クラスター、続いて 8 つの Beta 本番クラスター、そして最後に残りの 13 の本番クラスターです。
Argo CD をデプロイメントに使用し、階層間のプルリクエストを通じて変更をプロモーションします。

> 「開発テストクラスターで確実に問題を起こしましたが、それ以上プロモーションする前に修正しました」と Neil は述べました。

この段階的なアプローチにより、設定の問題が本番に到達する前に検出されています。
OpenTelemetry Collector デプロイメントの自動テストとロールバック機能はまだ導入していませんが、これらの改善は将来に計画されています。

## うまく機能していること {#what-works-well}

本番環境で OpenTelemetry を採用して以来、チームの経験は非常にポジティブなものでした。

> 「本当に手間がかからなかったです」と Neil は振り返りました。

Collector の最大の強みとして柔軟性が際立っています。

> 「やろうとしたことはすべて実際に実現できました。それはこのツールがいかに柔軟であるかを示していると思います」と Neil は説明しました。

その他のハイライトとしては、シンプルな設定でベンダー非依存を実現する OTLP プロトコル、明確でよく整理されたリリースノート、そしてチームメンバーが Collector コンポーネントのメモリリークを発見し修正に貢献した際のコミュニティの対応の速さが挙げられます。

## 教訓と課題 {#lessons-and-pain-points}

Skyscanner は一部のパイプラインで、まだ古い不安定な HTTP セマンティック規約を使用しています。
アップグレードには、Istio 属性をセマンティック規約名にマッピングする複数の transform プロセッサールールの更新が必要で、ドキュメントを手動で相互参照し、設定文字列を埋めていく作業が伴います。

チームは、セマンティック規約管理のための [Weaver](https://github.com/open-telemetry/weaver) を認識していますが、まだワークフローに統合していません。

6 か月ごとのアップグレードは、複数の破壊的変更を一度に対処することを意味します。
リリースノートはよく書かれており、変更点が明確に文書化されていますが、6 か月分の更新を一度にレビューすることは、リリースペースに追いつく場合と比べて摩擦を生みます。

## 他の組織へのアドバイス {#advice-for-others}

本番環境での経験に基づき、Skyscanner チームは以下のアドバイスを提供しています。

- **シンプルに始める**: メモリリミッター、バッチプロセッサー、基本的なエクスポーターだけで始めましょう。必要が生じたときにのみ複雑さを追加します。
- **初日からメモリリミッターを設定する**: スケール時のメモリ問題を防ぐために、すぐにセットアップしましょう。
- **フィルタープロセッサーを早期に検討する**: アプリケーションのステータスコードのセマンティクスを理解し、コストを制御するために高頻度の「偽陽性」をフィルタリングしましょう。
- **レジリエンシを過剰設計しない**: テレメトリーデータの場合、シンプルなインメモリバッチ処理で十分なことが多いです。
- **段階的なロールアウトで問題を検出する**: 環境の階層ごとの段階的なプロモーションにより、貴重な検証が可能になります。

## 今後の展望 {#whats-next}

このストーリーは、控えめな規模のプラットフォームエンジニアリングチームが、比較的低い運用オーバーヘッドで、大規模な OpenTelemetry Collector インフラストラクチャを成功裏に管理できることを示しています。

このシリーズでは、さまざまな規模の組織が本番環境で OpenTelemetry Collector をどのように活用しているかを引き続き探っていきます。それぞれが独自の課題と創造的なソリューションを持っています。

あなた自身の OpenTelemetry Collector のストーリーを共有しませんか？
CNCF の [#otel-devex](https://cloud-native.slack.com/archives/C01S42U83B2) Slack チャンネルにご参加ください。
本番環境で OpenTelemetry をどのように使用しているか、そして開発者体験をどのように改善し続けられるか、ぜひお聞かせください。
