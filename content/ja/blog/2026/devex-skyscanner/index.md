...
この記事では、スコットランドのエジンバラに拠点を置くグローバルな旅行検索プラットフォームである [Skyscanner](https://www.skyscanner.net/) を取り上げます。

世界中に 1,400 人の従業員を擁し、24 の本番 Kubernetes クラスターで 1,000 を超えるマイクロサービスを運用する Skyscanner の OpenTelemetry に関する取り組みは、大規模に運用する組織にとって貴重な教訓を提供します。

## 組織構造 {#organizational-structure}

6 人のプラットフォームエンジニアで構成される Hubble チームが、Skyscanner の Collector の大部分を管理しています。
...
Java サービスの場合、チームは事前に設定された OpenTelemetry Java エージェントを含むベース Docker イメージを継承します。
Python と Node.js サービスの場合、プラットフォームチームは環境とリソース属性に基づいて適切なデフォルト値を設定するラッパーライブラリを提供します。
これらのアプローチにより、定型的なセットアップが最小限に抑えられ、サービスチームは OpenTelemetry の深い知識がなくても、すぐにオブザーバビリティを利用できます。

## OpenTelemetry の導入 {#opentelemetry-adoption}

Skyscanner の OpenTelemetry への取り組みは 2021 年に始まりました。
同社は内製のオープンソーススタックから商用ベンダーへの移行を進めていました。
しかし、ベンダーロックインは避けたいと考えていました。
...
サービスがグローバルにどこで実行されていても、どのクラスターにあっても、この単一のアドレスにテレメトリーを送信します。
Istio が最も近い利用可能な Collector にリクエストをルーティングします。

デプロイメントは 2 つの異なる Collector パターンで構成されています。

**Gateway Collector（ReplicaSet）**: 大部分のサービスからの OTLP トラフィック（トレースとメトリクス）を処理し、処理の大部分がここで行われます。

**Agent Collector（DaemonSet）**: まだ OTLP をネイティブにサポートしていないオープンソースおよびプラットフォームサービスの Prometheus エンドポイントをスクレイプします。

![Skyscanner アーキテクチャ図](skyscanner-architecture.png)

## 設定: シンプルに始めて、徐々に進化させる {#configuration-start-simple-evolve-gradually}

Skyscanner が 2021 年に初めて Collector をデプロイしたとき、設定は最小限でした。
メモリリミッター、バッチプロセッサー、そしてトレース用の OTLP エクスポーターだけです。

時間の経過とともに、設定は自然に進化しました。
メトリクスパイプラインの追加、Istio スパンの取り込みの統合、スパンからメトリクスへの変換の実装、ノイズを削減しコストを制御するためのフィルタープロセッサーの追加などです。
...
Istio のネイティブメトリクスは、Prometheus のデプロイメントを圧倒するカーディナリティの爆発的増加の問題を抱えていました。
さらに、Skyscanner はコードを所有していないが、一貫したメトリクスが必要な、多くの既製サービスを運用しています。

彼らの解決策は次のとおりです。
Istio がスパンを発行するように設定し（元々は Zipkin 形式でしたが、現在 Istio は OTLP をサポートしています）、Zipkin レシーバーを使用して Collector でスパンを取り込み、セマンティック規約に準拠するように変換し、[span metrics connector](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/e8a502371ea1d2c3534235d623c1b1eb3b6b4b58/connector/spanmetricsconnector?from_branch=main) を使用して、アプリケーションの計装なしに一貫したメトリクスを生成します。

> 「アプリケーションオーナーがコードを計装する必要なく、プラットフォームレベルでそれを行うことができます」と Neil は述べました。
...
Collector はこれらのメトリクスを `http.client.duration` や `http.server.duration` のようなセマンティック規約の名前を使用するように変換し、クラスター、サービス名、HTTP ステータスコードごとに集約します。
これにより、コード変更なしにすべてのサービスのプラットフォームレベルの HTTP メトリクスが提供され、セマンティック規約に準拠した一貫した命名規則と、ネイティブの Istio メトリクスよりも低いカーディナリティが実現されます。

### 404 エラーの課題 {#the-404-error-challenge}

Collector の設定に関する注目すべき課題の 1 つは、キャッシュサービスがキャッシュにエントリが存在しないことを示すために HTTP 404 を返すケースに関するものでした。
...
このプロセッサーは、特定のキャッシュサービスからの 404 ステータスコードを持つスパンを照合し、エラーステータスを解除することで、エラーベースのサンプリングがトリガーされるのを防ぎます。

> 「最初からフィルタープロセッサーがあれば、より高品質で使いやすいトレースが得られたでしょう」と Neil は振り返りました。

しかし Neil は、OpenTelemetry SDK の[宣言的設定](/docs/languages/sdk-configuration/declarative-configuration/)が最近導入されたことで、このようなフィルタリングを中央の Collector 設定を変更するのではなく、サービスチーム自身が分散的に設定できるようになったと述べています。
...
- span metrics connector を使用して Istio スパンからメトリクスを生成します
- 広範な transform プロセッサーを使用して Istio の属性をセマンティック規約にマッピングします
- キャッシュサービスの 404 フィルタリングロジックを実装します
- OTLP 経由でメトリクスとトレースをオブザーバビリティベンダーにエクスポートします
...
[Agent Collector][agent-otelbin] は、各ノードからのインフラストラクチャおよびプラットフォームレベルのメトリクスの収集に焦点を当てています。

- さまざまなソース（node exporter、kube-state-metrics、kubelet）から Prometheus エンドポイントをスクレイプします
- 最小限の処理（メモリ制限、バッチ処理、属性のクリーンアップ）を実行します
- OTLP 経由でメトリクスをオブザーバビリティベンダーにエクスポートします

## 計装戦略 {#instrumentation-strategy}

Skyscanner の Java 中心の環境は、OpenTelemetry の自動計装機能から大きな恩恵を受けています。
ベース Docker イメージに事前に設定された Java エージェントが、HTTP と gRPC のスパン生成をすぐに利用可能な状態で提供します。

### 独自の方針に基づく自動計装 {#opinionated-auto-instrumentation}

チームは、自動計装に対して意図的に独自の方針に基づいたアプローチを取っています。
デフォルトですべてを有効にするのではなく、逆の方向からスタートします。
共有ベース Docker イメージですべての計装を無効にし、厳選されたセットのみを明示的に有効にします。

> 「逆のアプローチです。すべてを無効にしてから、必要なものを有効にします」と Neil は説明しました。
...
サービスチームはこれらのデフォルトを自動的に継承しますが、必要に応じて独自のサービス定義でオーバーライドしたり、追加の計装を有効にしたりすることができます。

このモデルにより、数百のサービス間で一貫性が確保されるとともに、エッジケースでの柔軟性も維持されます。
...
# すべての Java マイクロサービスが拡張する共通ベースイメージの定義
...
```bash run.sh
# OTel リソース属性を設定するために使用
# サービス起動時に環境変数から検出できるものを設定する
# これらの変数はデプロイメントシステムによって自動的に設定される
# 一部の環境変数は重複を避けるために省略している
setup_otel_agent() {
...
# my-service がデフォルト以外の計装を有効にしたい場合、簡単に拡張できる
...
HTTP と gRPC の計装は有効になっていますが、チームは SDK が生成するほとんどの HTTP および RPC メトリクスを意図的にドロップしています。
これは、前述のように、Istio サービスメッシュのスパンから一貫した低カーディナリティのプラットフォームメトリクスをすでに導出しているためです。

計装を完全に無効にするとスパンも削除されてしまうため、チームは OpenTelemetry SDK のビューを使用して、トレースを維持しながらメトリクスの集約をドロップしています。

- HTTP および RPC メトリクスはグローバルにドロップされます
- スパンは通常どおり送信され続けます
- サービスチームは、Istio が提供するものよりも追加の粒度が必要な場合、特定のメトリクス（たとえばサーバーサイドのレイテンシ）を選択的に再有効化できます

チームが SDK メトリクスを再度オプトインする場合、既存の Istio 由来のメトリクスとの衝突や二重カウントを避けるために、メトリクスの名前を変更することが多くあります。
...
# OTEL_EXPERIMENTAL_METRICS_VIEW_CONFIG が指すファイルに保存されている
# Istio からのメトリクスがすでにあるため、http と rpc のメトリクスをドロップする
# 計装を無効にするだけではなく、トレースは引き続き動作させたい
- selector:
...
# 保持するメトリクスを明示的に選択することで変更できる。
# 例: http.server.request.duration メトリクスは保持するが、
...
このアプローチにより、Skyscanner は高価値の分散トレースを維持し、メトリクスの重複を避け、カーディナリティを制御し、取り込みコストを削減できます。
これらはすべて、サービスオーナーが OpenTelemetry の内部構造を深く理解する必要なく実現されています。

全体として、この戦略は強固なプラットフォームの考え方を反映しています。
大規模で機能する適切なデフォルトを提供し、ノイズを最小限に抑え、「正しいこと」を簡単にできるようにしつつ、高度なニーズを持つチームがさらに先に進める余地を残しています。
...
チームはインタビュー中に Contrib は本番環境での使用が推奨されていないことを知り、必要なコンポーネントのみを含むカスタム Collector イメージの構築を検討する予定です。

Skyscanner は約 6 か月ごとに Collector を更新しますが、特定の機能や重要な修正を追跡している場合はより頻繁にアップグレードします。
RSS フィードと CNCF Slack チャンネルでリリース情報を把握しています。

ロールアウト戦略では、クラスターの段階を経て段階的にプロモーションを行います。
Dev クラスター、次に 3 つの Alpha 本番クラスター、続いて 8 つの Beta 本番クラスター、そして残りの 13 の本番クラスターの順です。
Argo CD をデプロイメントに使用し、段階間のプルリクエストを通じて変更がプロモートされます。

> 「開発テスト用のクラスターで確実に問題を起こしたことがありますが、それ以上にプロモートする前に修正しました」と Neil は述べました。

この段階的なアプローチにより、本番環境に到達する前に設定の問題が検出されています。
OpenTelemetry Collector のデプロイメントに対する自動テストとロールバック機能はまだありませんが、これらの改善は計画されています。

## うまくいったこと {#what-works-well}

本番環境で OpenTelemetry を採用して以来、チームの経験は非常にポジティブです。

> 「本当にかなり問題なく進みました」と Neil は振り返りました。

柔軟性が Collector の最大の強みとして際立っています。

> 「やろうとしたことはすべて実現できました。これは Collector の柔軟性を物語っていると思います」と Neil は説明しました。

その他のハイライトとしては、シンプルな設定でベンダー非依存を実現する OTLP プロトコル、明確でよく整理されたリリースノート、チームメンバーが Collector コンポーネントのメモリリークを発見して修正をコントリビュートした際のコミュニティの対応力が挙げられます。

## 教訓とペインポイント {#lessons-and-pain-points}

Skyscanner は、一部のパイプラインで古い不安定な HTTP セマンティック規約をまだ使用しています。
アップグレードには、Istio の属性をセマンティック規約の名前にマッピングする複数の transform プロセッサールールの更新が必要であり、ドキュメントを手動で照合し、設定文字列を埋める作業が伴います。

チームはセマンティック規約の管理のために [Weaver](https://github.com/open-telemetry/weaver) を認識していますが、まだワークフローに統合していません。

6 か月ごとのアップグレードは、複数の破壊的変更を一度に受けることを意味します。
リリースノートはよく書かれており、変更点が明確に文書化されていますが、6 か月分の更新を一度にレビューするのは、リリースに追従し続けるのに比べて摩擦が増します。

## 他の人へのアドバイス {#advice-for-others}

本番環境での経験に基づき、Skyscanner チームは次のアドバイスを提供しています。

- **シンプルに始める**: メモリリミッター、バッチプロセッサー、基本的なエクスポーターだけから始めましょう。
  複雑さは必要に応じてのみ追加します。
- **メモリリミッターは初日から**: スケールに応じてメモリの問題を防ぐために、すぐにセットアップしましょう。
- **フィルタープロセッサーは早めに検討する**: アプリケーションのステータスコードのセマンティクスを理解し、高頻度の「偽陽性」をフィルタリングしてコストを制御しましょう。
- **レジリエンスを過剰に設計しない**: テレメトリーデータの場合、シンプルなインメモリバッチ処理で十分なことが多いです。
- **段階的なロールアウトで問題を検出する**: 環境の段階を経た段階的なプロモーションは、貴重な検証を提供します。

## 次のステップ {#whats-next}

この事例は、適度な規模のプラットフォームエンジニアリングチームが、比較的低い運用オーバーヘッドで大規模な OpenTelemetry Collector インフラストラクチャを成功裏に管理できることを示しています。

このシリーズの次の記事もお楽しみに。
さまざまな規模の組織が本番環境で OpenTelemetry Collector をどのように活用しているか、それぞれ独自の課題と創造的な解決策を引き続き探っていきます。

あなた自身の OpenTelemetry Collector の事例がありますか？
CNCF の [#otel-devex](https://cloud-native.slack.com/archives/C01S42U83B2) Slack チャンネルに参加してください。
あなたが本番環境で OpenTelemetry をどのように使用しているか、そして開発者体験をどのように改善し続けられるかをぜひお聞かせください。
