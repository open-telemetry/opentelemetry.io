---
title:
  'Skyscanner が OpenTelemetry をスケールさせる方法: 24 の本番クラスター全体での
  Collector 管理'
...
Developer Experience SIG は、さまざまな業界や規模の企業における OpenTelemetry の実際のデプロイメントを紹介するブログ記事シリーズを公開しています。
...
サービスチーム自身は、デプロイメントおよびテレメトリー収集インフラストラクチャから抽象化された状態を維持しています。
Java サービスの場合、チームは事前設定された OpenTelemetry Java エージェントを含むベース Docker イメージを継承します。
Python および Node.js サービスの場合、プラットフォームチームは環境とリソース属性に基づいて適切なデフォルトを設定するラッパーライブラリを提供します。
これらのアプローチにより、定型的なセットアップが最小化され、サービスチームは OpenTelemetry の深い知識を必要とせずにすぐにオブザーバビリティを利用できます。

## OpenTelemetry の導入 {#opentelemetry-adoption}

Skyscanner の OpenTelemetry の導入は 2021 年に始まりました。
同社は内部で構築したオープンソーススタックから商用ベンダーへの移行を進めていました。
しかし、ベンダーロックインを避けたいと考えていました。

> 「ベンダー非依存の方法でベンダーに移行したかったのです」と、Skyscanner の Hubble プラットフォームチームのソフトウェアエンジニアである [Neil Fordyce](https://github.com/neilfordyce) は説明しました。

このベンダー非依存のアプローチにより、テレメトリーインフラストラクチャの中核として OpenTelemetry Collector を採用するに至りました。
...
**Gateway Collector（Replica Set）**: 大部分のサービスからの OTLP トラフィック（トレースとメトリクス）の大量処理を行い、処理の大部分がここで行われます。

**Agent Collector（DaemonSet）**: OTLP をネイティブにサポートしていないオープンソースおよびプラットフォームサービスの Prometheus エンドポイントをスクレイプします。

![Skyscanner アーキテクチャ図](skyscanner-architecture.png)

## 設定: シンプルに始めて、段階的に発展させる {#configuration-start-simple-evolve-gradually}

Skyscanner が 2021 年に初めて Collector をデプロイしたとき、設定は最小限でした。メモリリミッター、バッチプロセッサー、トレース用の OTLP エクスポーターだけでした。

時間とともに設定は有機的に発展しました。メトリクスパイプラインの追加、Istio スパンの取り込み統合、スパンからメトリクスへの変換の実装、ノイズを減らしコストを制御するためのフィルタープロセッサーの追加などです。

### Istio サービスメッシュのスパンをプラットフォームメトリクスに変換する {#turning-istio-service-mesh-spans-into-platform-metrics}

Skyscanner の Collector の最も革新的な使い方のひとつは、Istio サービスメッシュのスパンからメトリクスを生成することです。

Istio のネイティブメトリクスはカーディナリティの爆発的増加の問題を抱えており、Prometheus のデプロイメントを圧倒してしまいます。
さらに、Skyscanner はコードを所有していないが一貫したメトリクスが必要な既製サービスを多数運用しています。

彼らの解決策は、Istio にスパンを発行するよう設定し（元々は Zipkin フォーマットでしたが、現在 Istio は OTLP をサポートしています）、Zipkin レシーバーを使って Collector に取り込み、セマンティック規約に合わせて変換し、[span metrics connector](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/e8a502371ea1d2c3534235d623c1b1eb3b6b4b58/connector/spanmetricsconnector?from_branch=main) を使ってアプリケーションの計装なしで一貫したメトリクスを生成することでした。

> 「アプリケーションオーナーがコードを計装する必要なく、プラットフォームレベルでそれができるのです」と Neil は述べました。
...
Collector はこれらのメトリクスを `http.client.duration` や `http.server.duration` などのセマンティック規約名を使うように変換し、クラスター、サービス名、HTTP ステータスコードで集約します。
これにより、コード変更なしですべてのサービスに対するプラットフォームレベルの HTTP メトリクス、セマンティック規約に準拠した一貫した命名、ネイティブの Istio メトリクスよりも低いカーディナリティが実現されます。

### 404 エラーの課題 {#the-404-error-challenge}

Collector 設定で注目すべき課題のひとつは、キャッシュにエントリが存在しないことを示すために HTTP 404 を返すキャッシュサービスに関するものでした。
...
> 「最初からそのフィルタープロセッサーがあれば、より高品質で使いやすいトレースが得られたでしょう」と Neil は振り返りました。

しかし、Neil は OpenTelemetry SDK の[宣言的設定](/docs/languages/sdk-configuration/declarative-configuration/)の最近の導入により、このようなフィルタリングは中央の Collector 設定を変更する必要なく、サービスチーム自身が分散的に設定できるようになったと指摘しています。

### 設定の詳細 {#configuration-deep-dive}

Skyscanner は、他の人がこれらのパターンを実際に理解できるよう、本番 Collector の設定を共有しています。

#### Gateway Collector {#gateway-collector}

[Gateway Collector][gateway-otelbin] は処理の大部分を担っています。
...
下の図は、OTLP メトリクスとトレース、および Istio スパンがこれらの Gateway Collector にどのように到達するかを示しています。
...
- OTLP 経由でオブザーバビリティベンダーにメトリクスをエクスポートする

## 計装戦略 {#instrumentation-strategy}

Skyscanner の Java 中心の環境は、OpenTelemetry の自動計装機能から大きな恩恵を受けています。
ベース Docker イメージに事前設定された Java エージェントが、HTTP および gRPC のスパン生成をすぐに提供します。

### 独自の方針に基づく自動計装 {#opinionated-auto-instrumentation}

チームは自動計装に対して意図的に独自の方針を持ったアプローチを取っています。
デフォルトですべてを有効にするのではなく、反対の方向から始めています。共有ベース Docker イメージですべての計装を無効にし、厳選されたセットのみを明示的に有効にしています。

> 「逆のアプローチです。すべてを無効にしてから、必要なものを有効にするのです」と Neil は説明しました。

ベースイメージの環境変数を使用して、Skyscanner はランタイム、HTTP、gRPC 関連の計装のフォーカスされたセットをデフォルトで有効にしています。
これには JAX-RS、gRPC、Jetty、一般的な HTTP クライアント、エグゼキューターの計装、ロギングのコンテキスト伝搬が含まれます。
サービスチームはこれらのデフォルトを自動的に継承しますが、必要に応じて自身のサービス定義でオーバーライドしたり、追加の計装を有効にしたりする自由があります。

このモデルにより、数百のサービス間で一貫性を確保しながら、エッジでの柔軟性も維持できます。
...
# my-service が他のデフォルトでない計装を有効にしたい場合、簡単に拡張できる
...
### スパンは有効、メトリクスは無効（デフォルトでは） {#spans-yes-metrics-no-by-default}

Skyscanner の戦略で特に興味深いのは、メトリクスとトレースの扱い方です。
HTTP と gRPC の計装は有効になっていますが、チームは SDK が生成する HTTP と RPC のメトリクスのほとんどを意図的にドロップしています。
これは、前述のとおり、Istio サービスメッシュのスパンから一貫性のある低カーディナリティのプラットフォームメトリクスをすでに取得しているためです。

計装を完全に無効にする（スパンも削除されてしまう）のではなく、OpenTelemetry SDK のビューを使用してメトリクスの集約をドロップしながらトレースを維持しています。

- HTTP と RPC のメトリクスはグローバルにドロップされる
- スパンは通常どおり発行され続ける
- サービスチームは、Istio が提供するものを超えた追加の粒度が必要な場合、特定のメトリクス（たとえばサーバーサイドのレイテンシー）を選択的に再有効化できる
...
# Istio からすでにメトリクスを取得しているため、http と rpc のメトリクスをドロップ
# トレースは引き続き動作させたいので、計装自体は無効にしない
- selector:
...
一般的なユースケースは、`http.route` でリクエストを分類することです。

```yaml
# このドロップの挙動は、リストを拡張してビューを追加し、
# 保持するメトリクスを明示的に選択することで変更できる。
# 例: http.server.request.duration メトリクスを保持し、
...
このアプローチにより、Skyscanner は価値の高い分散トレースを維持し、メトリクスの重複を避け、カーディナリティを制御し、取り込みコストを削減できます。これらすべてが、サービスオーナーに OpenTelemetry の内部を深く理解することを要求せずに実現されています。

全体として、この戦略は強力なプラットフォーム思考を反映しています。大規模に機能する適切なデフォルトを提供し、ノイズを最小化し、「正しいこと」を簡単にする一方で、高度なニーズを持つチームがさらに先に進む余地を残しています。
...
チームはインタビュー中に Contrib が本番使用には推奨されていないことを知り、必要なコンポーネントのみを含むカスタム Collector イメージの構築を検討する予定です。

Skyscanner は約 6 ヶ月ごとに Collector を更新しますが、特定の機能や重大な修正を追跡している場合はより頻繁にアップグレードします。
RSS フィードと CNCF Slack チャンネルをフォローしてリリース情報を把握しています。

ロールアウト戦略は、クラスターティア間の段階的なプロモーションを使用します。Dev クラスター、次に 3 つの Alpha 本番クラスター、続いて 8 つの Beta 本番クラスター、最後に残りの 13 の本番クラスターです。
Argo CD をデプロイメントに使用し、変更はティア間のプルリクエストを通じてプロモートされます。

> 「開発テストクラスターでミスをしてしまったことは確実にありますが、それ以上にプロモートする前に修正しました」と Neil は語りました。

この段階的なアプローチにより、設定の問題が本番に到達する前に検出されてきました。
OpenTelemetry Collector のデプロイメントに対する自動テストやロールバック機能はまだありませんが、これらの改善は予定されています。

## うまくいっていること {#what-works-well}

本番で OpenTelemetry を導入して以来、チームの体験は非常にポジティブなものでした。

> 「本当にほとんど苦痛がありませんでした」と Neil は振り返りました。

柔軟性が Collector の最大の強みとして際立っています。

> 「やろうとしたことはすべて、実際に実現できました。それはこのツールの柔軟性を示していると思います」と Neil は説明しました。

その他のハイライトとして、OTLP プロトコルがシンプルな設定によるベンダー独立性を提供していること、明確でよく整理されたリリースノート、そしてチームメンバーが Collector コンポーネントのメモリリークを発見し修正を貢献した際のコミュニティの対応の良さがあります。

## 教訓と課題 {#lessons-and-pain-points}

Skyscanner は一部のパイプラインでまだ古い不安定な HTTP セマンティック規約を使用しています。
アップグレードには、Istio 属性をセマンティック規約名にマッピングする複数の transform プロセッサールールの更新が必要で、ドキュメントを手動でクロスリファレンスし、設定文字列を埋めていく作業が伴います。
...
リリースノートはよく書かれており変更を明確にドキュメント化していますが、6 ヶ月分の更新を一度にレビューすることは、リリースに追随し続ける場合と比べて摩擦を増やします。

## 他の人へのアドバイス {#advice-for-others}

本番での経験に基づき、Skyscanner チームは次のアドバイスを提供します。

- **シンプルに始める**: メモリリミッター、バッチプロセッサー、基本的なエクスポーターだけで始めましょう。
  複雑さはニーズが発生してから追加してください。
- **初日からメモリリミッターを**: スケールに伴うメモリの問題を防ぐために、すぐにセットアップしてください。
- **早い段階でフィルタープロセッサーを検討する**: アプリケーションのステータスコードのセマンティクスを理解し、大量の「偽陽性」をフィルタリングしてコストを制御してください。
- **レジリエンシーを過剰に設計しない**: テレメトリーデータの場合、シンプルなインメモリバッチ処理で十分なことが多いです。
- **段階的なロールアウトで問題を発見する**: 環境ティア間の段階的なプロモーションは、貴重な検証を提供します。

## 次のステップ {#whats-next}

この事例は、適度な規模のプラットフォームエンジニアリングチームが、比較的低い運用オーバーヘッドで大規模な OpenTelemetry Collector インフラストラクチャを成功裏に管理できることを示しています。

このシリーズのさらなる事例にご期待ください。さまざまな規模の組織が本番で OpenTelemetry Collector をどのように活用しているか、それぞれ独自の課題と創造的な解決策とともに引き続き探っていきます。
...
本番で OpenTelemetry をどのように使用しているか、また開発者体験の改善をどのように続けられるか、ぜひお聞かせください。
