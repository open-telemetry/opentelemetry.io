---
title: Skyscanner が OpenTelemetry をスケールさせる方法：24 の本番クラスターでの Collector 管理
...
Developer Experience SIG は、さまざまな業界や規模の企業における OpenTelemetry の実際のデプロイ事例を紹介するブログ記事シリーズを公開しています。
この記事では、スコットランドのエジンバラに拠点を置くグローバルな旅行検索プラットフォームである [Skyscanner](https://www.skyscanner.net/) を取り上げます。

Skyscanner は世界中に 1,400 人の従業員を擁し、24 の本番 Kubernetes クラスター上で 1,000 以上のマイクロサービスを運用しています。
同社の OpenTelemetry 導入の歩みは、大規模運用を行う組織にとって貴重な教訓を提供します。

## 組織構造 {#organizational-structure}

6 人のプラットフォームエンジニアで構成される Hubble チームが、Skyscanner の Collector の大部分を管理しています。
より広いプラットフォームエンジニアリング組織の一部として、Skyscanner の主に Java ベースのマイクロサービスアーキテクチャを実行するコンピュートプラットフォームを担当しています。

サービスチーム自体は、デプロイやテレメトリー収集インフラストラクチャから抽象化されています。
...
これらのアプローチにより、ボイラープレートのセットアップを最小限に抑え、OpenTelemetry の深い知識がなくてもサービスチームにすぐに使えるオブザーバビリティを提供します。
...
このベンダー非依存のアプローチが、テレメトリーインフラストラクチャの中核として OpenTelemetry Collector を採用することにつながりました。

## アーキテクチャ：集中ルーティングと分散収集 {#architecture-centralized-routing-distributed-collection}

Skyscanner の Collector アーキテクチャは、Istio ベースのインテリジェントルーティングを備えた中央 DNS エンドポイントを特徴としています。
サービスがグローバルのどこで実行されていても、どのクラスターにあっても、テレメトリーをこの単一のアドレスに送信します。
Istio が最も近い利用可能な Collector へのリクエストルーティングを処理します。

デプロイは 2 つの異なる Collector パターンで構成されています。

**Gateway Collector（Replica Set）**：大部分の処理が行われる、ほとんどのサービスからの OTLP トラフィック（トレースとメトリクス）を一括処理します。
...
## 設定：シンプルに始めて段階的に進化させる {#configuration-start-simple-evolve-gradually}

Skyscanner が 2021 年に Collector を初めてデプロイしたとき、設定はメモリリミッター、バッチプロセッサー、トレース用の OTLP エクスポーターというミニマルなものでした。

時間とともに設定は自然に進化していきました。
メトリクスパイプラインの追加、Istio スパンの取り込みの統合、スパンからメトリクスへの変換の実装、ノイズの削減とコスト管理のためのフィルタープロセッサーの追加などが行われました。

### Istio サービスメッシュスパンをプラットフォームメトリクスに変換する {#turning-istio-service-mesh-spans-into-platform-metrics}

Skyscanner による Collector の最も革新的な活用の 1 つは、Istio サービスメッシュスパンからメトリクスを生成することです。

Istio のネイティブメトリクスは、Prometheus デプロイを圧倒するカーディナリティ爆発の問題を抱えていました。
さらに、Skyscanner はコードを所有していないものの一貫したメトリクスが必要な、多くの既成サービスを運用しています。

彼らの解決策は次のとおりです。
Istio がスパンを出力するよう設定し（元は Zipkin フォーマットでしたが、現在 Istio は OTLP をサポートしています）、Zipkin レシーバーで Collector に取り込み、セマンティック規約に合わせて変換し、[span metrics connector](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/e8a502371ea1d2c3534235d623c1b1eb3b6b4b58/connector/spanmetricsconnector?from_branch=main) を使ってアプリケーション計装なしで一貫したメトリクスを生成するというものです。

> 「アプリケーションオーナーがコードを計装する必要なく、プラットフォームレベルでこれを実現できるのです」と Neil は述べました。
...
Collector はこれらのメトリクスを `http.client.duration` や `http.server.duration` などのセマンティック規約名に変換し、クラスター、サービス名、HTTP ステータスコードで集約します。
...
Collector はこれらの 404 をエラーとして扱い、実際には通常の高頻度の動作に対して 100% のトレースサンプリングをトリガーしていました。

解決策は、これらの特定の 404 レスポンスに対してエラーステータスをアンセットするフィルタープロセッサーを追加することでした。
...
このプロセッサーは、特定のキャッシュサービスからの 404 ステータスコードを持つスパンにマッチし、エラーステータスをアンセットすることで、エラーベースのサンプリングのトリガーを防ぎます。

> 「最初からそのフィルタープロセッサーがあれば、より高品質で使いやすいトレースが得られていたでしょう」と Neil は振り返りました。

ただし、Neil は最近導入された OpenTelemetry SDK の[宣言的設定](/docs/languages/sdk-configuration/declarative-configuration/)により、このようなフィルタリングは中央の Collector 設定を変更するのではなく、サービスチーム自身が分散的に設定できるようになったと述べています。

### 設定の詳細 {#configuration-deep-dive}

Skyscanner は、これらのパターンを実際に理解するための本番 Collector 設定を公開しています。
...
この図は、OTLP メトリクスとトレース、および Istio スパンがこれらの Gateway Collector に到達する仕組みを示しています。
...
- 最小限の処理（メモリ制限、バッチ処理、属性クリーンアップ）を行います
- メトリクスを OTLP 経由でオブザーバビリティベンダーにエクスポートします

## 計装戦略 {#instrumentation-strategy}

Skyscanner の Java 主体の環境は、OpenTelemetry の自動計装機能から大きな恩恵を受けています。
ベース Docker イメージに事前設定された Java エージェントは、HTTP および gRPC のスパン生成をすぐに利用できる形で提供します。

### こだわりのある自動計装 {#opinionated-auto-instrumentation}

チームは自動計装に対して意図的にこだわりのあるアプローチを取っています。
デフォルトですべてを有効にするのではなく、逆の方向から始めます。
共有ベース Docker イメージですべての計装を無効にし、厳選されたセットのみを明示的に有効にします。

> 「逆のアプローチです。すべてを無効にしてから、必要なものを有効にするのです」と Neil は説明しました。

ベースイメージの環境変数を使って、Skyscanner はランタイム、HTTP、gRPC 関連の計装のフォーカスされたセットをデフォルトで有効にしています。
これには JAX-RS、gRPC、Jetty、一般的な HTTP クライアント、executor 計装、ログコンテキスト伝搬が含まれます。
サービスチームはこれらのデフォルトを自動的に継承しますが、必要に応じて独自のサービス定義でオーバーライドしたり、追加の計装を有効にしたりすることができます。

このモデルにより、数百のサービス間の一貫性を確保しながら、エッジでの柔軟性も維持しています。
...
そのランチャースクリプト `run.sh` は、デプロイが提供する環境変数から `-javaagent` フラグと `otel.resource.attributes` を構築します。

```bash run.sh
# OTel リソース属性を設定するために使用
# サービス起動時に環境変数から検出できるものを設定
# これらの変数はデプロイシステムによって自動的に設定される
# 繰り返しを避けるため一部の環境変数は省略
setup_otel_agent() {
...
最後に、個々のサービスの Dockerfile は同じベースを拡張し、そのサービスが必要とする追加の計装のみを加えます。
...
# my-service が他の非デフォルト計装を有効にしたい場合に簡単に拡張可能
...
HTTP と gRPC の計装は有効にしていますが、チームは SDK が生成する HTTP および RPC メトリクスの大部分を意図的にドロップしています。
これは、前述のとおり、Istio サービスメッシュスパンから一貫した低カーディナリティのプラットフォームメトリクスをすでに導出しているためです。

計装を完全に無効にすると（スパンも削除されてしまうため）、かわりに OpenTelemetry SDK ビューを使用してメトリクス集約をドロップしつつ、トレースは維持しています。

- HTTP と RPC のメトリクスはグローバルにドロップされます
- スパンは通常どおり出力され続けます
- サービスチームは、Istio が提供するものを超える追加の粒度が必要な場合、特定のメトリクス（たとえばサーバーサイドのレイテンシー）を選択的に再有効化できます

チームが SDK メトリクスをオプトインする場合、既存の Istio 由来のメトリクスとの衝突や二重カウントを避けるためにリネームすることがよくあります。
...
```yaml
# このドロップ動作は、明示的に保持するメトリクスを選択するビューを
# リストに追加することで変更できる。
# 例：http.server.request.duration メトリクスを保持するが、
# http.client.* メトリクスのドロップは継続する
- selector:
    instrument_name: http.server.request.duration
  view:
    # Istio メトリクスに http.server.request.duration という名前のものが
    # すでにあるため、衝突と二重カウントを避けるためにリネーム
...
これらすべてを、サービスオーナーが OpenTelemetry の内部を深く理解する必要なく実現しています。

全体として、この戦略は強いプラットフォームマインドセットを反映しています。
大規模に機能する適切なデフォルトを提供し、ノイズを最小限に抑え、「正しいこと」を簡単にできるようにしつつ、高度なニーズを持つチームがさらに先に進む余地も残しています。

## デプロイとリリース管理 {#deployment-and-release-management}

Skyscanner は OpenTelemetry Collector Contrib ディストリビューションを使用しており、必要なものがすべて含まれていたために採用しました。
チームはインタビュー中に Contrib が本番環境での使用を推奨されていないことを知り、必要なコンポーネントのみを含むカスタム Collector イメージの構築を検討する予定です。

Skyscanner は約 6 か月ごとに Collector を更新していますが、特定の機能や重要な修正を追跡している場合はより頻繁にアップグレードします。
RSS フィードや CNCF Slack チャンネルを通じてリリース情報を入手しています。

ロールアウト戦略は、クラスター階層間の段階的なプロモーションを使用しています。
Dev クラスター、次に 3 つの Alpha 本番クラスター、続いて 8 つの Beta 本番クラスター、最後に残りの 13 の本番クラスターです。
Argo CD を使ったデプロイでは、階層間のプロモーションはプルリクエストを通じて行われます。

> 「開発テストクラスターで確実に失敗したことがありますが、その後修正してからさらにプロモーションしました」と Neil は語りました。

この段階的なアプローチにより、設定の問題が本番環境に到達する前に検出されてきました。
OpenTelemetry Collector デプロイの自動テストとロールバック機能はまだ備えていませんが、これらの改善は今後の課題です。

## うまくいっていること {#what-works-well}

OpenTelemetry を本番環境で採用して以来、チームの体験は非常にポジティブなものでした。

> 「本当にほとんど苦痛がありませんでした」と Neil は振り返りました。

柔軟性が Collector の最大の強みとして際立っています。

> 「やろうとしたことはすべて実際に実現できました。これは Collector がいかに柔軟であるかを示していると思います」と Neil は説明しました。

その他のハイライトとしては、OTLP プロトコルによるシンプルな設定でのベンダー独立性、明確でよく整理されたリリースノート、チームメンバーが Collector コンポーネントのメモリリークを発見して修正に貢献した際のコミュニティの対応力が挙げられます。

## 教訓と課題 {#lessons-and-pain-points}

Skyscanner は一部のパイプラインで古い不安定な HTTP セマンティック規約をまだ使用しています。
アップグレードには、Istio 属性をセマンティック規約名にマッピングする複数の transform プロセッサールールの更新が必要であり、ドキュメントを手動でクロスリファレンスして設定文字列を埋める作業が伴います。

チームはセマンティック規約管理のための [Weaver](https://github.com/open-telemetry/weaver) を認識していますが、まだワークフローに統合していません。

6 か月ごとのアップグレードは、一度に複数の破壊的変更に遭遇することを意味します。
リリースノートはよく書かれており変更が明確にドキュメント化されていますが、6 か月分の更新を一度にレビューすることは、リリースに追従し続ける場合と比べて摩擦を増やします。

## 他の組織へのアドバイス {#advice-for-others}

本番環境での経験に基づき、Skyscanner チームは以下のアドバイスを提供しています。

- **シンプルに始める**：メモリリミッター、バッチプロセッサー、基本的なエクスポーターだけで始めましょう。
  複雑さは必要に応じて追加しましょう。
- **メモリリミッターは初日から**：スケール時のメモリ問題を防ぐため、すぐにセットアップしましょう。
- **フィルタープロセッサーを早期に検討する**：アプリケーションのステータスコードのセマンティクスを理解し、高頻度の「偽陽性」をフィルタリングしてコストを管理しましょう。
- **レジリエンシーを過度に設計しない**：テレメトリーデータの場合、シンプルなインメモリバッチ処理で十分なことが多いです。
- **段階的なロールアウトで問題を検出**：環境階層間の段階的なプロモーションは、貴重な検証を提供します。

## 次のステップ {#whats-next}

この事例は、適度な規模のプラットフォームエンジニアリングチームが、比較的低い運用オーバーヘッドで大規模な OpenTelemetry Collector インフラストラクチャを成功裏に管理できることを示しています。

このシリーズの今後の記事にもご期待ください。
さまざまな規模の組織が本番環境で OpenTelemetry Collector をどのように活用しているか、それぞれ独自の課題と創造的な解決策を引き続き紹介していきます。

ご自身の OpenTelemetry Collector の事例を共有しませんか？
CNCF の [#otel-devex](https://cloud-native.slack.com/archives/C01S42U83B2) Slack チャンネルにご参加ください。
本番環境での OpenTelemetry の活用方法や、開発者体験の改善についてぜひお聞かせください。
