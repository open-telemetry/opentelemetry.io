...
sig: Java
default_lang_commit: 0e936e42e952148fbfc5e4285bfa5225d7a165ca
...
OpenTelemetry Java エージェントに対する軽量で柔軟な代替手段を提供し、Spring Boot アプリケーションのオブザーバビリティをこれまで以上に容易にします。

このブログ記事では、Spring Starter を使用すべきタイミング、安定版であることの意味、主な機能、そして直面した課題について説明します。
最後のパートでは、[GraalVM ネイティブイメージ](https://www.graalvm.org/latest/reference-manual/native-image/)アプリケーションを使用して、スターターの機能のいくつかをデモンストレーションします。

すぐに使い始めたい場合は、[Spring Starter のドキュメント](/docs/zero-code/java/spring-boot-starter)をご覧ください。

## Spring Starter を使用すべきタイミング {#when-to-use-the-spring-starter}

以下は、Spring Starter を使用したいシナリオです。
...
- OpenTelemetry Spring Boot スターターを構成するための **Spring Boot 設定ファイル**（`application.properties`、`application.yml`）が OpenTelemetry Java エージェントでは動作しない場合
- Spring Bean を使用した OpenTelemetry Spring Boot スターターの**プログラムによる設定**（[動的な認証ヘッダー](/docs/zero-code/java/spring-boot-starter/programmatic-configuration/#configure-the-exporter-programmatically)など）が必要な場合（OpenTelemetry Java エージェントでは[エクステンション](/docs/zero-code/java/agent/extensions/)が必要です）
- **コード依存関係を使用**: Docker ファイルなどに JVM オプションを追加する必要がありません。`pom.xml` または `build.gradle` ファイルに依存関係と BOM を追加するだけです

少し意外かもしれませんが、Spring ネイティブイメージアプリケーションを構築しない場合、デフォルトの推奨はバイトコード計装による [**OpenTelemetry Java エージェント**](/docs/zero-code/java/agent)の使用です。
後述するように、Java エージェントは Spring Starter の計装よりも多くのすぐに使える計装を提供するためです。

## 安定版であることの意味 {#what-does-it-mean-to-be-stable}

Spring Starter が安定版になったということは、本番環境での使用に対応していることを意味します。

- **安定した API**: API は安定しており、ユーザーに影響を与える形での変更はありません。
- **安定した設定**: 設定オプションは安定しており、ユーザーに影響を与える形での変更はありません。
- **成熟したドキュメント**: [ドキュメント](/docs/zero-code/java/spring-boot-starter/)は成熟しており、Spring Starter のすべての側面をカバーしています。
- **コミュニティサポート**: Spring Starter には、問題が発生した場合に支援してくれるユーザーの[コミュニティ](/community/)（`otel-java` Slack チャンネル）があります。
- **定期的なアップデート**: Spring Starter はアクティブにメンテナンスされており、新機能やバグ修正が定期的に提供されます。

Spring Starter は、まだ安定していない一部のセマンティック規約を使用しており、それらは現在も進化中で将来変更される可能性があることに注意してください。
...
## Spring Starter 安定版リリースの主な機能 {#main-features-of-the-spring-starter-stable-release}

2024年2月に Spring Boot スターター安定版プロジェクトを開始した際、達成したい主な機能を定義しました。
以下のセクションで説明します。

### すぐに使える計装 {#out-of-the-box-instrumentation}

OpenTelemetry スターターは、[最も一般的な用途に対するすぐに使える計装](/docs/zero-code/java/spring-boot-starter/out-of-the-box-instrumentation/)を提供します。
OpenTelemetry Java エージェントにはさらに多くの[すぐに使える計装](/docs/zero-code/java/agent/disable/#suppressing-specific-agent-instrumentation)が含まれていますが、Spring Starter では[少しの設定を追加する](/docs/zero-code/java/spring-boot-starter/additional-instrumentations/)ことで追加の計装をオプトインできます。

大幅に改善した例のひとつが Logback 計装です。

当初は、すべてのログ行を OpenTelemetry Collector に送信することができませんでした。
これは、最初のログ行が出力された時点では OpenTelemetry Bean がまだ利用できなかったためです。
そのため、ログをキャッシュし、OpenTelemetry Bean が作成された後に OpenTelemetry Collector に送信する必要がありました。

以前は、`logback-spring.xml` ファイルに OpenTelemetry Logback アペンダーを追加する必要がありました。
現在では、Spring Boot スターターは、Logback ファイルでアペンダーを定義していない場合、Spring Boot が[ロギングシステムを初期化](https://github.com/open-telemetry/opentelemetry-java-instrumentation/blob/a3f8b1082d8835a81dffd834ec28decca066a3f2/instrumentation/spring/spring-boot-autoconfigure/src/main/java/io/opentelemetry/instrumentation/spring/autoconfigure/internal/instrumentation/logging/LogbackAppenderApplicationListener.java#L64)した後に自動的にアペンダーを追加します。

### 宣言的な SDK 自動構成のセットアップ {#declarative-sdk-autoconfiguration-setup}

Spring Boot スターターでは、OpenTelemetry Java エージェントと同様に、すべての [SDK 自動構成](/docs/languages/java/configuration/)プロパティを設定できます。

しかし当初は、Spring Boot スターターは Spring Boot 設定ファイルで一部の設定プロパティのみをサポートしていました。
一部のプロパティは OpenTelemetry Java エージェントと互換性がありましたが、そうでないものもありました。
また、まったくサポートされていないプロパティもあり、OpenTelemetry Java エージェントと Spring Boot スターター間の切り替えが非常に困難でした。

最初のステップとして、Spring Boot スターターが OpenTelemetry Java エージェントと同じ [SDK 自動構成](/docs/languages/java/configuration/)を使用するようにし、両方で同じプロパティを使用できるようにしました。

しかし、SDK 自動構成は Spring Boot 設定ファイルをサポートしていなかったため、Spring の [Environment](https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/core/env/Environment.html) から Spring 設定値を検索するロジックで [ConfigProperties](https://github.com/open-telemetry/opentelemetry-java/blob/78a917da2e8f4bc3645f4fb10361e3e844aab9fb/sdk-extensions/autoconfigure-spi/src/main/java/io/opentelemetry/sdk/autoconfigure/spi/ConfigProperties.java?from_branch=main) インターフェイスを実装する必要がありました。

リストとマップについては特別な考慮が必要でした。
Spring Boot の Environment はそれらを直接処理できないため、[リスト](https://github.com/open-telemetry/opentelemetry-java-instrumentation/blob/release/v2.6.x/instrumentation/spring/spring-boot-autoconfigure/src/main/java/io/opentelemetry/instrumentation/spring/autoconfigure/internal/properties/SpringConfigProperties.java#L104-L106)と[マップ](https://github.com/open-telemetry/opentelemetry-java-instrumentation/blob/release/v2.6.x/instrumentation/spring/spring-boot-autoconfigure/src/main/java/io/opentelemetry/instrumentation/spring/autoconfigure/internal/properties/SpringConfigProperties.java#L126-L140)用に `@ConfigurationProperties` クラスを記述する必要があります。
幸い、Spring Boot には文字列をリストや[マップ](https://github.com/open-telemetry/opentelemetry-java-instrumentation/blob/release/v2.6.x/instrumentation/spring/spring-boot-autoconfigure/src/main/java/io/opentelemetry/instrumentation/spring/autoconfigure/internal/MapConverter.java)に変換する方法があるため、リソース属性を単一の環境変数（[仕様](/docs/languages/sdk-configuration/general/#otel_resource_attributes)に従って）または [Spring Boot 設定ファイル](/docs/zero-code/java/spring-boot-starter/sdk-configuration/#general-configuration)の両方で渡すことができます。

### プログラムによる SDK 自動構成のセットアップ {#programmatic-sdk-autoconfiguration-setup}

Spring Boot ユーザーは、高度な設定に Spring Bean を使用できることも期待しています。
SDK 自動構成は Spring Bean を認識していなかったため、OpenTelemetry SDK 自動構成によってロードされる Spring Bean を登録できる新しいインターフェイス [ComponentLoader](https://github.com/open-telemetry/opentelemetry-java/blob/release/v1.40.x/sdk-extensions/autoconfigure/src/main/java/io/opentelemetry/sdk/autoconfigure/internal/ComponentLoader.java) を考案しました。

[ComponentLoader インターフェイスの Spring Starter 実装](https://github.com/open-telemetry/opentelemetry-java-instrumentation/blob/f7cba3b86167946b3783fb8e575f1c169aec6972/instrumentation/spring/spring-boot-autoconfigure/src/main/java/io/opentelemetry/instrumentation/spring/autoconfigure/OpenTelemetryAutoConfiguration.java?from_branch=main#L162-L181)は、Spring の `ApplicationContext` を使用して特定の型のすべての Bean を検索します。

これにより、独自のカスタマイザーやその他の SDK コンポーネントを Spring Bean として登録できます。
Spring Bean として実装可能な SPI インターフェイスは、[OpenTelemetry SDK 自動構成 SPI ドキュメント](/docs/languages/java/configuration/#spi-service-provider-interface)で確認できます。
...
これは Spring Boot のイディオマティックなカスタマイズアプローチです。

次の例は、`/actuator` で始まるパスのスパンをドロップするようにサンプラーをカスタマイズする Bean の作成方法を示しています。
...
## OpenTelemetry Spring Boot スターターの実践 {#the-opentelemetry-spring-boot-starter-in-action}

人気のある Spring PetClinic アプリケーションを使用して、OpenTelemetry Spring Boot スターターの機能をいくつかデモンストレーションします。
...
次に、Spring PetClinic アプリケーションに OpenTelemetry Spring Boot スターターの依存関係を追加します。
...
このコマンドラインを動作させるために、`PostgresIntegrationTests` テストクラスを無効にする必要があるかもしれません（[参照](https://github.com/spring-projects/spring-petclinic/issues/1522)）。
...
お好みの OTLP 互換バックエンドも使用できます。
ここではシンプルにするために Collector のログを使用します。

そのために、`spring-petclinic` ディレクトリに以下の `docker-compose-otel.yml` ファイルを追加しましょう。
...
Spring PetClinic アプリケーションと OpenTelemetry Collector を実行しましょう。
...
Spring PetClinic アプリケーションの起動に関するログレコードを確認できます。
...
Web ブラウザーで <http://localhost:8080/vets.html> を開くか、以下の curl コマンドを実行しましょう。

```shell
curl http://localhost:8080/vets.html
```

Collector のログを見ると、トレース ID `16a0a5be5127309858c7c63a76b3f471` で HTTP リクエストに対して1つのスパンが作成されたことがわかります（あなたの場合は異なるトレース ID になります）。
...
同じトレース ID について、データベース計装によって出力されたテレメトリーデータを確認できます。
...
次に、Web ブラウザーで <http://localhost:8080/oups> の URL を選択するか、以下の curl コマンドを実行するとどうなるか見てみましょう。

```shell
curl http://localhost:8080/oups
```

HTTP 呼び出しに関連するスパンが表示されますが、このスパンに付与された `exception` スパンイベントも確認できます。
...
以下に HTTP リクエスト時間に関するメトリクスが表示されています。
...
Spring PetClinic アプリケーションのおかげで、OpenTelemetry Spring Boot スターターの機能をいくつか紹介できました。

その他の機能については、ぜひ [OpenTelemetry Spring Boot スターターのドキュメント](/docs/zero-code/java/spring-boot-starter)をご覧いただき、[Slack](/community/) で質問してください。
