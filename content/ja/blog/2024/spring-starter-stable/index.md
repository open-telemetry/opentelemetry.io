...
sig: Java
default_lang_commit: 7571ed74f52f193815b62ac39c0a68e667bd9f22
...
OpenTelemetry Java エージェントに代わる軽量で柔軟な選択肢を提供し、Spring Boot アプリケーションのオブザーバビリティをこれまで以上に容易にします。

このブログ記事では、Spring スターターをいつ使うべきか、安定版とは実際に何を意味するのか、主な機能は何か、そしてどのような課題に直面したかを説明します。
最後のパートでは、[GraalVM ネイティブイメージ](https://www.graalvm.org/latest/reference-manual/native-image/)アプリケーションを使用して、スターターの機能のいくつかを実演します。

すぐに始めたい場合は、[Spring スターターのドキュメント](/docs/zero-code/java/spring-boot-starter)をご覧ください。

## Spring スターターを使うべきタイミング {#when-to-use-the-spring-starter}
...
- 別のモニタリング Java エージェントがすでに使用されており、OpenTelemetry Java エージェントとの互換性の問題を引き起こしている場合
- **Spring Boot 設定ファイル**（`application.properties`、`application.yml`）を使用して OpenTelemetry Spring Boot スターターを設定したい場合（OpenTelemetry Java エージェントでは機能しません）
- [動的な認証ヘッダー](/docs/zero-code/java/spring-boot-starter/programmatic-configuration/#configure-the-exporter-programmatically)などの OpenTelemetry Spring Boot スターターの**プログラム設定**に Spring Bean を使用する場合（OpenTelemetry Java エージェントではこのために[エクステンション](/docs/zero-code/java/agent/extensions/)が必要です）
- **コード依存関係を使用**: JVM オプション（たとえば Docker ファイル内）を追加する必要はありません。`pom.xml` または `build.gradle` ファイルに依存関係と BOM を追加するだけです

少し意外かもしれませんが、Spring ネイティブイメージアプリケーションを構築しない場合、デフォルトの推奨事項はバイトコード計装を使用した [**OpenTelemetry Java エージェント**](/docs/zero-code/java/agent)です。
後で見るように、Spring スターターの計装よりも多くのすぐに使える計装を提供します。

## 安定版とはどういう意味か {#what-does-it-mean-to-be-stable}

Spring スターターは安定版になり、本番環境で使用する準備ができています。
...
[HTTP セマンティック規約](/docs/specs/semconv/http/http-metrics/)は安定しており、変更されることはありません。
[データベースセマンティック規約](https://github.com/open-telemetry/semantic-conventions/blob/v1.38.0/docs/database/database-metrics.md)はまだ実験的で変更される可能性があります。
ただし、2024年末までに安定版になると予想されています。

## Spring スターター安定版リリースの主な機能 {#main-features-of-the-spring-starter-stable-release}

2024年2月に Spring Boot スターター安定版プロジェクトを開始したとき、達成したい主な機能を定義しました。
以下のセクションで説明します。
...
OpenTelemetry Java エージェントにはさらに多くの[すぐに使える計装](/docs/zero-code/java/agent/disable/#suppressing-specific-agent-instrumentation)が含まれていますが、Spring スターターでは[少しの設定を追加する](/docs/zero-code/java/spring-boot-starter/additional-instrumentations/)ことで追加の計装をオプトインできます。

大幅に改善した例の1つが Logback 計装です。

当初、OpenTelemetry Bean が最初のログ行が出力される時点ではまだ利用可能ではなかったため、すべてのログ行を OpenTelemetry Collector に送信することができませんでした。
...
しかし当初、Spring Boot スターターは Spring Boot 設定ファイルで一部の設定プロパティのみをサポートしていました。
一部のプロパティは OpenTelemetry Java エージェントと互換性がありましたが、そうでないものもありました。
他のプロパティはまったくサポートされておらず、OpenTelemetry Java エージェントと Spring Boot スターター間の切り替えが非常に困難でした。
...
### プログラム的 SDK 自動設定のセットアップ {#programmatic-sdk-autoconfiguration-setup}

Spring Boot ユーザーは、高度な設定に Spring Bean を使用できることも期待しています。
SDK 自動設定は Spring Bean を認識していなかったため、OpenTelemetry SDK 自動設定によってロードされる Spring Bean を登録できる新しいインターフェイス [ComponentLoader](https://github.com/open-telemetry/opentelemetry-java/blob/release/v1.40.x/sdk-extensions/autoconfigure/src/main/java/io/opentelemetry/sdk/autoconfigure/internal/ComponentLoader.java) を考案しました。
...
Spring Bean として実装可能な SPI インターフェイスは、[OpenTelemetry SDK 自動設定 SPI ドキュメント](/docs/languages/java/configuration/#spi-service-provider-interface)で確認できます。
...
これは Spring Boot のカスタマイズに対する慣用的なアプローチです。

この例は、`/actuator` で始まるパスのスパンをドロップするようにサンプラーをカスタマイズする Bean の作成方法を示しています。
...
人気のある Spring PetClinic アプリケーションを使用して、OpenTelemetry Spring Boot スターターの機能をいくつか実演します。
...
お気に入りの OTLP 互換バックエンドを使用することもできます。
ここでは簡単にするために Collector のログを使用します。

これを行うには、`spring-petclinic` ディレクトリに以下の `docker-compose-otel.yml` ファイルを追加することから始めましょう。
...
Spring PetClinic アプリケーションの起動に関するログレコードを1つ確認できます。
...
ウェブブラウザで <http://localhost:8080/vets.html> を開くか、以下の curl コマンドを実行しましょう。

```shell
curl http://localhost:8080/vets.html
```

Collector のログを見ると、Trace ID `16a0a5be5127309858c7c63a76b3f471` の HTTP リクエストに対して1つのスパンが作成されたことがわかります（あなたの場合は異なる Trace ID になります）。
...
同じ Trace ID で、データベース計装によって出力されたテレメトリーデータを確認できます。
...
次に、ウェブブラウザで <http://localhost:8080/oups> の URL を選択するか、以下の curl コマンドを実行するとどうなるか見てみましょう。

```shell
curl http://localhost:8080/oups
```

HTTP 呼び出しに関連するスパンだけでなく、このスパンに付加された `exception` スパンイベントも確認できます。
...
以下に HTTP リクエストの持続時間に関するメトリクスが表示されています。
...
Spring PetClinic アプリケーションを使用して、OpenTelemetry Spring Boot スターターのいくつかの機能を紹介しました。

その他の機能について詳しく知りたい場合は、[OpenTelemetry Spring Boot スターターのドキュメント](/docs/zero-code/java/spring-boot-starter)をご覧になり、[Slack](/community/) で質問してください。
