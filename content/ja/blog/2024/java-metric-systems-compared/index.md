...
- **完全なりんご同士の比較ができない場合があります。** OpenTelemetry の指数ヒストグラムに相当する micrometer の機能はありません。
...
prometheus は OpenTelemetry の OTLP エクスポーターライブラリを直接使用していますが、垂直統合されている（つまりエクスポーターとコアメトリクスシステムが連携して最適な結果を達成する）ことで OpenTelemetry が実現できる最適化がありません。
