...
가관측성(Observability) 프레임워크입니다. 분산 트레이스와 메트릭을 수집하기 위한
단일화된 API, 라이브러리, 에이전트, 컬렉터 서비스를 제공합니다.
...
OpenTelemetry API와 SDK를 사용하여 코드를 한 번만 계측하세요. Jaeger, Prometheus,
상용 벤더, 또는 자체 솔루션 등 원하는 가관측성 백엔드로 텔레메트리 데이터를
내보낼 수 있습니다. 애플리케이션 코드 수정 없이 백엔드를 교체할 수 있습니다.
...
요청 경로 전반에 걸쳐 흐르는 공유 컨텍스트로 트레이스, 메트릭, 로그를
상관관계 지을 수 있습니다. 모든 컴포넌트와 서비스에서 애플리케이션 동작의
완전한 그림을 파악하세요.
...
또는 멀티 클라우드 환경에 완전한 유연성과 벤더 종속 없이 배포하세요.
필요한 곳 어디서나 워크로드를 이동시킬 수 있습니다.

{{% /homepage/main-feature %}}

{{< /homepage/main-features >}}

{{< homepage/signals-showcase title="가관측성 시그널" >}}
