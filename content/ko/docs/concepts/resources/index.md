...
사용하는 경우, 리소스 속성은 **Process** 탭에 모여 있다.
...
추가된다. 이 연결은 나중에 변경할 수 없다. 리소스를 추가하면 해당 프로바이더의
`Tracer`나 `Meter`에서 생성되는 모든 스팬과 메트릭에 해당 리소스가 연결된다.

## SDK가 기본값을 제공하는 시맨틱 속성 {#semantic-attributes-with-sdk-provided-default-value}

오픈텔레메트리(OpenTelemetry) SDK가 제공하는 속성이 있다. 그중 하나인
