...
질문에 답한다. 시스템이 항상 가동 중이더라도 사용자가 검은색 신발을 장바구니에
추가하려고 "장바구니에 추가"를 클릭했을 때 검은색 신발이 추가되지 않는 경우가
있다면, 그 시스템은 **신뢰하기 어려운** 시스템일 수 있다.
...
**분산 트레이스(distributed trace)**는 흔히 **트레이스(trace)**라고 부르며, 단일
...
스팬은 요청의 시작부터 끝까지를 나타낸다. 부모 스팬 아래의 하위 스팬은 요청 중
