...
1.  Deploy an OTel Collector inside Kubernetes configured with a simple OTLP
    gRPC receiver.
...
By default, the Gateway APIs do not come installed in the Kubernetes Cluster. As
of writing this blog, latest is
...
In a production environment, self-signed certificates are not recommended.
Instead, a valid Certificate Authority (CA) should be used. Certificate
management solutions like cert-manager are well-suited for this, as they provide
native support for [trusted CAs](https://cert-manager.io/docs/configuration/ca/)
and integrate seamlessly with tools like
[HashiCorp Vault](https://www.hashicorp.com/en/blog/certificate-management-with-vault).
The key advantage of these solutions is their ability to centrally manage CA
certificates and related security aspects such as certificate auto-rotation,
expiry management, and revocation. Overall it automates and simplifies
...
We are running a container of `otel-collector-contrib` by mounting the `config`
and the `certs`
