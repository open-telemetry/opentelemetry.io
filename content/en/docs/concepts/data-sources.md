...
- [Traces](#tracing-in-opentelemetry) (and [Spans](#spans-in-opentelemetry))
- [Metrics](#metrics-in-opentelemetry)
- [Logs](#logs-in-opentelemetry)
- [Baggage](#baggage-in-opentelemetry)

Below you will find details for each data source.

## Spans in OpenTelemetry

**Spans** are the building blocks of Traces. A Span represents a unit of
work or operation. In OpenTelemetry, they include the following information:
...
**Span Links** exist so that you can associate one span with one or more spans, 
implying a causal relationship. 

For example, let’s say we have a distributed 
system where some operations are tracked by a trace. In response to some of 
these operations, an additional operation is queued to be
...
A **Span Status** will be attached to a span. Typically, you will set a span status when
