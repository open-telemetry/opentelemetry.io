...
If you're looking to see OpenTelemetry in action in Kubernetes the best place to
...
intended to be an example of how to monitor Kubernetes itself. Once you're done
with this walkthrough it can be a fun experiment to install the demo and see how
all all the monitoring responds to an active workload.
...
the collector will be used to collector telemetry emitted by services, logs, and
...
unfamiliar with Helm, checkout [Helm](https://helm.sh/). If you're interested in
...
instance of the collector to gather telemetry related to nodes and workloads
...
- [OTLP Receiver](https://github.com/open-telemetry/opentelemetry-collector/tree/main/receiver/otlpreceiver):
  collects application traces, metrics and logs.
- [Kubernetes Attributes Processor](../collector/components#kubernetes-attributes-processor):
  adds Kubernetes metadata to incoming application telemetry.
- [Kubeletstats Receiver](../collector/components#kubeletstats-receiver): pulls
  node, pod, and container metrics from the API server on a kubelet.
- [Filelog Receiver](../collector/components#filelog-receiver): collects
...
[OTLP format](https://opentelemetry.io/docs/specs/otel/protocol/). If you are
...
To use this values.yaml with the chart, save it to your favorite file location
...
You should now have a daemonset installation of the collector running in your
...
A deployment with exactly 1 replica is used to ensure we don't produce duplicate
data.

This instance of the collector will use the following components:

- [Kubernetes Cluster Receiver](../collector/components#kubernetes-cluster-receiver):
  collects cluster-level metrics and entity events.
...
The OpenTelemetry Collector Helm chart makes configuring all of these components
in a deployment installation of the collector easy. It will also take care of
...
cluster collecting cluster metrics and events!
