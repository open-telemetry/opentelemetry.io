...
If a metric is to be removed it should go through the following stages:

> `development`/`alpha`/`beta`/`stable` metric -> deprecated metric → deleted
> metric
...
The deprecated and deleted stages are lifecycle states, not stability levels.
...
final step before `stable`. It is highly encouraged that metrics in beta stage
are also defined as Semantic Conventions based on the [Collector
guidelines][collectorSemConvGuidelines], ensuring cross-project consistency.
...
[collectorSemConvGuidelines]:
  https://github.com/open-telemetry/opentelemetry-collector/blob/main/docs/coding-guidelines.md#semantic-conventions-compatibility
[semConvGuidelines]: https://opentelemetry.io/docs/specs/semconv/how-to-write-conventions/
