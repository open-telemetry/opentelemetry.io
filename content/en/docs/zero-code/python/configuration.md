...
variable can be set to a comma-separated list of instrumentations entry point names to exclude from
instrumentation. Most of the time the entry point name is the same as the package name and it is set in
...
`opentelemetry-instrumentation-kafka-python` and `opentelemetry-instrumentation-grpc` packages to instrument them. To
