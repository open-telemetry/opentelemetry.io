...
\[1\]: Generic ADO.NET instrumentation that covers `DbCommand` methods. It covers
only implementations for overridden methods. Typically, a lot of implementations
utilize the `new` C# keyword; such cases need specific implementations. This
instrumentation does not cover libraries covered by specific instrumentations
such as `SQLCLIENT`, `NPGSQL`, `MYSQLDATA`, `MYSQLCONNECTOR`, `ORACLEMDA`, or
`SQLITE`.
